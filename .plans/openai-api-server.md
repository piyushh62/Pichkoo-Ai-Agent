# OpenAI-Compatible API Server for Pichkoo AI Agent
...
SSE streaming. By exposing this endpoint, pichkoo-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  pichkoo-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.pichkoo/.env`
2. Run `pichkoo gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with pichkoo-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns pichkoo-agent as a model) |
...
Authorization: Bearer pichkoo-api-key-here
...
But pichkoo-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "pichkoo-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured pichkoo-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of pichkoo-agent's core prompt
...
Once implemented, pichkoo-agent works as a drop-in backend for:
