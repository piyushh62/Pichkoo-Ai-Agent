# Pichkoo AI Agent - Development Guide

Instructions for AI coding assistants and developers working on the pichkoo-agent codebase.

**Never give up on the right solution.**

## What Pichkoo Is

Pichkoo is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Pichkoo ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`pichkoo tools`, `pichkoo setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `PICHKOO_HOME`. Mocks hide integration bugs.
...
- **New `PICHKOO_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `pichkoo tools`
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `pichkoo <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `pichkoo webhook`, `pichkoo cron`, `pichkoo tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.pichkoo/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.pichkoo/pichkoo-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── pichkoo_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── pichkoo-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `pichkoo --tui`
...
Applies to TypeScript across Pichkoo: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`pichkoo_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `PichkooCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.pichkoo/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`pichkoo_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/pichkoo` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `pichkoo_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `PichkooCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `pichkoo --tui` or `PICHKOO_TUI=1`.
...
npm run dev       # watch mode (rebuilds pichkoo-ink + tsx --watch)
npm start         # production
npm run build     # full build (pichkoo-ink + tsc)
...
### TUI in the Dashboard (`pichkoo dashboard` → `/chat`)

The dashboard embeds the real `pichkoo --tui` — **not** a rewrite.  See `pichkoo_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `pichkoo_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `pichkoo --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `pichkoo --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `pichkoo --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `pichkoo-desktop-app-work` skill, not `pichkoo-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Pichkoo built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Pichkoo core.
Use the plugin route instead: create `~/.pichkoo/plugins/<name>/plugin.yaml`
and `~/.pichkoo/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Pichkoo tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_PICHKOO_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_PICHKOO_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `pichkoo_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `pichkoo_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `pichkoo tools`, `pichkoo setup`, most CLI subcommands | `pichkoo_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`pichkoo_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
pichkoo_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.pichkoo/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Pichkoo gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `pichkoo_cli/skin_engine.py`:
...
Users create `~/.pichkoo/skins/<name>.yaml`:
...
Pichkoo has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.pichkoo/plugins/` and pip-installed entry points.

### General plugins (`pichkoo_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.pichkoo/plugins/`, `./.pichkoo/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `pichkoo` at startup so
  `pichkoo <pluginname> <subcmd>` works with no change to `main.py`
...
it at argparse setup time and wires it into `pichkoo <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `pichkoo --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `pichkoo_cli/main.py`, etc.).
...
into `~/.pichkoo/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `pichkoo memory setup` / `post_setup()` without
...
2. User: `$PICHKOO_HOME/plugins/model-providers/<name>/`
...
[`pichkoo-example-plugins`](https://github.com/NousResearch/pichkoo-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `pichkoo skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.pichkoo.tags`, `metadata.pichkoo.category`,
`metadata.pichkoo.related_skills`, `metadata.pichkoo.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.pichkoo.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Pichkoo tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Pichkoo AI Agent" is the secondary collaborator. If the
   contributor's commit shows "Pichkoo AI Agent" as author (because they
   used Pichkoo to draft the skill), replace it with their actual name
...
lives in the `pichkoo-agent-dev` skill at
...
`"messaging"`); `_PICHKOO_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `pichkoo tools` (the curses UI) or the
...
go to `~/.pichkoo/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `pichkoo_cli/curator.py` wires `pichkoo curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.pichkoo/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `pichkoo cron <verb>`
...
- File lock at `~/.pichkoo/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `pichkoo kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `pichkoo_cli/kanban.py` wires `pichkoo kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`pichkoo-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `PICHKOO_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Pichkoo-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `PICHKOO_BACKGROUND_NOTIFICATIONS` env var):
...
Pichkoo supports **profiles** — multiple fully isolated instances, each with its own
...
Existing call sites in `pichkoo_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `pichkoo_cli/curses_ui.py` — see
`pichkoo_cli/tools_config.py` for the canonical pattern.
...
- The plugin disables itself in child processes (sentinel envvar
  `PICHKOO_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
...
| HOME / `~/.pichkoo/` | Your real config+auth.json | Temp dir per test |
