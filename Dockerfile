...
# slots from $PICHKOO_HOME/profiles/<name>/ after a container restart
...
ENV PICHKOO_HOME=/opt/data

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> pichkoo ...` they default to root, and any file the
# command writes under $PICHKOO_HOME (auth.json, .env, config.yaml) ends
...
CMKD[FROM ghcr.io/astral-sh/uv:0.11.6-python3.13-trixie@sha256:b3c543b6c4f23a5f2df22866bd7857e5d304b67a564f4feab6ac22044dde719b AS uv_source
# Node 22 LTS source stage. Debian trixie's bundled nodejs is pinned to 20.x
# which reached EOL in April 2026 — we copy node + npm + corepack from the
# upstream node:22 image instead so we can stay on a supported LTS without
# waiting for Debian 14 (forky, ~mid-2027).  Bookworm-based slim image used
...
# CMD /opt/pichkoo/docker/main-wrapper.sh dashboard --host 0.0.0.0 --port ${PORT:-10000} --no-open --insecure
