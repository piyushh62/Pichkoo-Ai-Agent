# Pichkoo AI Agent Security Policy

This document describes Pichkoo AI Agent's trust model, names the one
...
- Environment details (`pichkoo version`, commit SHA, OS, Python
  version).
...
Pichkoo AI Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Pichkoo AI Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Pichkoo AI Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Pichkoo AI Agent's documentation
...
Pichkoo AI Agent supports two OS-level isolation postures. They address
...
Pichkoo AI Agent supports this in two ways:

- **Pichkoo AI Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Pichkoo AI Agent's in-process heuristics
...
Pichkoo AI Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Pichkoo AI Agent
itself. Bugs in Pichkoo AI Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Pichkoo AI Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Pichkoo AI Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Pichkoo AI Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  pichkoo-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Pichkoo AI Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Pichkoo AI Agent. Bugs in Pichkoo
...
- Pichkoo AI Agent includes supply-chain guards for MCP server
