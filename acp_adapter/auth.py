"""ACP auth helpers — detect and advertise Pichkoo authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "pichkoo-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Pichkoo runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Pichkoo can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Pichkoo.
    ...
    not have Pichkoo provider credentials configured yet, so Pichkoo always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Pichkoo using the currently configured "
