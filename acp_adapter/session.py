"""ACP session manager — maps ACP sessions to Pichkoo AI AgentAgent instances.

Sessions are persisted to the shared SessionDB (``~/.pichkoo/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Pichkoo itself is running in WSL.

    Windows ACP clients can launch ``pichkoo acp`` inside WSL while still sending
    ...
    """
    from pichkoo_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Pichkoo from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["pichkoo-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Pichkoo agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Pichkoo AI AgentAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Pichkoo runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.pichkoo/state.db``) is lazily created.
        """
