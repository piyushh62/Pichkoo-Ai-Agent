...
``auth.json``.  They intentionally have no dependency on ``pichkoo_cli.auth`` so
...
# Sources Pichkoo owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Pichkoo-owned OAuth/device-code state)
