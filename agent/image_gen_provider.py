...
as ``kind: backend``) or ``~/.pichkoo/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``pichkoo tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``pichkoo tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``pichkoo tools`` picker.
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$PICHKOO_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$PICHKOO_HOME/cache/images/``.
