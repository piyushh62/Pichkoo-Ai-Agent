...
appropriate.  All installs go to a Pichkoo-owned bin staging dir,
``<PICHKOO_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
# ``<PICHKOO_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
