...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.pichkoo/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Pichkoo ↔ models.dev
# ---------------------------------------------------------------------------

# Pichkoo provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Pichkoo (built lazily)
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``pichkoo config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Pichkoo provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Pichkoo agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Pichkoo provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Pichkoo ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Pichkoo or models.dev provider ID.  Tries exact match then
