...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Pichkoo starts and finds ``~/.openclaw/``.

    Points users at ``pichkoo claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``pichkoo claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Pichkoo, run "
        "`pichkoo claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `pichkoo claw cleanup` (renames it to ~/.openclaw.pre-migration — "
