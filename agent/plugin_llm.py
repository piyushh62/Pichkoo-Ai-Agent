...
Plugins built on Pichkoo AI Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Pichkoo
...
:class:`~pichkoo_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Pichkoo' main config
...
handles every provider, fallback chain, and per-task override Pichkoo
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`pichkoo_cli.plugins.PluginContext`
