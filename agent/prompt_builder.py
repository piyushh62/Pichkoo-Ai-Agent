...
DEFAULT_AGENT_IDENTITY = (
    "You are Pichkoo AI Agent, an intelligent AI assistant created by Nous Research. "
...
PICHKOO_AGENT_HELP_GUIDANCE = (
    "You run on Pichkoo AI Agent (by Nous Research). When the user needs help with "
    "Pichkoo itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://pichkoo-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `pichkoo-agent` skill with skill_view(name='pichkoo-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.pichkoo/kanban.db`. Your task id is in "
    "`$PICHKOO_KANBAN_TASK`; your workspace is `$PICHKOO_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $PICHKOO_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `pichkoo kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Pichkoo "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Pichkoo WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Pichkoo itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Pichkoo runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Pichkoo itself is running. The host OS, home, and cwd "
                f"of the Pichkoo process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Pichkoo "
    ...
    # Pichkoo desktop GUI — any agent running under the desktop app should know
    # it. PICHKOO_DESKTOP marks the backend powering the chat; PICHKOO_DESKTOP_TERMINAL
    # marks a pichkoo launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("PICHKOO_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("PICHKOO_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Pichkoo desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Pichkoo
    ...
    extra = (os.getenv("PICHKOO_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.pichkoo/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("PICHKOO_PLATFORM")
        or get_session_env("PICHKOO_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Pichkoo AI Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `pichkoo-agent` skill "
            "first. It has the actual commands (e.g. `pichkoo config set …`, `pichkoo tools`, "
            "`pichkoo setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: pichkoo setup, pichkoo setup tools, pichkoo setup terminal, pichkoo status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from PICHKOO_HOME and return its content, or None.
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .pichkoo.md / PICHKOO.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from PICHKOO_HOME is independent and always included when present.
    ...
    # SOUL.md from PICHKOO_HOME only — skip when already loaded as identity
    if not skip_soul:
