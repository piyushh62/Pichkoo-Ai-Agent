...
* The ``bws`` binary is auto-installed into ``<pichkoo_home>/bin/bws`` on
...
# In-process cache so repeated load_pichkoo_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `pichkoo chat -q ...`
...
# <pichkoo_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.pichkoo/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under pichkoo_home/cache/.

    `home_path` is what `load_pichkoo_dotenv()` already resolved; falling back
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<pichkoo_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _pichkoo_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``pichkoo secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _pichkoo_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="pichkoo-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "pichkoo-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<pichkoo_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from pichkoo_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_pichkoo_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `pichkoo secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `pichkoo secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `pichkoo secrets bitwarden setup` to install."
        )
        return result
