...
Owns one Codex thread per Pichkoo session. Drives `turn/start`, consumes
...
# Pichkoo' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_PICHKOO_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Pichkoo/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Pichkoo session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _PICHKOO_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("PICHKOO_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="pichkoo",
            client_title="Pichkoo AI Agent",
            client_version=_get_pichkoo_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Pichkoo' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Pichkoo' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Pichkoo' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Pichkoo approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Pichkoo returns 'once', 'session', 'always', or 'deny'.
...
def _get_pichkoo_version() -> str:
    """Best-effort Pichkoo version string for codex's userAgent line."""
    try:
