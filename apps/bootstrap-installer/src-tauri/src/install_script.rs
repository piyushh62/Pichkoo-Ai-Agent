...
//! Electron app's APP_ROOT/../.. trick, because Pichkoo-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Pichkoo-Setup's build-time
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/NousResearch/pichkoo-agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::new()
        .get(&url)
        .header("User-Agent", "pichkoo-setup/0.0.1")
        .send()
        .await
