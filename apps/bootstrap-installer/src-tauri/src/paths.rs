...
pub fn pichkoo_home() -> PathBuf {
    ...
    {
        // %LOCALAPPDATA%\pichkoo — matches scripts/install.ps1's $PichkooHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("pichkoo");
        }
    }

    // macOS + Linux + fallback: ~/.pichkoo (matches Python get_pichkoo_home(),
    // install.sh, and the Electron desktop's resolvePichkooHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".pichkoo");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".pichkoo")
}

pub fn log_dir() -> PathBuf {
    pichkoo_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    pichkoo_home().join("bootstrap-cache")
}
...
pub fn copy_self_to_pichkoo_home() -> std::io::Result<()> {
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[pichkoo-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
...
#[tauri::command]
pub fn get_pichkoo_home() -> String {
    pichkoo_home().to_string_lossy().into_owned()
}
