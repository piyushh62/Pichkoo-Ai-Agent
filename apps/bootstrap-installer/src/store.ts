...
export const $pichkooHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, pichkooHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_pichkoo_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $pichkooHome.set(pichkooHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Pichkoo" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Pichkoo-Setup.exe --update);
...
export async function launchPichkooDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_pichkoo_desktop', { installRoot })
}
