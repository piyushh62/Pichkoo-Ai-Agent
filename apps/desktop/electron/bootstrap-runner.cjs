 ...
 *
 * Drives apps/desktop's first-launch install of Pichkoo Agent by spawning
...
function bootstrapCacheDir(pichkooHome) {
  return path.join(pichkooHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.pichkoo/pichkoo-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(pichkooHome) {
  if (!pichkooHome) return null
  const candidate = path.join(pichkooHome, 'pichkoo-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(pichkooHome, commit) {
  return path.join(bootstrapCacheDir(pichkooHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/piyushh62/Pichkoo-AI-Agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, pichkooHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(pichkooHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(pichkooHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, pichkooHome } = {}) {
  return new Promise((resolve, reject) => {
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, pichkooHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, pichkooHome }) {
  const args = ['--dir', activeRoot, '--pichkoo-home', pichkooHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, pichkooHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, pichkooHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, pichkooHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, pichkooHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(pichkooHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, pichkooHome, emit })
