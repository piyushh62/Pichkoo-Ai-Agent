 ...
 *
 * Stamps the Pichkoo icon + identity onto the packed Windows Pichkoo.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `pichkoo desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Pichkoo')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Pichkoo'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Pichkoo.exe keeps the stock Electron icon`)
  }
}
