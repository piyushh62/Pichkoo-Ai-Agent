 ...
 * product name (`Pichkoo`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Pichkoo'
 *
 * This is a hard failure with no obvious cause for the user — `pichkoo desktop`
 ...
 * (the mac-unpacked Pichkoo.app bundle) and Windows (win-unpacked), so we
