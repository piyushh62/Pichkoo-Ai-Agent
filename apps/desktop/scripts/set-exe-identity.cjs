#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Pichkoo icon + version metadata onto the
// built Pichkoo.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Pichkoo.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `pichkoo desktop`, the installer's
...
// path (which rebuilds via `pichkoo desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Pichkoo.exe>
//
...
// Stamp the Pichkoo icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Pichkoo',
      FileDescription: 'Pichkoo',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — Pichkoo icon + identity stamped')
}
