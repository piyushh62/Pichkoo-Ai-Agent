...
// no longer bundles the Pichkoo Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Pichkoo.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Pichkoo'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Pichkoo.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'pichkoo'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Pichkoo-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Pichkoo-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Pichkoo-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Pichkoo-${version}-${os}-${arch}.${ext}'
...
function launchFresh() {
  ...
  const pichkooHome = path.join(sandbox, 'pichkoo-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(pichkooHome, { recursive: true })
...
//   - The Pichkoo Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of pichkoo_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'pichkoo-agent', 'pichkoo_cli', 'main.py')
