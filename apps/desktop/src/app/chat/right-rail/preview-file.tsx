...
import { PICHKOO_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  if (window.hermesDesktop.readFileText) {
    try {
      ...
      if (!message.includes("No handler registered for 'pichkoo:readFileText'")) {
        throw error
      }
    }
  }
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(PICHKOO_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
