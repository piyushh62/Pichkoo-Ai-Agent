...
// bug that drives the REAL useGatewayBoot hook + REAL PichkooGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshPichkooConfig: async () => undefined,
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForPichkoo) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startPichkoo()'s remote branch awaits waitForPichkoo() for 45s before it
    ...
    // After ~45s waitForPichkoo gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Pichkoo backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
