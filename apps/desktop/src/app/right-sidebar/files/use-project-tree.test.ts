...
import type { PichkooReadDirResult } from '@/global'

import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<PichkooReadDirResult>>()
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: PichkooReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<PichkooReadDirResult>(resolve => {
