...
import { getPichkooConfig, getPichkooConfigDefaults } from '@/pichkoo'
...
interface PichkooConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function usePichkooConfig({ activeSessionIdRef, refreshProjectBranch }: PichkooConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshPichkooConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getPichkooConfig(), getPichkooConfigDefaults().catch(() => ({}))])
  ...
  return { refreshPichkooConfig, sttEnabled, voiceMaxRecordingSeconds }
}
