...
import {
  getElevenLabsVoices,
  getPichkooConfigDefaults,
  getPichkooConfigRecord,
  getPichkooConfigSchema,
  savePichkooConfig
} from '@/pichkoo'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, PichkooConfigRecord } from '@/types/pichkoo'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<PichkooConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<PichkooConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getPichkooConfigRecord(), getPichkooConfigDefaults(), getPichkooConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          await savePichkooConfig(config)
  ...
  const updateConfig = (next: PichkooConfigRecord) => {
