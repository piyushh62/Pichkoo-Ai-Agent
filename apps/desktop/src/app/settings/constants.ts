...
export const EMPTY_SELECT_VALUE = '__pichkoo_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Pichkoo & Nous-trained models',
  ...
  { prefix: 'PICHKOO_GEMINI_', name: 'Gemini', priority: 4 },
  {
    prefix: 'DEEPSEEK_',
    name: 'DeepSeek',
  ...
  { prefix: 'PICHKOO_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // pichkoo_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Pichkoo needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Pichkoo stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Pichkoo can read from one file request.',
  approvals: {
    mode: 'How Pichkoo handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Pichkoo updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
