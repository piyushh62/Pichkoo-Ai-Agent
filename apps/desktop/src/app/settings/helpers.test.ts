...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: PichkooConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: PichkooConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // PICHKOO_QWEN_ and PICHKOO_GEMINI_ both share the PICHKOO_ stem.
      expect(providerGroup('PICHKOO_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('PICHKOO_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
