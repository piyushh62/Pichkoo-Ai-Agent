...
import { getPichkooConfigDefaults, getPichkooConfigRecord, savePichkooConfig } from '@/pichkoo'
...
export function SettingsView({ gateway, onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getPichkooConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'pichkoo-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await savePichkooConfig(await getPichkooConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
