...
import type { AuxiliaryModelsResponse, ModelOptionProvider, StaleAuxAssignment } from '@/pichkoo'
...
// backend now surfaces the full `pichkoo model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in pichkoo_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `pichkoo model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Pichkoo runs the flow for you.`}
          </p>
        )}
