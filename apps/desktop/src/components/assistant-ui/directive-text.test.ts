...
import { formatRefValue, pichkooDirectiveFormatter } from './directive-text'
...
describe('pichkooDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = pichkooDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = pichkooDirectiveFormatter.parse('@file:src/main.tsx the entry point')
