...
const PICHKOO_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type PichkooRefType = (typeof PICHKOO_REF_TYPES)[number]
...
const ICON_PATHS: Record<PichkooRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as PichkooRefType] ?? ICON_FALLBACK
...
const PICHKOO_DIRECTIVE_RE = new RegExp(
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(PICHKOO_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as PichkooRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: PichkooRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Pichkoo directives (`@file:...`, `@image:...`) as
