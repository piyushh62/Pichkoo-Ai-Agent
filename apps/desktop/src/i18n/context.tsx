...
import { getPichkooConfigRecord, type PichkooConfigRecord, savePichkooConfig } from '@/pichkoo'
...
export interface I18nConfigClient {
  getConfig: () => Promise<PichkooConfigRecord>
  saveConfig: (config: PichkooConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.pichkooDesktop?.api) {
      return Promise.resolve({})
    }

    return getPichkooConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.pichkooDesktop?.api) {
    ...
    return savePichkooConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: PichkooConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: PichkooConfigRecord, locale: Locale): PichkooConfigRecord {
