...
const DESKTOP_COMMAND_META = [
  ...
  ['/profile', 'Switch the active Pichkoo profile'],
  ...
  ['/version', 'Show Pichkoo AI Agent version'],
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  return DESKTOP_COMMANDS.has(canonical) || !isKnownPichkooSlashCommand(normalized)
}

/**
 * An "extension" command is anything the backend surfaces that is NOT one of
 * Pichkoo' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownPichkooSlashCommand(normalized)
}
...
function isKnownPichkooSlashCommand(command: string): boolean {
