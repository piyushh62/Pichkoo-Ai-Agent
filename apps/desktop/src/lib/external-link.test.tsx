...
const desktopWindow = window as unknown as { pichkooDesktop?: Window['pichkooDesktop'] }
const initialPichkooDesktop = desktopWindow.pichkooDesktop

function installDesktopBridge(partial: Partial<Window['pichkooDesktop']> = {}) {
  desktopWindow.pichkooDesktop = {
    fetchLinkTitle: vi.fn().mockResolvedValue(''),
    openExternal: vi.fn().mockResolvedValue(undefined),
    ...partial
  } as unknown as Window['pichkooDesktop']
}
...
afterEach(() => {
  ...
  if (initialPichkooDesktop) {
    desktopWindow.pichkooDesktop = initialPichkooDesktop
  } else {
    delete desktopWindow.pichkooDesktop
  }
})
...
describe('external link helpers', () => {
  ...
  it('deduplicates in-flight title fetches and caches results', async () => {
    ...
    installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['pichkooDesktop']['fetchLinkTitle'] })
  ...
  it('opens links via the desktop bridge', () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['pichkooDesktop']['openExternal'] })
