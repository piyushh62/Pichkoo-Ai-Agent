...
import { PichkooGateway } from '@/pichkoo'
...
const isOpen = (gateway: PichkooGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<PichkooGateway | null>(null)
...
let primaryGateway: PichkooGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: PichkooGateway | null, profile = 'default'): void {
...
export function activeGateway(): PichkooGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new PichkooGateway()
...
export async function ensureActiveGatewayOpen(): Promise<PichkooGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
