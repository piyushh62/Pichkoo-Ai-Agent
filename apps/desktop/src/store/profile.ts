...
import { getProfiles, setApiRequestProfile } from '@/pichkoo'
...
import type { ProfileInfo } from '@/types/pichkoo'
...
// /api/profiles/active `current`). "default" is the root ~/.pichkoo. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'pichkoo.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'pichkoo.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.pichkooDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new PICHKOO_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.pichkooDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'pichkoo.desktop.showAllProfiles'
...
// Switch to the default (root ~/.pichkoo) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.pichkooDesktop?.touchBackend?.(target).catch(() => undefined)
}
