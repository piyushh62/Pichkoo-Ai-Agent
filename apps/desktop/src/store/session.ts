...
import type { PichkooConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistString, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/pichkoo'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'pichkoo.desktop.workspace-cwd'
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.pichkooDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.pichkooDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<PichkooConnection | null>(null)
...
export const setConnection = (next: Updater<PichkooConnection | null>) => updateAtom($connection, next)
