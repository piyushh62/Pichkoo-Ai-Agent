...
import { checkPichkooUpdate, getActionStatus, updatePichkoo } from '@/pichkoo'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: translateNow('notifications.updatePichkoo'), onClick: () => void applyBackendUpdate() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.pichkooDesktop?.getVersion?.()
...
export async function checkBackendUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    const status = mapBackendCheck(await checkPichkooUpdate(true))
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.pichkooDesktop?.updates
...
export async function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    const started = await updatePichkoo()
