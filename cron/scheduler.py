...
Uses a file-based lock (~/.pichkoo/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `pichkoo update` reloads
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``pichkoo tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from pichkoo_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Pichkoo profile.
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("PICHKOO_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid PICHKOO_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within PICHKOO_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against PICHKOO_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within PICHKOO_HOME/scripts/.
    try:
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    os.environ["PICHKOO_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed PICHKOO_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads PICHKOO_SESSION_PLATFORM
    #     and PICHKOO_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via PICHKOO_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     PICHKOO_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read PICHKOO_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the PICHKOO_CRON_AUTO_DELIVER_* vars set
    # below, so clearing PICHKOO_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "PICHKOO_CRON_AUTO_DELIVER_PLATFORM",
        "PICHKOO_CRON_AUTO_DELIVER_CHAT_ID",
        "PICHKOO_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        if delivery_target:
            _VAR_MAP["PICHKOO_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["PICHKOO_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["PICHKOO_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("PICHKOO_MODEL") or ""
        ...
        prefill_file = (
            os.getenv("PICHKOO_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject PICHKOO_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # PICHKOO_HOME. When a workdir is configured, also inject project
        ...
        # override via PICHKOO_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("PICHKOO_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid PICHKOO_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        # Set PICHKOO_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("PICHKOO_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid PICHKOO_CRON_MAX_PARALLEL value; defaulting to unbounded")
