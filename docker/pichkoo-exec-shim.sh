#!/bin/sh
# shellcheck shell=sh
# /opt/pichkoo/bin/pichkoo — `docker exec` privilege-drop shim.
#
# Background
# ----------
...
# returns "Provider authentication failed: Pichkoo is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> pichkoo
...
# (/opt/pichkoo/.venv/bin/pichkoo), so the second hop cannot re-enter this
...
# state via the pichkoo CLI. Default is to drop.

set -e

REAL=/opt/pichkoo/.venv/bin/pichkoo

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "pichkoo-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user pichkoo`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi
...
# Root, no opt-out. Drop to the pichkoo user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "pichkoo-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "pichkoo-shim: re-run with --user pichkoo or set HERMES_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the pichkoo user's home before dropping privileges. Without
