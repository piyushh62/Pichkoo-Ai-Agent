# Pichkoo Observer Hooks

Pichkoo observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Pichkoo' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Pichkoo catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Pichkoo only builds
