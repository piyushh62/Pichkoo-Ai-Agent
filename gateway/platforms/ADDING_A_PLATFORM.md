...
There are two ways to add a platform to the Pichkoo gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.pichkoo/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Pichkoo code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `pichkoo gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `pichkoo_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Pichkoo core.
...
And add it to the `pichkoo-gateway` composite:

```python
"pichkoo-gateway": {
    "includes": [..., "pichkoo-your-platform"]
}
```
...
## 12. Status Display (`pichkoo_cli/status.py`)
...
## 13. Gateway Setup Wizard (`pichkoo_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ pichkoo_cli/ toolsets.py \
  --include="*.py" -l | sort -u
