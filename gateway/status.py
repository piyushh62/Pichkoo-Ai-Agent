...
The PID file lives at ``{PICHKOO_HOME}/gateway.pid``.  PICHKOO_HOME defaults to
``~/.pichkoo`` but can be overridden via the environment variable.  This means
separate PICHKOO_HOME directories naturally get separate PID files — a property
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("PICHKOO_GATEWAY_LOCK_DIR")
    if override:
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Pichkoo gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different PICHKOO_HOME dirs).
    """
