"""
Pichkoo MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Pichkoo-specific extra)

Usage:
    pichkoo mcp serve
    pichkoo mcp serve --verbose
...
logger = logging.getLogger("pichkoo.mcp_serve")
...
class EventBridge:
    ...
    This is the Pichkoo equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Pichkoo MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "pichkoo",
        instructions=(
            "Pichkoo AI Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Pichkoo MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
