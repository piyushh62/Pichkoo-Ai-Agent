...
#   - cli.py, pichkoo_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("PICHKOO_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("PICHKOO_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by PICHKOO_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like pichkoo-cli)
    ...
    # window). Core Pichkoo tools (toolsets._PICHKOO_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from pichkoo_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from pichkoo_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from pichkoo_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from pichkoo_cli.plugins import get_pre_tool_call_block_message
        ...
        try:
            ...
            from pichkoo_cli.middleware import run_tool_execution_middleware
