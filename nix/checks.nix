...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      pichkoo-agent = self'.packages.default;
      pichkooVenv = pichkoo-agent.pichkooVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "pichkoo-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "pichkoo-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "pichkoo-build-package" { } ''
          echo "PASS: package built at ${pichkoo-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "pichkoo-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "pichkoo-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${pichkoo-agent}/bin/pichkoo || (echo "FAIL: pichkoo binary missing"; exit 1)
          test -x ${pichkoo-agent}/bin/pichkoo-agent || (echo "FAIL: pichkoo-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${pichkoo-agent}/bin/pichkoo version 2>&1 | grep -qi "pichkoo" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "pichkoo-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in pichkoo pichkoo-agent pichkoo-acp; do
            test -x ${pichkoo-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "pichkoo-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking pichkoo --help ==="
          ${pichkoo-agent}/bin/pichkoo --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${pichkoo-agent}/bin/pichkoo --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "pichkoo-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${pichkoo-agent}/share/pichkoo-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${pichkoo-agent}/share/pichkoo-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HERMES_BUNDLED_SKILLS" ${pichkoo-agent}/bin/pichkoo || \
        ...
        bundled-plugins = pkgs.runCommand "pichkoo-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${pichkoo-agent}/share/pichkoo-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${pichkoo-agent}/share/pichkoo-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HERMES_BUNDLED_PLUGINS" ${pichkoo-agent}/bin/pichkoo || \
        ...
        bundled-locales = pkgs.runCommand "pichkoo-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${pichkoo-agent}/share/pichkoo-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${pichkoo-agent}/share/pichkoo-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${pichkoo-agent}/share/pichkoo-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "HERMES_BUNDLED_LOCALES" ${pichkoo-agent}/bin/pichkoo || \
          ...
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && HERMES_BUNDLED_LOCALES=${pichkoo-agent}/share/pichkoo-agent/locales \
            ${pichkooVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          ...
          BARE_DIR=$(cd "$HOME" && ${pichkooVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${pichkooVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
        ...
        bundled-tui = pkgs.runCommand "pichkoo-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${pichkoo-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${pichkoo-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HERMES_TUI_DIR" ${pichkoo-agent}/bin/pichkoo || \
        ...
        pichkoo-node = pkgs.runCommand "pichkoo-node-version" { } ''
          set -e
          echo "=== Checking HERMES_NODE in wrapper ==="
          grep -q "HERMES_NODE" ${pichkoo-agent}/bin/pichkoo || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${pichkoo-agent}/bin/pichkoo)
        ...
        managed-guard = pkgs.runCommand "pichkoo-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${pichkoo-agent}/bin/pichkoo config set model foo
          check_blocked "config edit" ${pichkoo-agent}/bin/pichkoo config edit
        ...
        extra-python-packages = let
          ...
          pichkooWithExtra = pichkoo-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "pichkoo-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${pichkooWithExtra}/bin/pichkoo || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${pichkooWithExtra}/bin/pichkoo || \
          ...
          if grep -q "PYTHONPATH" ${pichkoo-agent}/bin/pichkoo; then
        ...
        extra-dependency-groups = let
          pichkooWithGroups = pichkoo-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "pichkoo-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${pichkooWithGroups}"
          echo "venv: ${pichkooWithGroups.pichkooVenv}"
        ...
        messaging-variant = pkgs.runCommand "pichkoo-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.pichkooVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "pichkoo-config-roundtrip" {
