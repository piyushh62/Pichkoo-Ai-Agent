# nix/nixosModules.nix — NixOS module for pichkoo-agent
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.pichkoo-agent;
    effectivePackage =
    ...
    pichkoo-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-pichkoo-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "pichkoo-config-attrs";
      description = "Pichkoo YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "pichkoo-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "pichkoo-documents" { } (
      ''
        mkdir -p $out
    ...
    # pichkoo user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "pichkoo-container-entrypoint" ''
      set -eu
      ...
      if [ ! -f /var/lib/pichkoo-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/pichkoo-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/pichkoo ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/pichkoo
        chmod 0440 /etc/sudoers.d/pichkoo
      fi
    ...
    # Default: /var/lib/pichkoo/workspace → /data/workspace.
  ...
  in {
    options.services.pichkoo-agent = with lib; {
      enable = mkEnableOption "Pichkoo Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = pichkoo-agent;
        description = "The pichkoo-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Pichkoo config (attrset). Deep-merged across module
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `pichkoo gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the pichkoo user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the pichkoo plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Pichkoo discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          pichkoo_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as pichkoo (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in pichkoo-agent's
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.pichkoo symlink to the service
            stateDir. These users are automatically added to the pichkoo group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.pichkoo-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.pichkoo-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the pichkoo user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.pichkoo-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed pichkoo
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure pichkoo is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.pichkoo        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.pichkoo/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.pichkoo/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.pichkoo/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.pichkoo/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.pichkoo/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."pichkoo-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.pichkoo
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.pichkoo ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.pichkoo ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.pichkoo -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.pichkoo/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.pichkoo/$_subdir"
            chmod 2770 "${cfg.stateDir}/.pichkoo/$_subdir"
            find "${cfg.stateDir}/.pichkoo/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # pichkoo-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.pichkoo/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.pichkoo/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.pichkoo/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.pichkoo/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.pichkoo/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.pichkoo/.managed
          chmod 0644 ${cfg.stateDir}/.pichkoo/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.pichkoo/.container-mode <<'HERMES_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    pichkoo_bin=${containerDataDir}/current-package/bin/pichkoo
    HERMES_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.pichkoo/.container-mode
            chmod 0644 ${cfg.stateDir}/.pichkoo/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.pichkoo/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.pichkoo";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.pichkoo" ]; then
                  rm -f "${symlinkPath}"
                  echo "pichkoo-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.pichkoo -> stateDir/.pichkoo for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.pichkoo";
                target = "${cfg.stateDir}/.pichkoo";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "pichkoo-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.pichkoo/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.pichkoo/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.pichkoo/auth.json
              fi
            ''}
          ''}
    ...
    HERMES_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.pichkoo/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.pichkoo/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.pichkoo/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.pichkoo-agent = {
          description = "Pichkoo Agent Gateway";
          ...
          serviceConfig = {
            ...
            # so interactive users in the pichkoo group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.pichkoo-agent = {
          description = "Pichkoo Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/pichkoo gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
