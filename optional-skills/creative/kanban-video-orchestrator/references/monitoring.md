...
pichkoo kanban watch --tenant <project-slug>

# Snapshot of the board
pichkoo kanban list --tenant <project-slug>
pichkoo kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
pichkoo kanban stats --tenant <project-slug>

# Visual dashboard (browser)
pichkoo dashboard

# Inspect a specific task (includes comments + events)
pichkoo kanban show <task-id>

# Follow a single task's event stream
pichkoo kanban tail <task-id>
```

Verify available subcommands with `pichkoo kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `pichkoo kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `pichkoo kanban block <id>` then `pichkoo kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `pichkoo kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `pichkoo kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `pichkoo profile list` (profile exists?), `pichkoo status` (gateway/dispatcher up?), and verify tenant. |
...
pichkoo kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
pichkoo kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(pichkoo kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
pichkoo kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
pichkoo kanban block <task-id>
pichkoo kanban archive <task-id>

# Diagnose what happened
pichkoo kanban show <task-id>      # task body, comments, recent events
pichkoo kanban tail <task-id>      # follow the live event stream
pichkoo kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `pichkoo kanban create` for the director
...
while true; do
    clear
    pichkoo kanban list --tenant <slug>
    echo "---"
    pichkoo kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `pichkoo kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `pichkoo profile list` and start
