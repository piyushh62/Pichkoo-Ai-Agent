...
Maps each role archetype to the Pichkoo skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public pichkoo-agent
...
## Pichkoo skills relevant to video production

### Visual / rendering skills (`pichkoo-agent/skills/creative/`)
...
### Design / pre-production skills (`pichkoo-agent/skills/creative/`)
...
### Audio / media skills (`pichkoo-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`pichkoo-agent/skills/devops/`)
...
These are **not** Pichkoo skills but external CLIs / APIs that profiles invoke.
...
## Built-in Pichkoo tools for media review

These are native Pichkoo tools — not invoked via terminal but through their own
...
key is present in `~/.pichkoo/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Pichkoo profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.pichkoo/.env` → environment variable.
...
When a new Pichkoo-public video skill ships:
