#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `pichkoo kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed pichkoo CLI."""
    try:
        out = subprocess.run(
            ["pichkoo", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `pichkoo kanban list`
    out = subprocess.run(
        ["pichkoo", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["pichkoo", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
