...
Usage (via cron with --no-agent):
    ...
    pichkoo cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$PICHKOO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Pichkoo-Watcher/1.0"})
