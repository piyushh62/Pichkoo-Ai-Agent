...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("PICHKOO_HOME", os.path.expanduser("~/.pichkoo")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("PICHKOO_HOME", Path.home() / ".pichkoo")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Pichkoo config paths
# ═══════════════════════════════════════════════════════════════════

PICHKOO_HOME = Path(os.getenv("PICHKOO_HOME", Path.home() / ".pichkoo"))
CONFIG_PATH = PICHKOO_HOME / "config.yaml"
PREFILL_PATH = PICHKOO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Pichkoo models — already uncensored, just needs clean prompt
    "pichkoo": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "pichkoo" in model_lower or "nous" in model_lower:
        return "pichkoo"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Pichkoo config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.pichkoo/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Pichkoo for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Pichkoo for changes to take effect.")
