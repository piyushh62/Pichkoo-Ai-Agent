"""Windows UTF-8 bootstrap for Pichkoo entry points.
...
should be imported at the very top of every Pichkoo entry point
(``pichkoo``, ``pichkoo-agent``, ``pichkoo-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import pichkoo_bootstrap``
# (or ``from pichkoo_bootstrap import apply_windows_utf8_bootstrap``) at
