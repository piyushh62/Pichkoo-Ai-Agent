...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # <pichkoo-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `pichkoo --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``pichkoo auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`pichkoo auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Pichkoo reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `pichkoo auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `pichkoo auth` is called bare."""
