"""
Backup and import commands for pichkoo CLI.

`pichkoo backup` creates a zip archive of the entire ~/.pichkoo/ directory
...
_EXCLUDED_DIRS = {
    "pichkoo-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to pichkoo root) should be skipped."""
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"pichkoo-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"pichkoo-backup-{stamp}.zip"
    ...
    print(f"\nRestore with: pichkoo import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a pichkoo home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.pichkoo/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a pichkoo dir name
        if prefix in {".pichkoo", "pichkoo"}:
            return prefix + "/"
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # pichkoo_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: pichkoo profile list  (after installing pichkoo)")
...
# — `pichkoo update` snapshots this set before pulling so approved-user lists
...
def run_quick_backup(args) -> None:
    """CLI entry point for pichkoo backup --quick."""
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `pichkoo claw migrate`)
# ---------------------------------------------------------------------------
