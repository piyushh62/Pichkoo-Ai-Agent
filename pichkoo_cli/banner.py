...
Pure display functions with no PichkooCLI state dependency.
"""
...
from pichkoo_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def _fetch_pypi_latest(package: str = "pichkoo-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Pichkoo update is available.

    Two paths: if ``PICHKOO_REVISION`` is set (nix builds embed it), compare
    ...
    # PICHKOO_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `pichkoo update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/pichkoo/update/check`
    ...
    if embedded_rev:
        ...
        # $PICHKOO_HOME/pichkoo-agent/ may be a stale copy from --clone-all;
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Pichkoo git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$PICHKOO_HOME/pichkoo-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``pichkoo_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from pichkoo_cli.build_info import get_build_sha
...
def format_banner_version_label() -> str:
    ...
    base = f"Pichkoo AI Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else PICHKOO_CADUCEUS
    except Exception:
        _bskin = None
        _hero = PICHKOO_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    if os.getenv("PICHKOO_YOLO_MODE"):
    ...
    try:
        from pichkoo_cli.codex_runtime_switch import get_current_runtime
        from pichkoo_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from pichkoo_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built pichkoo; we know an update
    ...
    # Pip-install warning — `pip install pichkoo-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else PICHKOO_AGENT_LOGO
        console.print(_logo)
        console.print()
