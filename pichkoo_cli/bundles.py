"""Implementation of the ``pichkoo bundles`` CLI subcommand.

Mirrors the structure of ``pichkoo_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `pichkoo bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  pichkoo bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``pichkoo bundles`` argparse tree.

    Called from ``pichkoo_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `pichkoo bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``pichkoo bundles <subcommand>`` to the right handler."""
