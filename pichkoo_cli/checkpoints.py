"""`pichkoo checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.pichkoo/checkpoints/``.  Actions:

    pichkoo checkpoints               # same as `status`
    pichkoo checkpoints status        # total size, project count, breakdown
    pichkoo checkpoints list          # per-project checkpoint counts + workdir
    pichkoo checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    pichkoo checkpoints clear [-f]    # nuke the entire base (asks first)
    pichkoo checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    pichkoo checkpoints
    pichkoo checkpoints prune --retention-days 3 --max-size-mb 200
    pichkoo checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: pichkoo checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``pichkoo checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `pichkoo checkpoints` → status
