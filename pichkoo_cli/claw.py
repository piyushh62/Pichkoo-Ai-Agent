"""pichkoo claw — OpenClaw migration commands.

Usage:
    pichkoo claw migrate              # Preview then migrate (always shows preview first)
    pichkoo claw migrate --dry-run    # Preview only, no changes
    pichkoo claw migrate --yes        # Skip confirmation prompt
    pichkoo claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    pichkoo claw migrate --no-backup  # Skip pre-migration snapshot
    pichkoo claw cleanup              # Archive leftover OpenClaw directories
    pichkoo claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Pichkoo may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Pichkoo gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Pichkoo gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'pichkoo gateway stop'.")
    print()
...
def claw_command(args):
    """Route pichkoo claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: pichkoo claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Pichkoo")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'pichkoo claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Pichkoo migration."""
    ...
    # migrate-pichkoo posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: pichkoo claw migrate --source /path/to/.openclaw")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.pichkoo/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: pichkoo claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Pichkoo home ─────────
    # Delegates to pichkoo_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `pichkoo import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: pichkoo import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Pichkoo home."
            )
    ...
    # 'pichkoo claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: pichkoo claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: pichkoo claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  pichkoo claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  pichkoo claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  pichkoo config set OPENROUTER_API_KEY sk-or-v1-...")
