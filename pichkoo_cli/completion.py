"""Shell completion script generation for pichkoo CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Pichkoo AI Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef pichkoo
# Pichkoo AI Agent zsh completion
# Add to ~/.zshrc:
...
_pichkoo_profiles() {{
    ...
    if [[ -d "$HOME/.pichkoo/profiles" ]]; then
        profiles+=($HOME/.pichkoo/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'pichkoo command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Pichkoo AI Agent fish completion",
        "# Add to your config:",
