"""
Configuration management for Pichkoo AI Agent.

Config files are stored in ~/.pichkoo/ for easy access:
- ~/.pichkoo/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.pichkoo/.env         - API keys and secrets

This module provides:
- pichkoo config          - Show current configuration
- pichkoo config edit     - Open config in editor
- pichkoo config set      - Set a specific value
- pichkoo config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``pichkoo config set`` (which rewrites
    ...
    ``config.yaml`` in place: pichkoo never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.pichkoo/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``pichkoo logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  pichkoo config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Pichkoo makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Pichkoo itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``pichkoo update``, the TUI build.
...
#   on every plugin install / ``pichkoo update``.
...
# * ``PICHKOO_HOME`` / ``PICHKOO_PROFILE`` / ``PICHKOO_CONFIG`` /
#   ``PICHKOO_ENV`` — Pichkoo runtime location flags. Writing these into
...
#
# IMPORTANT: ``PICHKOO_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (PICHKOO_GEMINI_CLIENT_ID,
# PICHKOO_LANGFUSE_PUBLIC_KEY, PICHKOO_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Pichkoo runtime location — never via dashboard env writer.
    # NOT a PICHKOO_* blanket: integration credentials (PICHKOO_GEMINI_*,
    # PICHKOO_LANGFUSE_*, PICHKOO_SPOTIFY_*, ...) ARE allowed.
    "PICHKOO_HOME", "PICHKOO_PROFILE", "PICHKOO_CONFIG", "PICHKOO_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Pichkoo runtime location "
            "(PICHKOO_HOME, PICHKOO_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.pichkoo/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `pichkoo plugins enable
    # observability/langfuse` or `pichkoo tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "PICHKOO_LANGFUSE_ENV",
    "PICHKOO_LANGFUSE_RELEASE",
    "PICHKOO_LANGFUSE_SAMPLE_RATE",
    "PICHKOO_LANGFUSE_MAX_CHARS",
    "PICHKOO_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from pichkoo_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("PICHKOO_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    """Check if Pichkoo is running in package-manager-managed mode.

    Two signals: the PICHKOO_MANAGED env var (set by the systemd service),
    or a .managed marker file in PICHKOO_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Pichkoo was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.pichkoo/.install_method`` file (written by installers)
    2. PICHKOO_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    so ``pichkoo update`` bailed with "doesn't apply inside the Docker
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Pichkoo lives in a ``uv tool`` layout.

    ``uv tool install pichkoo-agent`` places the install at
    ``.../uv/tools/pichkoo-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``pichkoo-agent`` happens to be uv-tool-installed on the machine while
    the *active* Pichkoo is a regular pip/venv install, causing
    ``pichkoo update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/pichkoo-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade pichkoo-agent"
        return "pip install --upgrade pichkoo-agent"
    return "pichkoo update"
...
# Long-form text for ``pichkoo update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# pichkoo_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Pichkoo, it doesn't update
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``pichkoo update`` inside Docker.
...
def format_managed_message(action: str = "modify this Pichkoo installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("PICHKOO_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Pichkoo installation is managed by NixOS "
            f"(PICHKOO_MANAGED={env_hint}).\n"
            "Edit services.pichkoo-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Pichkoo installation is managed by Homebrew "
            f"(PICHKOO_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade pichkoo-agent"
        )

    return (
        f"Cannot {action}: this Pichkoo installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Pichkoo."
    )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    """
    if os.environ.get("PICHKOO_DEV") == "1":
        return None
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("PICHKOO_UID", "").strip()
    gid_str = os.environ.get("PICHKOO_GID", "").strip()
    try:
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the pichkoo group can
    share state with the gateway service.

    The mode can be overridden via the PICHKOO_HOME_MODE environment variable
    (e.g. PICHKOO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse PICHKOO_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``PICHKOO_UID``/``PICHKOO_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("PICHKOO_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Pichkoo runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("PICHKOO_CONTAINER") or os.environ.get("PICHKOO_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set PICHKOO_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into PICHKOO_HOME if the user doesn't have one yet."""
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"PICHKOO_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Pichkoo-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Pichkoo
        ...
        # identity slot (SOUL.md). Empty by default. The PICHKOO_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Pichkoo auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Pichkoo sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Pichkoo
        ...
        # bundled Pichkoo image, which drops to the `pichkoo` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Pichkoo sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Pichkoo should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``pichkoo chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.pichkoo/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: pichkoo sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Pichkoo truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``pichkoo kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``pichkoo kanban decompose`` and the kanban
        ...
        # ``pichkoo profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `pichkoo model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `pichkoo` (and `pichkoo chat`) launches by default:
        ...
        # REPL and `--tui` (or PICHKOO_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `pichkoo --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `pichkoo -c` muscle memory.  Default off so existing
        # users aren't surprised.  PICHKOO_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `pichkoo --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/pichkoo`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``PICHKOO_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``PICHKOO_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``PICHKOO_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``PICHKOO_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/pichkoo``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Pichkoo appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.pichkoo/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.pichkoo/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Pichkoo feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Pichkoo auto-pauses the goal and
    ...
    # always goes to ~/.pichkoo/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${PICHKOO_SKILL_DIR} and ${PICHKOO_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `pichkoo curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `pichkoo update`, so pruning them only
        ...
        # skipped), snapshot ~/.pichkoo/skills/ into
        # ~/.pichkoo/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `pichkoo curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for pichkoo-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Pichkoo AI Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (PICHKOO_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.pichkoo/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or PICHKOO_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `pichkoo doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``pichkoo_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Pichkoo to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via PICHKOO_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `pichkoo -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `pichkoo` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `pichkoo kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     pichkoo-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Pichkoo tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.pichkoo/logs/.
    ...
    # update model picker lists without shipping a pichkoo-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://pichkoo-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `pichkoo model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.pichkoo/.env,
        ...
        # behavior: files must live under the Pichkoo cache, under
        ...
        # user. Bridged to PICHKOO_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Pichkoo
        # cache (~/.pichkoo/cache/{documents,images,audio,video,screenshots})
        ...
        # to PICHKOO_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # PICHKOO_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to PICHKOO_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.pichkoo/state.db.
    ...
    "sessions": {
        ...
        # default of ``pichkoo sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.pichkoo/sessions/session_{sid}.json`` on every turn
    ...
    # ``pichkoo update`` behaviour.
    "updates": {
        # Run a full ``pichkoo backup``-style zip of PICHKOO_HOME before every
        # ``pichkoo update``.  Backups land in ``<PICHKOO_HOME>/backups/`` and
        # can be restored with ``pichkoo import <path>``.  Off by default —
        # on large PICHKOO_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `pichkoo update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<PICHKOO_HOME>/lsp/bin/`` on first use.
    ...
    # `pichkoo tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.pichkoo/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.pichkoo/.env (or your shell) and never in config.yaml.
            ...
            # ~/.pichkoo/bin/ on first use.  When False you must install
            ...
            # `pichkoo secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'pichkoo model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Pichkoo auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @pichkoo:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. PICHKOO_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #pichkoo)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: pichkoo-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'pichkoo-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Pichkoo API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Pichkoo API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Pichkoo API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # PICHKOO_TOOL_PROGRESS and PICHKOO_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.pichkoo.config`` entries, then checks
    ...
    try:
        ...
        # should never break `pichkoo update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Pichkoo versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``pichkoo_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'pichkoo setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Pichkoo won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'pichkoo doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("PICHKOO_HOME", "~/.pichkoo")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("PICHKOO_TOOL_PROGRESS")
            old_mode = get_env_value("PICHKOO_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from PICHKOO_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from PICHKOO_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("PICHKOO_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from PICHKOO_TIMEZONE)")
            else:
                config["timezone"] = ""
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `pichkoo plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `pichkoo curator status` has no
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: pichkoo config set <key> <value>")
    ...
    # metadata.pichkoo.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.pichkoo/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.pichkoo/config.yaml.
    ...
    (which change ``PICHKOO_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — pichkoo auth) — OpenAI Codex
#   nous         (OAuth — pichkoo auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.pichkoo/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.pichkoo/.env.
    ...
    interactive menu render (`pichkoo tools`, `pichkoo setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `pichkoo tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Pichkoo env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.pichkoo/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.pichkoo/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.pichkoo/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.pichkoo/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Pichkoo — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Pichkoo vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.pichkoo/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Pichkoo Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from pichkoo_cli.auth import get_anthropic_key
    ...
    # Warn on stale PICHKOO_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("PICHKOO_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale PICHKOO_MAX_ITERATIONS={_env_ghost} "
                f"(run 'pichkoo doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  pichkoo config edit     # Edit config file", Colors.DIM))
    print(color("  pichkoo config set <key> <value>", Colors.DIM))
    print(color("  pichkoo setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: pichkoo config set <key> <value>")
            print()
            print("Examples:")
            print("  pichkoo config set model anthropic/claude-sonnet-4")
            print("  pichkoo config set terminal.backend docker")
            print("  pichkoo config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'pichkoo config migrate' to add them")
        ...
        print("  pichkoo config           Show current configuration")
        print("  pichkoo config edit      Open config in editor")
        print("  pichkoo config set <key> <value>   Set a config value")
        print("  pichkoo config check     Check for missing/outdated config")
        print("  pichkoo config migrate   Update config with new options")
        print("  pichkoo config path      Show config file path")
        print("  pichkoo config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``pichkoo config`` UI so users
