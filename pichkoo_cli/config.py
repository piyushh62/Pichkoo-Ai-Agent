...
Hermes Agent runs as a published image (nousresearch/pichkoo-agent), not a
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # bundled Hermes image, which drops to the `pichkoo` user via
    ...
    "updates": {
        # Run a full ``pichkoo backup``-style zip of HERMES_HOME before every
        # ``pichkoo update``.  Backups land in ``<HERMES_HOME>/backups/`` and
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HERMES_HOME", "~/.pichkoo")
