...
``$PICHKOO_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-pichkoo-setup
(the stage2 hook) has chowned the volume and seeded $PICHKOO_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$PICHKOO_HOME``,
    not under ``profiles/``). The dispatcher in ``pichkoo_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``pichkoo gateway start`` (no ``-p``) targets. Without it,
    bare ``pichkoo gateway start`` inside the container would land on
    ...
    ``$PICHKOO_HOME/gateway_state.json`` (sibling to the profile root,
    ...
    # ``pichkoo gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `pichkoo profile create` (config.yaml
            # is not — that comes later via `pichkoo setup`). Use it as the
            ...
            # via ``pichkoo -p default-named gateway start`` if they
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    if os.environ.get("PICHKOO_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "pichkoo":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `pichkoo -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit pichkoo
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the pichkoo user) won't hit EACCES. See
