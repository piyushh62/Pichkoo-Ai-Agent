"""
Cron subcommand for pichkoo CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(pichkoo\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*pichkoo)"
    r"|(systemctl\s+(restart|stop|start)\s+.*pichkoo)"
    r"|(p?kill\s+.*pichkoo.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'pichkoo cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: pichkoo gateway install", Colors.DIM))
        print(color("                    sudo pichkoo gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    pichkoo gateway install    # Install as a user service")
        print("    sudo pichkoo gateway install --system  # Linux servers: boot-time system service")
        print("    pichkoo gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `pichkoo gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: pichkoo cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
