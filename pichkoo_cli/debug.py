"""``pichkoo debug`` debug tools for Pichkoo AI Agent.

Currently supports:
    pichkoo debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.pichkoo/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[pichkoo debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.pichkoo/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``pichkoo debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``pichkoo debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Pichkoo version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `pichkoo debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "pichkoo-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``pichkoo debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.pichkoo/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``pichkoo debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "pichkoo-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----PichkooDebugBoundary9f3c"
...
def _capture_dump() -> str:
    """Run ``pichkoo dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``pichkoo dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``pichkoo debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "pichkoo debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `pichkoo debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  pichkoo debug delete <url>")

    print(f"\nShare these links with the Pichkoo team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: pichkoo debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'pichkoo debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``pichkoo debug`` call.
    ...
    # best-effort — any failure is swallowed so ``pichkoo debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: pichkoo debug <command>")
        print()
        print("Commands:")
