"""
Gateway subcommand for pichkoo CLI.

Handles: pichkoo gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "pichkoo CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``pichkoo gateway status`` from falsely counting the ``pichkoo`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``pichkoo gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "pichkoo_cli.main gateway",
        "pichkoo_cli.main --profile",
        "pichkoo_cli.main -p",
        "pichkoo_cli/main.py gateway",
        "pichkoo_cli/main.py --profile",
        "pichkoo_cli/main.py -p",
        "pichkoo gateway",
        ...
        # ``pichkoo.exe --profile`` / ``pichkoo.exe -p`` would also match
        # ``pichkoo.exe --profile foo dashboard`` and other CLI subcommands,
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. PICHKOO_HOME
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``pichkoo update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Pichkoo profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Pichkoo profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "pichkoo_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``pichkoo update`` users
    # with no running gateway until they re-invoke ``pichkoo gateway``
    ...
    from pichkoo_cli._subprocess_compat import (
...
def _sync_pichkoo_home_from_systemd_unit(system: bool) -> None:
    ...
    unit_home = env.get("PICHKOO_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("PICHKOO_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["PICHKOO_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}pichkoo gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}pichkoo gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from pichkoo_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `pichkoo gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``pichkoo gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from pichkoo_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (PICHKOO_HOME-scoped).
...
from pichkoo_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Pichkoo Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``pichkoo gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``PICHKOO_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("PICHKOO_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "pichkoo-gateway"
SERVICE_DESCRIPTION = "Pichkoo AI Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current PICHKOO_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (PICHKOO_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary PICHKOO_HOME paths
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this PICHKOO_HOME.

    Default ``~/.pichkoo`` returns ``pichkoo-gateway`` (backward compatible).
    Profile ``~/.pichkoo/profiles/coder`` returns ``pichkoo-gateway-coder``.
    Any other PICHKOO_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Pichkoo installs that predate the
# pichkoo-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (pichkoo-gateway-*.service) and unrelated third-party
# "pichkoo" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("pichkoo.service",)
...
def _find_legacy_pichkoo_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``pichkoo.service`` before the rename to
    ``pichkoo-gateway.service``). When both a legacy unit and the current
    ``pichkoo-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``pichkoo-gateway-coder.service`` and unrelated third-party
      ``pichkoo-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``pichkoo.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy Pichkoo gateway unit files if any are installed.
    ...
    print_info("  These run alongside the current pichkoo-gateway service and")
...
def remove_legacy_pichkoo_units(
    ...
    print()
    print("Legacy Pichkoo gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: pichkoo gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo pichkoo gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    pichkoo gateway uninstall")
    print_info("    sudo pichkoo gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Pichkoo can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo pichkoo gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo pichkoo gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo pichkoo gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Pichkoo often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.pichkoo`` → ``ai.pichkoo.gateway.plist`` (backward compatible).
    Profile ``~/.pichkoo/profiles/coder`` → ``ai.pichkoo.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.pichkoo.gateway-{suffix}" if suffix else "ai.pichkoo.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.pichkoo/pichkoo-agent  -> /home/alice/.pichkoo/pichkoo-agent
      /opt/pichkoo                 -> /opt/pichkoo  (kept as-is)
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m pichkoo_cli.main``, so module
    ...
    dir, or a clone that ``pichkoo update`` later relocates/removes — the path
    ...
    ``PICHKOO_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if PICHKOO_HOME
...
User={username}
Group={group_name}
ExecStart={python_path} -m pichkoo_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Pichkoo install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo pichkoo gateway uninstall --system")
    print_info("         pichkoo gateway install")
    print_info("         pichkoo gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("PICHKOO_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (pichkoo.service from pre-rename installs)
    # before installing the new pichkoo-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}pichkoo gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}pichkoo gateway status{scope_flag}             # Check status"
    )
    print(
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `pichkoo gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `pichkoo gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}pichkoo gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}pichkoo gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}pichkoo gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}pichkoo gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.pichkoo.gateway-{suffix}" if suffix else "ai.pichkoo.gateway"
...
# (the documented `nohup pichkoo gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m pichkoo_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active PICHKOO_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "pichkoo_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup pichkoo gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: pichkoo gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup pichkoo gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>pichkoo_cli.main</string>",
    ]
    if profile_arg:
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Pichkoo install"
    )
    return True
...
def launchd_stop():
    ...
    # `pichkoo gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Pichkoo install")
    else:
        print("⚠ Service definition is stale relative to the current Pichkoo install")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("PICHKOO_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Pichkoo gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'pichkoo' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/pichkoo/docker/entrypoint.sh before the Pichkoo command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$PICHKOO_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set PICHKOO_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `pichkoo gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set PICHKOO_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `pichkoo gateway restart` which already
    ...
    # `pichkoo gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Pichkoo Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # PICHKOO_GATEWAY_EXIT_DIAG env var (default: on while we're still
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Pichkoo will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @pichkoo:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. pichkoo) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Pichkoo delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Pichkoo agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Pichkoo will use (e.g., pichkoo@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Pichkoo connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: pichkoo pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Pichkoo will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``pichkoo setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run pichkoo under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.pichkoo/plugins/ still require
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from pichkoo_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'pichkoo pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: pichkoo pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'pichkoo gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from pichkoo_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from pichkoo_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: pichkoo pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'pichkoo gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from pichkoo_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Pichkoo will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Pichkoo will store the returned account_id/token in ~/.pichkoo/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `pichkoo gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `pichkoo pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `pichkoo pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "PichkooAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``pichkoo_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from pichkoo_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.pichkoo/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.pichkoo/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: pichkoo gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: pichkoo gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: pichkoo gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo pichkoo gateway install --system"
                        )
                    print_info("  Or run in foreground:  pichkoo gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: pichkoo gateway run")
                print_info(
                    "  For persistence:   tmux new -s pichkoo 'pichkoo gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: pichkoo gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup pichkoo gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'pichkoo gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`pichkoo_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from pichkoo_cli.service_manager import (
    ...
    if profile is None:
        ...
        # PICHKOO_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``pichkoo gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``pichkoo gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``pichkoo gateway run`` are unaffected.
      2. ``PICHKOO_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``pichkoo gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``PICHKOO_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("PICHKOO_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("PICHKOO_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${PICHKOO_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if PICHKOO_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or PICHKOO_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-pichkoo service's
    # pattern in docker/s6-rc.d/main-pichkoo/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: pichkoo gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: pichkoo gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s pichkoo 'pichkoo gateway run'"
                )
                print()
            ...
            print()
            print(
                "  pichkoo gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s pichkoo 'pichkoo gateway run'         # persistent via tmux"
            )
            print(
                "  nohup pichkoo gateway run > ~/.pichkoo/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  pichkoo profile create <name>     # creates the s6 service slot")
                print("  pichkoo -p <name> gateway start   # bring it up via s6")
                print("  pichkoo status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: pichkoo gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: pichkoo gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: pichkoo gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  pichkoo profile delete <name>     # tears down the s6 service slot")
                print("  pichkoo -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `pichkoo -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: pichkoo gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_PICHKOO_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `pichkoo gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_PICHKOO_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `pichkoo gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: pichkoo gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  pichkoo gateway install")
                else:
                    print("To install as a service:")
                    print("  pichkoo gateway install")
                    print("  sudo pichkoo gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  pichkoo gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup pichkoo gateway run > ~/.pichkoo/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s pichkoo 'pichkoo gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  pichkoo gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  pichkoo gateway install  # Install as user service")
                    print(
                        "  sudo pichkoo gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Pichkoo gateway unit files from
        # pre-rename installs (e.g. pichkoo.service). Profile units and
        # unrelated third-party services are never touched.
