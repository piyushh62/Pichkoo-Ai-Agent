...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m pichkoo_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Pichkoo_Gateway"
_TASK_DESCRIPTION = "Pichkoo AI Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Pichkoo profile."""
    from pichkoo_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "pichkoo_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("PICHKOO_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("PICHKOO_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("PICHKOO_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["PICHKOO_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["PICHKOO_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["PICHKOO_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("PICHKOO_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("PICHKOO_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("PICHKOO_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Pichkoo_Gateway``
    Named profile X: ``Pichkoo_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during pichkoo_cli boot.
    from pichkoo_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\pichkoo\\gateway-service\\<task_name>.cmd``
    (or ``<PICHKOO_HOME>/gateway-service/<task_name>.cmd`` so per-profile
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``PICHKOO_HOME`` whenever it
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports PICHKOO_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m pichkoo_cli.main [--profile X] gateway run``
    ...
    # if someone imports pichkoo_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "pichkoo_cli.main"]
    if profile_arg:
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Pichkoo builds and failed
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m pichkoo_cli.main gateway run``
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("PICHKOO_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("PICHKOO_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from pichkoo_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `pichkoo -p <profile> gateway install` must be safe.
    ...
    from pichkoo_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"pichkoo {profile_arg} gateway start" if profile_arg else "pichkoo gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: pichkoo gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Pichkoo gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: pichkoo gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: pichkoo gateway start")
        _print_next_steps()
        return
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Pichkoo gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: pichkoo gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: pichkoo gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: pichkoo gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from pichkoo_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
