"""
Pichkoo Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``pichkoo desktop`` / ``pichkoo gui``)
     Built inside the agent checkout under ``$PICHKOO_HOME/pichkoo-agent/``:
       ...
       - ``$PICHKOO_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Pichkoo.app`` or ``~/Applications/Pichkoo.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Pichkoo`` (NSIS per-user)
...
the app name ("Pichkoo"), separate from ``$PICHKOO_HOME``:
  - macOS:   ``~/Library/Application Support/Pichkoo``
  - Windows: ``%APPDATA%\\Pichkoo``
  - Linux:   ``$XDG_CONFIG_HOME/Pichkoo`` (default ``~/.config/Pichkoo``)
...
``pichkoo uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Pichkoo"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Pichkoo"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Pichkoo" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Pichkoo.app"),
            home / "Applications" / "Pichkoo.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Pichkoo).
            local_base / "Programs" / "Pichkoo",
            # Older / alternate layout some builds used.
            local_base / "pichkoo-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Pichkoo")
    else:
        ...
        paths += [
            data_base / "applications" / "pichkoo.desktop",
            data_base / "applications" / "Pichkoo.desktop",
        ]
    return paths
...
def uninstall_gui(pichkoo_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``pichkoo-agent/pichkoo_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$PICHKOO_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove pichkoo' or "
            "'sudo dnf remove pichkoo'). AppImage builds are a single file you "
