"""pichkoo hooks — inspect and manage shell-script hooks.

Usage::

    pichkoo hooks list
    pichkoo hooks test <event> [--for-tool X] [--payload-file F]
    pichkoo hooks revoke <command>
    pichkoo hooks doctor

Consent records live under ``~/.pichkoo/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.pichkoo/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``pichkoo hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: pichkoo hooks {list|test|revoke|doctor}")
        print("Run 'pichkoo hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.pichkoo/config.yaml.")
        print("See `pichkoo hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `pichkoo hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `pichkoo hooks test` and `pichkoo hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Pichkoo wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `pichkoo hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `pichkoo hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `pichkoo hooks doctor`.")
