...
Used by ``pichkoo kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``pichkoo_cli/goals.py`` — same aux
...
PICHKOO_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("PICHKOO_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Pichkoo AI Agent board.
...
def _profile_author() -> str:
    """Mirror of ``pichkoo_cli.kanban._profile_author``. Kept local to
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=PICHKOO_KANBAN_SPECIFY_MAX_TOKENS,
