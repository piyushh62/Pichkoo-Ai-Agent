"""``pichkoo logs`` — view and filter Pichkoo log files.
...
Usage examples::
    ...
    pichkoo logs                    # last 50 lines of agent.log
    pichkoo logs -f                 # follow agent.log in real time
    pichkoo logs errors             # last 50 lines of errors.log
    pichkoo logs gateway -n 100    # last 100 lines of gateway.log
    pichkoo logs gui -f            # follow gui.log (dashboard/pty/ws)
    pichkoo logs desktop -f        # follow desktop.log (Electron app boot/backend)
    pichkoo logs --level WARNING    # only WARNING+ lines
    pichkoo logs --session abc123   # filter by session ID substring
    pichkoo logs --component tools  # only tool-related lines
    pichkoo logs --since 1h         # lines from the last hour
    pichkoo logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_pichkoo_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Pichkoo runs — try 'pichkoo chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_pichkoo_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_pichkoo_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_pichkoo_home()}/logs/")
        return

    print(f"Log files in {display_pichkoo_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'pichkoo chat' to generate logs)")
