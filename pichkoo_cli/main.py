#!/usr/bin/env python3
"""
Pichkoo CLI - Main entry point.

Usage:
    pichkoo                     # Interactive chat (default)
    pichkoo chat                # Interactive chat
    pichkoo gateway             # Run gateway in foreground
    pichkoo gateway start       # Start gateway as service
    pichkoo gateway stop        # Stop gateway service
    pichkoo gateway status      # Show gateway status
    pichkoo gateway install     # Install gateway service
    pichkoo gateway uninstall   # Uninstall gateway service
    pichkoo setup               # Interactive setup wizard
    pichkoo logout              # Clear stored authentication
    pichkoo status              # Show status of all components
    pichkoo cron                # Manage cron jobs
    pichkoo cron list           # List cron jobs
    pichkoo cron status         # Check if cron scheduler is running
    pichkoo doctor              # Check configuration and dependencies
    pichkoo honcho setup                    # Configure Honcho AI memory integration
    pichkoo honcho status                   # Show Honcho config and connection status
    pichkoo honcho sessions                 # List directory → session name mappings
    pichkoo honcho map <name>               # Map current directory to a session name
    pichkoo honcho peer                     # Show peer names and dialectic settings
    pichkoo honcho peer --user NAME         # Set user peer name
    pichkoo honcho peer --ai NAME           # Set AI peer name
    pichkoo honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    pichkoo honcho mode                     # Show current memory mode
    pichkoo honcho mode [hybrid|honcho|local]  # Set memory mode
    pichkoo honcho tokens                   # Show token budget settings
    pichkoo honcho tokens --context N       # Set session.context() token cap
    pichkoo honcho tokens --dialectic N     # Set dialectic result char cap
    pichkoo honcho identity                 # Show AI peer identity representation
    pichkoo honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    pichkoo honcho migrate                  # Step-by-step migration guide: OpenClaw native → Pichkoo + Honcho
    pichkoo version             Show version
    pichkoo update              Update to latest version
    pichkoo uninstall           Uninstall Pichkoo AI Agent
    pichkoo acp                 Run as an ACP server for editor integration
    pichkoo sessions browse     Interactive session picker with search

    pichkoo claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: pichkoo_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``pichkoo_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``pichkoo update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``pichkoo_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, pichkoo crashes on import and the user can't run
# ``pichkoo update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'pichkoo' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``pichkoo tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``pichkoo.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("pichkoo")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"pichkoo", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"pichkoo")
        # Windows: the .exe name is already ``pichkoo.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* pichkoo_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".pichkoo", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``PICHKOO_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("PICHKOO_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``PICHKOO_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("PICHKOO_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`pichkoo --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from pichkoo_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Pichkoo AI Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``pichkoo --version`` before config/logging imports on Termux."""
    if os.environ.get("PICHKOO_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from pichkoo_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from pichkoo_cli.subcommands.cron import build_cron_parser
from pichkoo_cli.subcommands.gateway import build_gateway_parser
from pichkoo_cli.subcommands.profile import build_profile_parser
from pichkoo_cli.subcommands.model import build_model_parser
from pichkoo_cli.subcommands.setup import build_setup_parser
from pichkoo_cli.subcommands.postinstall import build_postinstall_parser
from pichkoo_cli.subcommands.whatsapp import build_whatsapp_parser
from pichkoo_cli.subcommands.slack import build_slack_parser
from pichkoo_cli.subcommands.login import build_login_parser
from pichkoo_cli.subcommands.logout import build_logout_parser
from pichkoo_cli.subcommands.auth import build_auth_parser
from pichkoo_cli.subcommands.status import build_status_parser
from pichkoo_cli.subcommands.webhook import build_webhook_parser
from pichkoo_cli.subcommands.hooks import build_hooks_parser
from pichkoo_cli.subcommands.doctor import build_doctor_parser
from pichkoo_cli.subcommands.security import build_security_parser
from pichkoo_cli.subcommands.dump import build_dump_parser
from pichkoo_cli.subcommands.debug import build_debug_parser
from pichkoo_cli.subcommands.backup import build_backup_parser
from pichkoo_cli.subcommands.import_cmd import build_import_cmd_parser
from pichkoo_cli.subcommands.config import build_config_parser
from pichkoo_cli.subcommands.version import build_version_parser
from pichkoo_cli.subcommands.update import build_update_parser
from pichkoo_cli.subcommands.uninstall import build_uninstall_parser
from pichkoo_cli.subcommands.dashboard import build_dashboard_parser
from pichkoo_cli.subcommands.gui import build_gui_parser
from pichkoo_cli.subcommands.logs import build_logs_parser
from pichkoo_cli.subcommands.prompt_size import build_prompt_size_parser
from pichkoo_cli.subcommands.memory import build_memory_parser
from pichkoo_cli.subcommands.acp import build_acp_parser
from pichkoo_cli.subcommands.tools import build_tools_parser
from pichkoo_cli.subcommands.insights import build_insights_parser
from pichkoo_cli.subcommands.skills import build_skills_parser
from pichkoo_cli.subcommands.pairing import build_pairing_parser
from pichkoo_cli.subcommands.plugins import build_plugins_parser
from pichkoo_cli.subcommands.mcp import build_mcp_parser
from pichkoo_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (pichkoo tools, pichkoo setup, pichkoo model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'pichkoo {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any pichkoo module import.
#
# Many modules cache PICHKOO_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("PICHKOO_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.pichkoo/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set PICHKOO_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors pichkoo_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If PICHKOO_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.pichkoo/profiles/coder or
    # /opt/data/profiles/coder).  If PICHKOO_HOME points to the pichkoo root
    # instead (e.g. systemd hardcodes PICHKOO_HOME=/root/.pichkoo), we must
    ...
    # 3. If we found a profile, resolve and set PICHKOO_HOME
    if profile_name is not None:
        try:
            ...
            # A bug in profiles.py must NEVER prevent pichkoo from starting
            print(
...
try:
    ...
    if _cfg_path.exists():
        ...
        if "PICHKOO_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["PICHKOO_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `pichkoo` subcommands
...
try:
    from pichkoo_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from pichkoo_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if pichkoo_constants not importable yet
...
from pichkoo_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to pichkoo_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (pichkoo_cli.main._model_flow_*) keep resolving unchanged.
from pichkoo_cli.model_setup_flows import (
...
def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("PICHKOO_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("PICHKOO_TERMUX_PREFETCH_UPDATES") == "1"
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo pichkoo {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo pichkoo {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  pichkoo --tui --resume {target}")
    if title:
        print(f'  pichkoo --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @pichkoo/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@pichkoo" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/pichkoo-ink/package.json",
    "packages/pichkoo-ink/index.js",
    "packages/pichkoo-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``PICHKOO_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("PICHKOO_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``PICHKOO_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("PICHKOO_SKIP_NODE_BOOTSTRAP"):
        return
...
def _find_bundled_tui(pichkoo_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if pichkoo_cli_dir is None:
        pichkoo_cli_dir = Path(__file__).parent
    bundled = pichkoo_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (PICHKOO_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("PICHKOO_NODE")
    ...
    ext_dir = os.environ.get("PICHKOO_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with PICHKOO_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset PICHKOO_TUI_DIR (e.g. `unset PICHKOO_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("PICHKOO_QUIET"):
    ...
    if tui_dev:
        # Keep the local @pichkoo/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @pichkoo/ink resolves through
        # packages/pichkoo-ink/dist/entry-exports.js. If that dist bundle is
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from pichkoo_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from pichkoo_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="pichkoo-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["PICHKOO_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["PICHKOO_PYTHON_SRC_ROOT"] = os.environ.get(
        "PICHKOO_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("PICHKOO_PYTHON", sys.executable)
    env.setdefault("PICHKOO_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["PICHKOO_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["PICHKOO_MODEL"] = model
        env["PICHKOO_INFERENCE_MODEL"] = model
    if provider:
        env["PICHKOO_TUI_PROVIDER"] = provider
        env["PICHKOO_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["PICHKOO_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["PICHKOO_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["PICHKOO_TUI_SKILLS"] = value
    if query:
        env["PICHKOO_TUI_QUERY"] = query
    if image:
        env["PICHKOO_TUI_IMAGE"] = image
    if checkpoints:
        env["PICHKOO_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["PICHKOO_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["PICHKOO_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["PICHKOO_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["PICHKOO_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["PICHKOO_ACCEPT_HOOKS"] = "1"
    ...
    # PICHKOO_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `pichkoo --tui` try to
    ...
    # still set PICHKOO_TUI_RESUME themselves.
    env.pop("PICHKOO_TUI_RESUME", None)
    if resume_session_id:
        env["PICHKOO_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `pichkoo update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``PICHKOO_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``pichkoo kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``pichkoo kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("PICHKOO_KANBAN_BOARD"):
        return
    try:
        from pichkoo_cli.kanban_db import get_current_board

        os.environ["PICHKOO_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.pichkoo/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Pichkoo — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Pichkoo still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``PICHKOO_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("PICHKOO_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'pichkoo sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from pichkoo_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from pichkoo_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'pichkoo doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Pichkoo isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  pichkoo setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'pichkoo setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from pichkoo_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["PICHKOO_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.pichkoo/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["PICHKOO_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["PICHKOO_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["PICHKOO_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from pichkoo_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from pichkoo_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Pichkoo?")
        print()
    ...
    # has no creds.json.  Every subsequent `pichkoo gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: pichkoo gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `pichkoo gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  pichkoo gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Pichkoo AI Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  pichkoo gateway")
        ...
        print()
        print("  Or install as a service: pichkoo gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'pichkoo whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from pichkoo_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from pichkoo_cli.config import stamp_install_method
    from pichkoo_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Pichkoo post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``pichkoo model``) and the setup wizard
    ...
    from pichkoo_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("PICHKOO_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'pichkoo model' for "
                "available providers, or run 'pichkoo doctor' to diagnose config "
                "issues."
            )
    ...
    from pichkoo_cli.models import (
        CANONICAL_PROVIDERS,
        _PROVIDER_LABELS,
        group_providers,
    ...
    # PROVIDER_GROUPS in pichkoo_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.pichkoo/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.pichkoo/.env if the active provider is not 'custom'.
    ...
    """
    from pichkoo_cli.config import get_env_value, save_env_value, load_config
...
#
# Pichkoo uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `pichkoo model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `pichkoo model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`pichkoo_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from pichkoo_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Pichkoo only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``pichkoo model`` flow, then route aux tasks to them here.
    """
    from pichkoo_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from pichkoo_cli.auth import _prompt_model_selection
    from pichkoo_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from pichkoo_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from pichkoo_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Pichkoo URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from pichkoo_cli.curses_ui import curses_radiolist
...
# downstream call sites read `pichkoo_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `pichkoo` invocation — including
# fast paths like `pichkoo --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from pichkoo_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``pichkoo setup`` / ``pichkoo model``.
    ...
    recover from a malformed paste without editing ``~/.pichkoo/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from pichkoo_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from pichkoo_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `pichkoo setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_login(args):
    """Authenticate Pichkoo CLI with a provider."""
    from pichkoo_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from pichkoo_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from pichkoo_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from pichkoo_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from pichkoo_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from pichkoo_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``pichkoo slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: pichkoo slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `pichkoo slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from pichkoo_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from pichkoo_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from pichkoo_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from pichkoo_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `pichkoo security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from pichkoo_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from pichkoo_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from pichkoo_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from pichkoo_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Pichkoo home directory to a zip file."""
    if getattr(args, "quick", False):
        from pichkoo_cli.backup import run_quick_backup
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from pichkoo_cli.banner import check_for_updates
        from pichkoo_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Pichkoo AI Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from pichkoo_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from pichkoo_cli.uninstall import run_gui_uninstall
    ...
    from pichkoo_cli.uninstall import run_uninstall
...
# Critical files that every ``pichkoo`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``pichkoo update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "pichkoo_cli/main.py",
    "pichkoo_cli/config.py",
    "pichkoo_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "pichkoo_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``pichkoo`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="pichkoo-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``pichkoo update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``pichkoo_cli/web_dist/`` (per vite.config.ts
    outDir: "../pichkoo_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "pichkoo_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``pichkoo`` launcher present but ``pichkoo_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the pichkoo venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — pichkoo venv python3, no nix-shell overhead
    ...
    # a pichkoo venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``pichkoo update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``pichkoo web``).
    ...
    # build path usable on a stock `py -m pichkoo_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (pichkoo web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``pichkoo update`` can unconditionally call ``pichkoo desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``pichkoo desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $PICHKOO_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``pichkoo update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Pichkoo.app/Contents/MacOS/Pichkoo"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Pichkoo.exe",
            release_dir / "win-ia32-unpacked" / "Pichkoo.exe",
            release_dir / "win-arm64-unpacked" / "Pichkoo.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "pichkoo",
            release_dir / "linux-unpacked" / "Pichkoo",
            release_dir / "linux-arm64-unpacked" / "pichkoo",
            release_dir / "linux-arm64-unpacked" / "Pichkoo",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Pichkoo'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Pichkoo.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Pichkoo.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Pichkoo.exe: Access is
    ...
    an unrelated "Pichkoo" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Pichkoo is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Pichkoo.app/Contents/MacOS/Pichkoo  ->  app bundle = .../Pichkoo.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Pichkoo Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Pichkoo Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from pichkoo_logging import setup_logging as _setup_logging_gui
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  pichkoo gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Pichkoo.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Pichkoo` rename: unpack-electron
                ...
                if purged:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Pichkoo.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Pichkoo.exe, close any")
                    print("  running Pichkoo desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> pichkoo desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Pichkoo is
    ...
    if source_mode:
        print("→ Launching Pichkoo Desktop from source build...")
    ...
    print(f"→ Launching packaged Pichkoo Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``pichkoo dashboard`` processes other than ourselves.

    ``pichkoo dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``pichkoo update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Pichkoo Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``pichkoo dashboard`` as
    ...
    environment variable ``PICHKOO_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "pichkoo dashboard",
        "pichkoo_cli.main dashboard",
        "pichkoo_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "pichkoo.*dashboard"`) keeps us consistent
            # with `pichkoo_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `pichkoo update`.
    ...
    print("  Preview now:  pichkoo curator run --dry-run")
    print("  Pause it:     pichkoo curator pause")
    print(
        "  Docs:         https://pichkoo-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``pichkoo update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``pichkoo update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: pichkoo curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``pichkoo dashboard`` processes.

    Called at the end of ``pichkoo update`` (default ``reason``) and also
    from ``pichkoo dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Pichkoo Desktop Electron app spawns this dashboard as a
    # backend child, it sets PICHKOO_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("PICHKOO_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    pichkoo dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Pichkoo AI Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `pichkoo update "
            f"--branch {branch}`, or update against main with `pichkoo update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="pichkoo-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"pichkoo-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for pichkoo-agent itself
        ...
        # GitHub ZIPs extract to pichkoo-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"pichkoo-agent-{branch}")
    ...
    from pichkoo_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from pichkoo_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "pichkoo-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Pichkoo behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes pichkoo completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Pichkoo couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Pichkoo couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Pichkoo behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Pichkoo couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Pichkoo couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `pichkoo update`
# =========================================================================
...
def _invalidate_update_cache():
    ...
    ``pichkoo update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``pichkoo update`` mutates the
...
# survives and the next ``pichkoo`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $PICHKOO_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``pichkoo update`` left half-done.
    ...
    protocol stream (``pichkoo acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `pichkoo update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from pichkoo_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``pichkoo update`` is still progressing even if pip/uv itself is silent.
    """
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "pichkoo.exe",
        scripts_dir / "pichkoo-gateway.exe",
    ]
...
def _detect_concurrent_hermes_instances(
    ...
    The Pichkoo Desktop Electron app spawns ``pichkoo.EXE`` as a backend child,
    so during ``pichkoo update`` the user-invoked process and the desktop's
    ...
    shims (``pichkoo.exe`` / ``pichkoo-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``pichkoo update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``pichkoo.exe``) is a separate process from the Python
    ...
    # setuptools-generated pichkoo.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``pichkoo update``
    ...
    #      pichkoo.exe sitting *under* a non-Pichkoo parent (e.g. a Pichkoo
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "pichkoo.exe"
    lines = ["✗ Another pichkoo.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Pichkoo Desktop, exit any open `pichkoo` REPLs, and")
    lines.append("  stop the gateway (`pichkoo gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `pichkoo update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    when ``pichkoo update`` runs, ``pichkoo.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``pichkoo.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next pichkoo invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Pichkoo Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Pichkoo Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Pichkoo Desktop, exit other `pichkoo` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `pichkoo update`."
        )
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``pichkoo.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``pichkoo.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``pichkoo`` drops off PATH. This wraps
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``pichkoo.exe.old.*`` left by prior updates.

    Called early on every pichkoo invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``pichkoo update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `pichkoo update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``pichkoo update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``pichkoo update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``pichkoo.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``pichkoo`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `pichkoo update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `pichkoo update --force` after closing other pichkoo processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from pichkoo_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$PICHKOO_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from pichkoo_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `pichkoo update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``pichkoo update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.pichkoo/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``pichkoo update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``pichkoo update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.pichkoo/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``pichkoo update --gateway``) the update is already
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``pichkoo update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``pichkoo update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from pichkoo_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from pichkoo_cli.config import recommended_update_command
        from pichkoo_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``pichkoo update`` without requiring a reinstall.  The
    ...
    ``pichkoo`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/pichkoo.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v pichkoo'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/pichkoo, code at /usr/local/lib/pichkoo-agent).
    fhs_link = Path("/usr/local/bin/pichkoo")
    ...
    path_comment = (
        "# Pichkoo AI Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of PICHKOO_HOME before running the update.
    ...
    PICHKOO_HOME directories.  The ``--backup`` flag on ``pichkoo update``
    ...
    print(f"  Restore:  pichkoo import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``pichkoo update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Pichkoo AI Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Pichkoo AI Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `pichkoo update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Pichkoo via pip (for PyPI installs)."""
    from pichkoo_cli import __version__
    from pichkoo_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "pichkoo-agent"]
        ...
        cmd = [pipx, "upgrade", "pichkoo-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "pichkoo-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "pichkoo-agent"]
    ...
    print("✓ Update complete! Restart pichkoo to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Pichkoo AI Agent...")
    print()

    # On Windows, abort early if another pichkoo.exe is holding the venv shim
    ...
    try:
        ...
        # though `git pull` can't touch $PICHKOO_HOME, this is cheap
        ...
        # orphan merge-conflict markers in pichkoo_cli/config.py bricked
        # every user who ran ``pichkoo update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``pichkoo
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``pichkoo update`` again later once a fix lands.")
                    else:
        ...
        # the marker survives and the next ``pichkoo`` launch finishes the
        ...
        # build.  ``pichkoo desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``pichkoo desktop`` shouldn't be forced into a full
        # Electron build by ``pichkoo update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "pichkoo_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `pichkoo desktop` to retry)")
        ...
        # current — so copy it straight over ~/.pichkoo/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit PICHKOO_HOME so
        # it is not affected by sync_skills()'s module-level PICHKOO_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's PICHKOO_HOME env var points at the default or a named profile.
        try:
            from pichkoo_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'pichkoo config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: pichkoo config migrate")
            else:
                print()
                print("Skipped. Run 'pichkoo config migrate' later to configure.")
        else:
        ...
        # check `pichkoo curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``pichkoo update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from pichkoo_cli.tools_config import install_cua_driver
        ...
        # When running as ``pichkoo update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            # Discover all pichkoo-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. pichkoo-gateway.service or pichkoo-gateway-coder.service
                            ...
                            # path in `pichkoo gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: pichkoo gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: pichkoo -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Pichkoo gateway unit files are still installed.
        # When both pichkoo.service (from a pre-rename install) and the
        # current pichkoo-gateway.service are enabled, they SIGTERM-fight
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("  These pre-rename units (pichkoo.service) fight the current")
                print("  pichkoo-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    pichkoo gateway migrate-legacy")
                print()
        ...
        print("  pichkoo model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``pichkoo -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → pichkoo -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.pichkoo)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  pichkoo profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     pichkoo -p {name} chat")
                else:
        ...
        from pichkoo_cli import profiles as _profiles_mod
        ...
        from pichkoo_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `pichkoo profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → pichkoo -p {name}  ({wrapper})")
        print()
        ...
        from pichkoo_cli.profiles import profile_exists
        ...
        from pichkoo_cli.profiles import rename_profile
        ...
        from pichkoo_cli.profiles import export_profile
        ...
        from pichkoo_cli.profiles import import_profile
        ...
        import tempfile
        from pichkoo_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  pichkoo -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      pichkoo -p {plan.manifest.name} chat")
        ...
        from pichkoo_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `pichkoo profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"pichkoo -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from pichkoo_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from pichkoo_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _report_dashboard_status() -> int:
    """Print ``pichkoo dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``pichkoo dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No pichkoo dashboard processes running.")
        return 0

    print(f"{len(pids)} pichkoo dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No pichkoo dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `pichkoo update`.
    ...
    # the same logs directory as every other Pichkoo surface.
    try:
        from pichkoo_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "PICHKOO_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["PICHKOO_WEB_DIST"])
            if "PICHKOO_WEB_DIST" in os.environ
            else PROJECT_ROOT / "pichkoo_cli" / "web_dist"
        )
    ...
    from pichkoo_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from pichkoo_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_completion(args, parser=None):
    ...
    from pichkoo_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from pichkoo_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Pichkoo log files."""
    from pichkoo_cli.logs import tail_log, list_logs
...
# so that in ``pichkoo -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``pichkoo_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``pichkoo -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``pichkoo`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("PICHKOO_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from pichkoo_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from pichkoo_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["PICHKOO_DEFER_AGENT_STARTUP"] = "1"
            os.environ["PICHKOO_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["PICHKOO_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `pichkoo --tui` is the hot path on phones. The full parser setup imports
...
def cmd_acp(args):
    """Launch Pichkoo AI Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from pichkoo_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from pichkoo_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from pichkoo_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from pichkoo_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from pichkoo_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from pichkoo_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from pichkoo_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from pichkoo_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from pichkoo_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for pichkoo CLI."""
    # Cosmetic: make the process show up as 'pichkoo' instead of 'python3.11'
    ...
    # Self-heal a venv left half-built by an interrupted ``pichkoo update``
    ...
    # ``pichkoo skills install update``) merely defers recovery one launch;
    # under-matching (missing ``pichkoo -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in pichkoo_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from pichkoo_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://pichkoo-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `pichkoo model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.pichkoo/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://pichkoo-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from pichkoo_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from pichkoo_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in pichkoo_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in pichkoo_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in pichkoo_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in pichkoo_cli/subcommands/whatsapp.py)
    # =========================================================================
    build_whatsapp_parser(subparsers, cmd_whatsapp=cmd_whatsapp)

    # =========================================================================
    # slack command  (parser built in pichkoo_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from pichkoo_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in pichkoo_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in pichkoo_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in pichkoo_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in pichkoo_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in pichkoo_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in pichkoo_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from pichkoo_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from pichkoo_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in pichkoo_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in pichkoo_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in pichkoo_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in pichkoo_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in pichkoo_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in pichkoo_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.pichkoo/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo pichkoo uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from pichkoo_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in pichkoo_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in pichkoo_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in pichkoo_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in pichkoo_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from pichkoo_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in pichkoo_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``pichkoo --help``, ``pichkoo version``, ``pichkoo logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from pichkoo_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from pichkoo_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in pichkoo_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in pichkoo_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `pichkoo computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `pichkoo tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: pichkoo computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: pichkoo computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in pichkoo_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch pichkoo --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in pichkoo_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in pichkoo_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in pichkoo_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in pichkoo_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in pichkoo_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in pichkoo_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in pichkoo_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in pichkoo_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Pichkoo-Setup.exe success screen tells users to
    # run `pichkoo desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in pichkoo_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in pichkoo_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in pichkoo_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``pichkoo -c Pokemon Agent Dev`` → ``pichkoo -c 'Pokemon Agent Dev'``
    ...
    from pichkoo_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'pichkoo -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (pichkoo hooks list, cron
