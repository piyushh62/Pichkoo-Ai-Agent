"""
MCP Server Management CLI — ``pichkoo mcp`` subcommand.

Implements ``pichkoo mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.pichkoo/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from pichkoo_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.pichkoo/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``pichkoo mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import PichkooTokenStorage
        return PichkooTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # pichkoo_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  pichkoo mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  pichkoo mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  pichkoo mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: pichkoo mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `pichkoo mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  pichkoo mcp add <name> --url <endpoint>')
        _info('  pichkoo mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `pichkoo mcp remove` + `pichkoo mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `pichkoo mcp login " + name + "`.")
            return
        if tools:
...
# ─── pichkoo mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'pichkoo mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``pichkoo mcp`` subcommands."""
    ...
    if action == "picker":
        from pichkoo_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from pichkoo_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from pichkoo_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `pichkoo plugin`.
        from pichkoo_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("pichkoo mcp                                    Open the catalog picker (default)")
        _info("pichkoo mcp catalog                            List Nous-approved MCPs")
        _info("pichkoo mcp install <name>                     Install a catalog MCP")
        _info("pichkoo mcp serve                              Run as MCP server")
        _info("pichkoo mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("pichkoo mcp add <name> --command <cmd>         Add a stdio server")
        _info("pichkoo mcp add <name> --preset <preset>       Add from a known preset")
        _info("pichkoo mcp remove <name>                      Remove a server")
        _info("pichkoo mcp list                               List configured servers")
        _info("pichkoo mcp test <name>                        Test connection")
        _info("pichkoo mcp configure <name>                   Toggle tools")
        _info("pichkoo mcp login <name>                       Re-authenticate OAuth")
        print()
