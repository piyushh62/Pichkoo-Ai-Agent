"""MCP picker — interactive `pichkoo mcp picker` (also the default `pichkoo mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``pichkoo mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `pichkoo plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from pichkoo_cli.cli_output import prompt_yes_no
from pichkoo_cli.curses_ui import curses_single_select
from pichkoo_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Pichkoo session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from pichkoo_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `pichkoo mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: pichkoo mcp install <name>    Picker: pichkoo mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Pichkoo is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Pichkoo — run `pichkoo update` "
...
def show_catalog() -> None:
    """`pichkoo mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`pichkoo mcp picker` (and default `pichkoo mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`pichkoo mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `pichkoo mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
