...
- ``pichkoo_cli.providers``        -- canonical provider identity + overlays
- ``pichkoo_cli.model_normalize``  -- per-provider name formatting
...
_PICHKOO_MODEL_WARNING = (
    "Nous Research Pichkoo 3 & 4 models are NOT agentic and are not designed "
    "for use with Pichkoo AI Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Pichkoo 3 / Pichkoo 4 chat families.
# The previous substring check (`"pichkoo" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``pichkoo-brain:qwen3-14b-ctx16k`` that just
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``pichkoo config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from pichkoo_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'pichkoo model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'pichkoo doctor' — config issues detected:"
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from pichkoo_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in pichkoo_cli.runtime_provider.resolve_runtime_provider;
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from pichkoo_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from pichkoo_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from pichkoo_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by pichkoo provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://pichkoo-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Pichkoo release. Falls back to the in-repo
    ...
    # --- 2. Check Pichkoo-only providers (nous, openai-codex, copilot, opencode-go) ---
    from pichkoo_cli.providers import PICHKOO_OVERLAYS
    from pichkoo_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in PICHKOO_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from pichkoo_cli.auth import get_provider_auth_state as _nous_state
    ...
    # in PROVIDER_TO_MODELS_DEV or PICHKOO_OVERLAYS (keeps /model in sync
    # with `pichkoo model`).
    try:
        from pichkoo_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Pichkoo's canonical write key (matches
            ...
            # Pichkoo writes ``models:`` as a dict keyed by model id
            # (see pichkoo_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Pichkoo's own writer uses; a hyphen variant is accepted
            ...
            # active model. Pichkoo's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, pichkoo_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`pichkoo_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from pichkoo_cli.models import fetch_openrouter_models
