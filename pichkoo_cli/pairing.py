...
Usage:
    pichkoo pairing list              # Show all pending + approved users
    pichkoo pairing approve <platform> <code>  # Approve a pairing code
    pichkoo pairing revoke <platform> <user_id> # Revoke user access
    pichkoo pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle pichkoo pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: pichkoo pairing {list|approve|revoke|clear-pending}")
        print("Run 'pichkoo pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.pichkoo/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'pichkoo pairing list' to see pending codes.\n")
