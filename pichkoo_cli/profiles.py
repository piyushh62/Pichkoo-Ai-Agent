...
The "default" profile is ``~/.pichkoo`` itself — backward compatible,
zero migration needed.

Usage::

    pichkoo profile create coder          # fresh profile + bundled skills
    pichkoo profile create coder --clone  # also copy config, .env, SOUL.md, skills
    pichkoo profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    pichkoo -p coder chat                 # or via flag
    pichkoo profile use coder             # set as sticky default
    pichkoo profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See pichkoo_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.pichkoo``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   pichkoo-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `pichkoo profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `pichkoo
...
# `pichkoo skills install` or drop SKILL.md files into the profile's skills/.
...
# Directories/files to exclude when exporting the default (~/.pichkoo) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "pichkoo-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, pichkoo subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``pichkoo -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``pichkoo profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``pichkoo profile list`` for an unrelated profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``pichkoo update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.pichkoo)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.pichkoo/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `pichkoo update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`pichkoo profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `pichkoo profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `pichkoo -p <profile> gateway start` can supervise it via
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.pichkoo).\n"
            "To remove everything, use: pichkoo uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from pichkoo_cli.service_manager import get_service_manager
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.pichkoo/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: pichkoo profile create {canon}"
        )
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.pichkoo itself — its parent is ~/ and its
        # directory name is ".pichkoo", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: pichkoo profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.pichkoo itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.pichkoo). "
            "Specify a different name: pichkoo profile import <archive> --name <name>"
        )
