...
Reads the user's Nous OAuth state from ``~/.pichkoo/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `pichkoo auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `pichkoo auth add nous` to re-authenticate."
                )
    ...
    # to pichkoo_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
