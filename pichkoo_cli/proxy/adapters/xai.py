...
# xAI's public API is OpenAI-compatible for the endpoints Pichkoo commonly
# uses. The Responses endpoint is included because Pichkoo' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Pichkoo-managed OAuth credentials."""

    auth_hint = "pichkoo auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`pichkoo auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`pichkoo auth reset xai-oauth` or re-authenticate with "
                    "`pichkoo auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `pichkoo auth add xai-oauth --type oauth`."
            )
