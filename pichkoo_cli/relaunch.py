"""
Unified self-relaunch for Pichkoo CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``pichkoo sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``pichkoo`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from pichkoo_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched pichkoo."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_pichkoo_bin() -> Optional[str]:
    ...
    Priority:
      ...
      2. ``shutil.which("pichkoo")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m pichkoo_cli.main``.
    ...
    ``pichkoo.exe`` on PATH or the ``sys.executable -m pichkoo_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with pichkoo.
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "pichkoo_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh pichkoo invocation.
    ...
    relaunch contract wants: "run pichkoo again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``pichkoo.exe`` (a Python console-script shim that wraps
    ``python -m pichkoo_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "pichkoo exited, then
    new pichkoo started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``pichkoo`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nPichkoo relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run pichkoo.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
