...
the existing module-level functions in pichkoo_cli.gateway and
pichkoo_cli.gateway_windows directly. This protocol is a thin facade
...
``pichkoo gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``pichkoo gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged pichkoo
...
# in ``pichkoo_cli.gateway`` (systemd/launchd) and ``pichkoo_cli.gateway_windows``
...
# active (set via the ``pichkoo -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in pichkoo_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from pichkoo_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from pichkoo_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from pichkoo_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from pichkoo_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in pichkoo_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from pichkoo_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from pichkoo_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from pichkoo_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from pichkoo_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``pichkoo_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from pichkoo_cli import gateway_windows
        from pichkoo_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `pichkoo profile create`
# runs inside the container (Phase 4). Static services (main-pichkoo, dashboard)
...
# UID/GID of the in-image ``pichkoo`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the pichkoo user.
    ...
    is dispatched as the pichkoo user.
    ...
    with pichkoo ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           pichkoo:pichkoo, 0755 (parent must already exist)
    ``svc_dir/event/``                     pichkoo:pichkoo, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 pichkoo:pichkoo, 0755
    ``svc_dir/supervise/event/``           pichkoo:pichkoo, 03730
    ``svc_dir/supervise/control``          pichkoo:pichkoo, 0660    (FIFO)
    ...
    slot's supervise/ was pichkoo-owned.
    ...
    # when the parent slot's supervise/ is pichkoo-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``pichkoo -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`pichkoo profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-pichkoo, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          ...
          4. Drops to the pichkoo user and exec's
             ``pichkoo -p <profile> gateway run`` (or just ``pichkoo
        ...
        ``pichkoo_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        if profile == "default":
            ...
            gateway_cmd = f"pichkoo -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid pichkoo {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``pichkoo logs`` reads and what
        ...
        return (
            ...
            f'exec s6-setuidgid pichkoo s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``pichkoo gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with pichkoo ownership
            ...
            # the pichkoo user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is pichkoo-owned (see ``_seed_supervise_skeleton``), and on
