"""
Interactive setup wizard for Pichkoo AI Agent.
...
_DOCS_BASE = "https://pichkoo-agent.nousresearch.com/docs"
...
from pichkoo_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from pichkoo_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Pichkoo Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Pichkoo using environment variables or config commands:")
    print_info("  pichkoo config set model.provider custom")
    print_info("  pichkoo config set model.base_url http://localhost:8080/v1")
    print_info("  pichkoo config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'pichkoo setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from pichkoo_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'pichkoo setup' to configure"))
    ...
    # Video generation — opt-in via `pichkoo tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from pichkoo_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'pichkoo setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'pichkoo setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'pichkoo setup terminal'"))
    ...
    # Spotify (OAuth via pichkoo auth spotify — check auth.json, not env vars)
    try:
    ...
    print()
    print(f"   {color('pichkoo setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('pichkoo setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('pichkoo setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('pichkoo setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('pichkoo setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('pichkoo config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('pichkoo config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('pichkoo config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('pichkoo', Colors.GREEN)}              Start chatting")
    print(f"   {color('pichkoo gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('pichkoo doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `pichkoo tools` and `pichkoo setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``pichkoo model``)
    ...
    provider added to ``pichkoo model`` is automatically available here.
    ...
    # Delegate to the shared pichkoo model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from pichkoo_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: pichkoo model")
    ...
    # on demand via `pichkoo auth add`, `pichkoo setup` vision, and
    # `pichkoo setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``pichkoo model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from pichkoo_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.pichkoo/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Pichkoo auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'pichkoo setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Pichkoo runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `pichkoo setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `pichkoo setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into PICHKOO_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("PICHKOO_MAX_ITERATIONS")
    ...
    print_info("  Run `pichkoo setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # PICHKOO_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from pichkoo_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Pichkoo delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `pichkoo update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://pichkoo-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under PICHKOO_HOME, and print
    ...
    try:
        ...
        print_info(
            "   Re-run `pichkoo slack manifest --write` anytime to refresh after "
            "Pichkoo adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Pichkoo delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Pichkoo to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from pichkoo_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://pichkoo-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://pichkoo-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  pichkoo config edit")
    print_info("   Open config in your editor:  pichkoo config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from pichkoo_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Pichkoo from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'pichkoo setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     pichkoo config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo pichkoo gateway install --system")
                print_info("  Or run in foreground:  pichkoo gateway")
        else:
            from pichkoo_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   pichkoo gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   pichkoo gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `pichkoo setup tools` and `pichkoo tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``pichkoo_cli.auth`` — lists every supported
    ...
    """
    try:
        from pichkoo_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from pichkoo_cli.config
    ...
    if section_key == "model":
        ...
        from pichkoo_cli.gateway import _all_platforms, _platform_status
...
# Config values may have different semantics between OpenClaw and Pichkoo.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Pichkoo to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Pichkoo at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Pichkoo at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Pichkoo at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Pichkoo at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Pichkoo equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Pichkoo config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Pichkoo.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Pichkoo's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Pichkoo can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: pichkoo claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: pichkoo claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Pichkoo configs are
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Pichkoo (use pichkoo claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``pichkoo setup --portal`` and ``pichkoo portal``. This is the
    ...
    Pichkoo session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``pichkoo setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``pichkoo model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``pichkoo portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Pichkoo Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `pichkoo portal` == quick setup's Nous step.
    try:
        from pichkoo_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `pichkoo portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `pichkoo portal`: %s", exc)
        print()
    ...
    print_info("  Run `pichkoo portal info` to inspect routing.")
    print_info("  Run `pichkoo` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      pichkoo setup           — full or quick (auto-detected)
      pichkoo setup model     — just model/provider
      pichkoo setup tts       — just text-to-speech
      pichkoo setup terminal  — just terminal backend
      pichkoo setup gateway   — just messaging platforms
      pichkoo setup tools     — just tool configuration
      pichkoo setup agent     — just agent settings
    """
    from pichkoo_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Pichkoo Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Pichkoo AI Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Pichkoo AI Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Pichkoo configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'pichkoo setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Pichkoo?",
            [
    ...
    # Tune later with `pichkoo setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``pichkoo setup <section>``
    or switch providers with ``pichkoo model``.
    """
    from pichkoo_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'pichkoo setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    pichkoo setup")
    if gateway_choice != 0:
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'pichkoo setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Pichkoo to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'pichkoo setup gateway'.")
