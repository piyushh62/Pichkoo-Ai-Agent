"""
Skills configuration for Pichkoo AI Agent.
`pichkoo skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.pichkoo/config.yaml under:
...
from pichkoo_cli.config import cfg_get, load_config, save_config
from pichkoo_cli.colors import Colors, color
from pichkoo_cli.platforms import PLATFORMS as _PLATFORMS
...
def skills_command(args=None):
    """Entry point for `pichkoo skills`."""
    from pichkoo_cli.curses_ui import curses_checklist
