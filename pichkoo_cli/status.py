"""
Status command for pichkoo CLI.

Shows the status of all Pichkoo AI Agent components.
"""
...
from pichkoo_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``pichkoo config``'s
...
def show_status(args):
    """Show status of all Pichkoo AI Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Pichkoo AI Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: pichkoo portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: pichkoo model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: pichkoo auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: pichkoo auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: pichkoo model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from pichkoo_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from pichkoo_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   pichkoo gateway")
    ...
    print(color("  Run 'pichkoo doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'pichkoo setup' to configure", Colors.DIM))
    print()
