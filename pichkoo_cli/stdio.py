...
on Japanese locales, etc.).  Pichkoo's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``pichkoo_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Pichkoo is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``PICHKOO_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("PICHKOO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Pichkoo-managed Git install directories so
    ...
    # already-running shells don't see the change — which means pichkoo
    ...
    # install without Pichkoo-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Pichkoo's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Pichkoo's
    ``pichkoo config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Pichkoo (User env var in Windows Settings, or
    export in a PowerShell profile) and Pichkoo picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Pichkoo-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\pichkoo\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``pichkoo`` from right after install) retain their old PATH.

    Any subprocess Pichkoo spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Pichkoo-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "pichkoo", "git", "cmd"),
        os.path.join(local_appdata, "pichkoo", "git", "bin"),
        os.path.join(local_appdata, "pichkoo", "git", "usr", "bin"),
        # Pichkoo venv Scripts directory — host of the pichkoo.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes pichkoo, but harmless to include.
        os.path.join(local_appdata, "pichkoo", "pichkoo-agent", "venv", "Scripts"),
