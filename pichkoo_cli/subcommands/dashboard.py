"""``pichkoo dashboard`` subcommand parser.

Extracted verbatim from ``pichkoo_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Pichkoo AI Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `pichkoo dashboard` cmdlines and SIGTERM them directly — the same
    # path `pichkoo update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running pichkoo dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running pichkoo dashboard processes and exit",
    )
    # Backward-compat shim: older Pichkoo desktop app shells (<= 0.15.x) spawn the
    # backend as `pichkoo dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Pichkoo couldn't start" with no actionable cause. Accept and silently
    ...
    # `pichkoo dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.pichkoo/.env.
    # Nested subparser so bare `pichkoo dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "PICHKOO_DASHBOARD_OAUTH_CLIENT_ID into ~/.pichkoo/.env, and prints "
            "how to engage the login gate. Requires being logged in (pichkoo setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://pichkoo.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via PICHKOO_DASHBOARD_PORTAL_URL. Mainly for "
