"""``pichkoo setup`` subcommand parser.
...
def build_setup_parser(subparsers, *, cmd_setup: Callable) -> None:
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Pichkoo AI Agent with an interactive wizard. "
        "Run a specific section: pichkoo setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'pichkoo setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
