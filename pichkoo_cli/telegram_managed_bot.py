...
manual BotFather token copy-paste. Pichkoo talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.pichkoo-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "PichkooSetupBot"

DEFAULT_BOT_NAME = "Pichkoo AI Agent"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Pichkoo Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Pichkoo Telegram onboarding service.")
