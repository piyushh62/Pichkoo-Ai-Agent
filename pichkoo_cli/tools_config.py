"""
Unified tool configuration for Pichkoo AI Agent.

`pichkoo tools` and `pichkoo setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.pichkoo/config.yaml under
...
from pichkoo_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from pichkoo_cli.nous_account import format_nous_portal_entitlement_message
...
from pichkoo_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _PICHKOO_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `pichkoo tools` → Video Generation, which walks
...
# `pichkoo tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from pichkoo_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `pichkoo tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``pichkoo tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from pichkoo_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``pichkoo tools`` checklist actually offers
    for ``platform``.
    ...
    so ``pichkoo tools`` never claims to add or remove a toolset the user was
...
from pichkoo_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Pichkoo routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. PICHKOO_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "PICHKOO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "PICHKOO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "PICHKOO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "PICHKOO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "PICHKOO_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("PICHKOO_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``pichkoo update`` and
      by ``pichkoo computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `pichkoo update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Pichkoo process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in pichkoo-agent directory)")
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.pichkoo/config.yaml")
        ...
        # Run the full `pichkoo auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.pichkoo/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from pichkoo_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: pichkoo auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `pichkoo auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: pichkoo auth spotify")
        except Exception as exc:
        ...
        try:
            from pichkoo_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: pichkoo plugins enable observability/langfuse")
        _print_info("    Restart Pichkoo for tracing to take effect.")
        _print_info("    Verify: pichkoo plugins list")
        ...
        try:
            ...
            _print_info("    Run later: pichkoo auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `pichkoo auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: pichkoo auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: pichkoo auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``pichkoo tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``pichkoo tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: pichkoo tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    # "pichkoo-cli" (which include all _PICHKOO_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[pichkoo-cli, spotify]`` after enabling Spotify via ``pichkoo
        ...
        # (e.g. "pichkoo-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``pichkoo tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `pichkoo tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"pichkoo-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("pichkoo-")}
    ...
    # shipped as a bundled plugin but user must opt in via `pichkoo tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `pichkoo tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by pichkoo tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (pichkoo-cli, pichkoo-telegram, etc.)
    ...
    # Opening `pichkoo tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `pichkoo tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `pichkoo model`).
    if managed_feature:
        from pichkoo_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `pichkoo tools` and `pichkoo setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Pichkoo Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://pichkoo-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, pichkoo mcp
