"""
Pichkoo AI Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.pichkoo/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Pichkoo PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing pichkoo-agent or pichkoo PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Pichkoo AI Agent" comment and following line
                if '# Pichkoo AI Agent' in line or '# pichkoo-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('pichkoo' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing pichkoo
                if 'pichkoo' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the pichkoo wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "pichkoo",
        Path("/usr/local/bin/pichkoo"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains pichkoo_cli reference)
                content = wrapper.read_text()
                if 'pichkoo_cli' in content or 'pichkoo-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(pichkoo_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``pichkoo`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Pichkoo home's ``node``
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``pichkoo gateway run`` processes
    ...
    try:
        from pichkoo_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from pichkoo_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``PICHKOO_HOME`` and ``PICHKOO_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\pichkoo\git\cmd``, ``%LOCALAPPDATA%\pichkoo\git\bin``,
#      ``%LOCALAPPDATA%\pichkoo\git\usr\bin``, ``%LOCALAPPDATA%\pichkoo\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\pichkoo\git\`` and Node to
#      ``%LOCALAPPDATA%\pichkoo\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``pichkoo dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\pichkoo\gateway-service\`` and sometimes
...
def remove_pichkoo_env_vars_windows() -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("PICHKOO_HOME", "PICHKOO_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its PICHKOO_HOME directory.

    We shell out to ``pichkoo -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current PICHKOO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    # 3. Wipe the profile's PICHKOO_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``pichkoo uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$PICHKOO_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from pichkoo_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    print(color("│         ⚕ Pichkoo Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Pichkoo agent is still installed. Run 'pichkoo' to use the CLI,")
    print("or 'pichkoo uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.pichkoo/ (configs, data, logs)
    ...
    # full wipe (code + ~/.pichkoo data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Pichkoo AI Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their PICHKOO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Pichkoo data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Pichkoo code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``pichkoo`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$PICHKOO_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        if removed_env:
            ...
            log_info("No Pichkoo-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing pichkoo command...")
    ...
    if removed_node_links:
        ...
        log_info("No Pichkoo-managed node/npm/npx symlinks found")
    ...
    # 5. Optionally remove ~/.pichkoo/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile PICHKOO_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://pichkoo-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://pichkoo-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Pichkoo AI Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m pichkoo_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import pichkoo_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``pichkoo_constants`` + ``pichkoo_cli.colors``
    (and lazily ``pichkoo_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m pichkoo_cli.uninstall")
    parser.add_argument(
        "--mode",
