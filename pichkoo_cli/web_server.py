"""
Pichkoo AI Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m pichkoo_cli.main web          # Start on http://127.0.0.1:9119
    python -m pichkoo_cli.main web --port 8080
"""
...
try:
    ...
    # running `pichkoo dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["PICHKOO_WEB_DIST"]) if "PICHKOO_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``pichkoo gateway run`` — but the
    desktop app spawns a ``pichkoo dashboard`` backend, not a gateway, so a cron
    ...
    PICHKOO_HOME — whichever process grabs the lock first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (PICHKOO_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `pichkoo
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("PICHKOO_DESKTOP") == "1":
...
app = FastAPI(title="Pichkoo AI Agent", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# PICHKOO_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("PICHKOO_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Pichkoo-Session-Token"
...
#
# This list is defined in ``pichkoo_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from pichkoo_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Pichkoo-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from pichkoo_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Pichkoo (no terminal prompt), "
...
async def get_status():
    ...
    # and which providers are registered so ``pichkoo status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from pichkoo_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the pichkoo home root).
    """
...
async def get_portal_status():
    ...
    features = []
    try:
        from pichkoo_cli.nous_subscription import get_nous_subscription_features
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from pichkoo_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.pichkoo/logs/<action>.log`` so
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "pichkoo-update": "pichkoo-update.log",
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    cmd = [sys.executable, "-m", "pichkoo_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "PICHKOO_NONINTERACTIVE": "1"},
    }
...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    """Spawn ``pichkoo gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``pichkoo gateway restart`` children race each other on the
...
async def restart_gateway():
    """Kick off a ``pichkoo gateway restart`` in the background."""
    try:
...
async def update_hermes():
    ...
    if install_method == "docker":
        ...
        _record_completed_action("pichkoo-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "pichkoo-update",
...
async def check_hermes_update(force: bool = False):
    ...
    ``POST /api/pichkoo/update`` actually runs ``pichkoo update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Pichkoo version string
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.pichkoo/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Pichkoo surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Pichkoo supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in pichkoo_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options():
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `pichkoo model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `pichkoo model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from pichkoo_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.pichkoo/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            # post-model-selection behaviour (pichkoo_cli/main.py
            ...
            if provider.strip().lower() == "nous":
                try:
                    from pichkoo_cli.nous_subscription import apply_nous_managed_defaults
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Pichkoo from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Pichkoo to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Pichkoo from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Pichkoo to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Pichkoo in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Pichkoo through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Pichkoo via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Pichkoo through an IMAP/SMTP mailbox.",
        "docs_url": "https://pichkoo-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Pichkoo to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Pichkoo inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Pichkoo through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Pichkoo to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Pichkoo to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Pichkoo as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://pichkoo-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://pichkoo-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.pichkoo-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"PichkooDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Pichkoo AI Agent").strip() or "Pichkoo AI Agent"
...
def _restart_gateway_after_telegram_onboarding() -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Pichkoo appears
...
# the canonical ``pichkoo auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Pichkoo resolves Anthropic creds in this order at runtime:
    1. ``~/.pichkoo/.anthropic_oauth.json`` — Pichkoo-managed PKCE flow
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Pichkoo even
    when they also have a separate Pichkoo-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "pichkoo auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "pichkoo auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "pichkoo auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "pichkoo auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "pichkoo auth add xai-oauth",
        "docs_url": "https://pichkoo-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "pichkoo auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from pichkoo_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Pichkoo-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _PICHKOO_OAUTH_FILE
            if _PICHKOO_OAUTH_FILE.exists():
                _PICHKOO_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from pichkoo_cli.auth import clear_provider_auth
...
#          → persists to ~/.pichkoo/.anthropic_oauth.json AND credential pool
...
# Guarded so pichkoo web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Pichkoo file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``pichkoo auth add anthropic``.
    """
    from agent.anthropic_adapter import _PICHKOO_OAUTH_FILE
    payload = {
    ...
    _PICHKOO_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _PICHKOO_OAUTH_FILE.with_name(
        f"{_PICHKOO_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _PICHKOO_OAUTH_FILE)
        try:
            _PICHKOO_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "pichkoo-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("PICHKOO_PORTAL_BASE_URL")
...
# the tokens exactly like `pichkoo auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("PICHKOO_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `pichkoo auth add xai-oauth`'s credential-pool insert.
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``pichkoo auth add minimax-oauth``.
    """
    from pichkoo_cli.auth import (
        _minimax_poll_token,
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from pichkoo_cli.auth import _save_codex_tokens
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `pichkoo sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `pichkoo sessions prune`)."""
...
async def get_logs(
    ...
    from pichkoo_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from pichkoo_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, PICHKOO_HOME)."""
    from pichkoo_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process PICHKOO_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (pichkoo_cli.mcp_config), so
# servers managed here show up under `pichkoo mcp list` and vice versa.  Secrets
...
async def list_mcp_servers():
    from pichkoo_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    from pichkoo_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str):
    from pichkoo_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from pichkoo_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    `pichkoo mcp catalog` / `pichkoo mcp install` read.
    """
    try:
        from pichkoo_cli import mcp_catalog
    except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (pichkoo_cli.webhook); the webhook
...
async def list_webhooks():
    import pichkoo_cli.webhook as wh
...
# spawn the real `pichkoo gateway <verb>` so behaviour matches the CLI exactly.
...
async def list_credential_pool():
    ...
    from pichkoo_cli.auth import read_credential_pool
...
# `pichkoo memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `pichkoo memory setup` to configure a new one.",
            )
...
async def list_hooks():
    ...
    form can offer them.
    """
    from pichkoo_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from pichkoo_cli.plugins import VALID_HOOKS
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from pichkoo_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
def _profile_setup_command(name: str) -> str:
    ...
    return "pichkoo setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local PICHKOO_HOME override so the write lands in the target
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``pichkoo profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from PICHKOO_HOME).
    """
    from pichkoo_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``pichkoo profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the PICHKOO_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``pichkoo profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from pichkoo_cli import profile_describer
...
async def get_skills():
    ...
    from pichkoo_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from pichkoo_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from pichkoo_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``pichkoo tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``pichkoo tools`` picker shows
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``pichkoo tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate):
    ...
    Writes each ``key: value`` to ``~/.pichkoo/.env`` via ``save_env_value`` —
    the same store ``pichkoo tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(name: str, body: ToolsetPostSetup):
    ...
    ``pichkoo tools post-setup <key>`` and the frontend tails the log via
...
#
# The endpoint spawns the same ``pichkoo --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from pichkoo_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from pichkoo_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from pichkoo_cli.dashboard_auth.audit import AuditEvent, audit_log
        from pichkoo_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``pichkoo --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``PICHKOO_TUI_RESUME`` env var —
    matching what ``pichkoo_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``PICHKOO_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``PICHKOO_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from pichkoo_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("PICHKOO_TUI_DISABLE_MOUSE", "1")
    env.setdefault("PICHKOO_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["PICHKOO_TUI_RESUME"] = resume

    if sidecar_url:
        env["PICHKOO_TUI_SIDECAR_URL"] = sidecar_url

    if gateway_ws_url := _build_gateway_ws_url():
        env["PICHKOO_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from pichkoo_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Pichkoo inside WSL2 to use the dashboard's /chat "
...
# PICHKOO_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`pichkoo_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from pichkoo_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/pichkoo/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /pichkoo`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__PICHKOO_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/pichkoo``)
        ...
        ``__PICHKOO_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__PICHKOO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__PICHKOO_BASE_PATH__="{prefix}";'
                f"window.__PICHKOO_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__PICHKOO_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__PICHKOO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__PICHKOO_BASE_PATH__="{prefix}";'
                f"window.__PICHKOO_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/pichkoo`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Pichkoo backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Pichkoo Teal",         "description": "Classic dark teal — the canonical Pichkoo look"},
    {"name": "default-large", "label": "Pichkoo Teal (Large)", "description": "Pichkoo Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.pichkoo/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.pichkoo/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.pichkoo/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as pichkoo_cli.plugins):
    1. User plugins:    ~/.pichkoo/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.pichkoo/plugins/  (only if PICHKOO_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    # ``pichkoo_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("PICHKOO_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".pichkoo" / "plugins", "project"))
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"pichkoo auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from pichkoo_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from pichkoo_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from pichkoo_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from pichkoo_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.pichkoo/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.pichkoo/plugins/ if you trust it)",
...
from pichkoo_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from pichkoo_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing PICHKOO_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Pichkoo AI Agent exposes a
    ...
    print(f"  Pichkoo Web UI → http://{host}:{port}")
