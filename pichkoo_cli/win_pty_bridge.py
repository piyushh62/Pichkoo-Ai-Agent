"""Windows ConPTY bridge for the `pichkoo dashboard` chat tab.

Drop-in counterpart to ``pichkoo_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``pichkoo_cli.web_server`` consumes: ``spawn``, ``read``,
