...
Pure logic: walks a Pichkoo config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `pichkoo doctor` and a future `pichkoo migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Pichkoo config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Pichkoo config and return retirement issues.
