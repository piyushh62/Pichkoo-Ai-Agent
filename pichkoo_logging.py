"""Centralized logging setup for Pichkoo AI Agent.
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``pichkoo_cli.web_server``,
    ``pichkoo_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
COMPONENT_PREFIXES = {
    ...
    "cron": ("cron",),
    "gui": (
        "pichkoo_cli.web_server",
        "pichkoo_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Pichkoo logging subsystem.
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the pichkoo group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from pichkoo_cli.config import is_managed
