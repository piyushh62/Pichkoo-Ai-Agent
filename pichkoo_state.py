#!/usr/bin/env python3
"""
SQLite State Store for Pichkoo AI Agent.
...
# pichkoo_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``pichkoo_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see pichkoo_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple pichkoo processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``pichkoo_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `pichkoo update` to rebuild the venv with a "
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Pichkoo surface and jump
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Pichkoo, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Pichkoo session.

        A Pichkoo session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Pichkoo session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``PICHKOO_DISABLE_FTS_TRIGRAM`` or not yet created), so
