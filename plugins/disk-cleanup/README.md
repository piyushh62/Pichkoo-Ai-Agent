...
Auto-tracks and cleans up ephemeral files created during Pichkoo AI Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$PICHKOO_HOME` and `/tmp/pichkoo-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `PICHKOO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `PICHKOO_HOME` or `/tmp/pichkoo-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$PICHKOO_HOME/disk-cleanup/` is itself excluded
- `$PICHKOO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
