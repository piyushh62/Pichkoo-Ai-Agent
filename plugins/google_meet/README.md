...
Let the pichkoo agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where pichkoo runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `pichkoo meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `pichkoo meet` CLI |
...
| `cli.py` | `pichkoo meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$PICHKOO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `pichkoo meet node {run,list,approve,remove,status,ping}` |
...
pichkoo meet install                                      # pip + Chromium
pichkoo meet setup                                        # preflight
pichkoo meet auth                                         # optional
pichkoo meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
pichkoo meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.pichkoo/.env
pichkoo meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
pichkoo meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
pichkoo meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.pichkoo/.env
pichkoo meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, pichkoo will **not** switch your system audio input automatically — the
...
pichkoo plugins enable google_meet
pichkoo meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
pichkoo meet node approve my-mac ws://<mac-ip>:18789 <token>
pichkoo meet node ping my-mac
