...
pichkoo meet install                 # pip deps + Chromium (transcribe only)
pichkoo meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
pichkoo meet auth                    # optional; skips guest-lobby wait
pichkoo meet setup                   # preflight checks
```

`pichkoo meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
pip install playwright websockets && python -m playwright install chromium
pichkoo plugins enable google_meet
pichkoo meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
pichkoo meet node approve my-mac ws://<mac-ip>:18789 <token>
pichkoo meet node ping my-mac                   # confirm reachable
```

Run `pichkoo meet setup` to preflight local prereqs.
