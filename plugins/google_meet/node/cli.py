"""`pichkoo meet node ...` subcommand tree.

Wired into the existing ``pichkoo meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``pichkoo meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="pichkoo-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``pichkoo meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             pichkoo meet node approve <name> ws://<host>:{args.port} {token}")
        try:
