...
session-scoped state file under ``$PICHKOO_HOME/workspace/meetings/.active.json``
...
def start(
    ...
    *,
    ...
    guest_name: str = "Pichkoo AI Agent",
    ...
    If a bot is already running for this pichkoo install, leave it first —
    ...
    env = os.environ.copy()
    env["PICHKOO_MEET_URL"] = url
    env["PICHKOO_MEET_OUT_DIR"] = str(out)
    env["PICHKOO_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["PICHKOO_MEET_HEADED"] = "1"
    if auth_state:
        env["PICHKOO_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["PICHKOO_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if PICHKOO_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["PICHKOO_MEET_MODE"] = mode
    if realtime_model:
        env["PICHKOO_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["PICHKOO_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["PICHKOO_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["PICHKOO_MEET_REALTIME_KEY"] = realtime_api_key
