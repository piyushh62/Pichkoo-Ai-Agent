/**
 * Pichkoo Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.pichkoo/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__PICHKOO_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "pichkoo-kanban-dot-triage",
    todo: "pichkoo-kanban-dot-todo",
    ready: "pichkoo-kanban-dot-ready",
    running: "pichkoo-kanban-dot-running",
    blocked: "pichkoo-kanban-dot-blocked",
    done: "pichkoo-kanban-dot-done",
    archived: "pichkoo-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-pichkoo-task";
  ...
  const DOCS_URL = "https://pichkoo-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://pichkoo-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "pichkoo.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="pichkoo-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "pichkoo-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "pichkoo-kanban-md",
  ...
  // standard `drop` event and our `pichkoo-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("pichkoo-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("pichkoo-kanban-column--drop");
          if (target) target.classList.add("pichkoo-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("pichkoo-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("pichkoo-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("pichkoo-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__PICHKOO_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "pichkoo-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "pichkoo-kanban-attention",
        "pichkoo-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "pichkoo-kanban-attention-bar" },
        h("span", { className: "pichkoo-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "pichkoo-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "pichkoo-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "pichkoo-kanban-attention-row",
                  "pichkoo-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "pichkoo-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "pichkoo-kanban-attention-row-id" }, task.id),
                h("span", { className: "pichkoo-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "pichkoo-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "pichkoo-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "pichkoo-kanban-diag-action-btn",
      action.suggested ? "pichkoo-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " pichkoo-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".pichkoo-kanban-drawer-comment-row input, .pichkoo-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "pichkoo-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("pichkoo-kanban-diag", sevClass) },
      h("div", { className: "pichkoo-kanban-diag-header" },
        h("span", { className: "pichkoo-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "pichkoo-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "pichkoo-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "pichkoo-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "pichkoo-kanban-diag-data-row" },
                  h("span", { className: "pichkoo-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "pichkoo-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "pichkoo-kanban-diag-data-row" },
                h("span", { className: "pichkoo-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "pichkoo-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "pichkoo-kanban-diag-reassign-row" },
            h("span", { className: "pichkoo-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "pichkoo-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "pichkoo-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "pichkoo-kanban-diag-msg",
              msg.ok ? "pichkoo-kanban-diag-msg--ok" : "pichkoo-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "pichkoo-kanban-section" },
      h("div", { className: "pichkoo-kanban-section-head-row" },
        h("span", { className: "pichkoo-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "pichkoo-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "pichkoo-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "pichkoo-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "pichkoo-kanban-docs-link",
      title: "Open Pichkoo Kanban docs in a new tab",
      "aria-label": "Pichkoo Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `pichkoo kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "pichkoo-kanban-msg-ok" : "pichkoo-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "pichkoo-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "pichkoo-kanban-boardswitcher" },
      h("div", { className: "pichkoo-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "pichkoo-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "pichkoo-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "pichkoo-kanban-dialog-title" },
        ...
        h("div", { className: "pichkoo-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Pichkoo profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "pichkoo-kanban-bulk" },
      h("span", { className: "pichkoo-kanban-bulk-count" },
      ...
      h("div", { className: "pichkoo-kanban-bulk-priority",
      ...
      h("div", { className: "pichkoo-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Pichkoo profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "pichkoo-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("pichkoo-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("pichkoo-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "pichkoo-kanban-trash",
        dragOver ? "pichkoo-kanban-trash--drop" : "",
        props.draggingTaskId ? "pichkoo-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "pichkoo-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "pichkoo-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".pichkoo-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "pichkoo-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("pichkoo-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("pichkoo-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "pichkoo-kanban-column",
        dragOver ? "pichkoo-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "pichkoo-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "pichkoo-kanban-col-check",
        ...
        h("span", { className: cn("pichkoo-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "pichkoo-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "pichkoo-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "pichkoo-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "pichkoo-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "pichkoo-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "pichkoo-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "pichkoo-kanban-lane" },
                  h("div", { className: "pichkoo-kanban-lane-head" },
                    h("span", { className: "pichkoo-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "pichkoo-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "pichkoo-kanban-card--stale-red";
    if (age >= tier.amber) return "pichkoo-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".pichkoo-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "pichkoo-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "pichkoo-kanban-card",
        props.selected ? "pichkoo-kanban-card--selected" : "",
        props.failed ? "pichkoo-kanban-card--failed" : "",
        props.draggingSource ? "pichkoo-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "pichkoo-kanban-card-content" },
          h("div", { className: "pichkoo-kanban-card-row" },
            h("label", {
              className: "pichkoo-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "pichkoo-kanban-card-check",
            ...
            h("span", { className: "pichkoo-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or pichkoo kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "pichkoo-kanban-warning-badge",
                    "pichkoo-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "pichkoo-kanban-needs-assignee",
          ...
          h("div", { className: "pichkoo-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "pichkoo-kanban-card-row pichkoo-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "pichkoo-kanban-assignee",
                            title: `Assigned to Pichkoo profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "pichkoo-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "pichkoo-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "pichkoo-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "pichkoo-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Pichkoo profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Pichkoo profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "pichkoo-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "pichkoo-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "pichkoo-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "pichkoo-kanban-drawer-close",
        ...
        data ? h("div", { className: "pichkoo-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "pichkoo-kanban-section" },
      h("div", { className: "pichkoo-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "pichkoo-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "pichkoo-kanban-drawer-body" },
      h("div", { className: "pichkoo-kanban-drawer-title" },
        h("span", { className: cn("pichkoo-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "pichkoo-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "pichkoo-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "pichkoo-kanban-section" },
        h("div", { className: "pichkoo-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "pichkoo-kanban-section" },
        h("div", { className: "pichkoo-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "pichkoo-kanban-comment" },
                h("div", { className: "pichkoo-kanban-comment-head" },
                  h("span", { className: "pichkoo-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "pichkoo-kanban-comment-ago" },
      ...
      h("div", { className: "pichkoo-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "pichkoo-kanban-event",
              isDiag ? "pichkoo-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "pichkoo-kanban-event-header" },
                  h("span", { className: "pichkoo-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "pichkoo-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "pichkoo-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "pichkoo-kanban-event-header-plain" },
                  h("span", { className: "pichkoo-kanban-event-kind" }, e.kind),
                  h("span", { className: "pichkoo-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "pichkoo-kanban-event-phantom-row" },
                  h("span", { className: "pichkoo-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "pichkoo-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "pichkoo-kanban-event-payload" },
    ...
    return h("div", { className: "pichkoo-kanban-section" },
      h("div", { className: "pichkoo-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "pichkoo-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `pichkoo-kanban-run--${r.outcome || r.status || "ended"}`
          : "pichkoo-kanban-run--active";
        return h("div", { key: r.id, className: cn("pichkoo-kanban-run", outcomeClass) },
          h("div", { className: "pichkoo-kanban-run-head" },
            h("span", { className: "pichkoo-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "pichkoo-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "pichkoo-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "pichkoo-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "pichkoo-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "pichkoo-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "pichkoo-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "pichkoo-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "pichkoo-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "pichkoo-kanban-pre pichkoo-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "pichkoo-kanban-meta-row" },
      h("span", { className: "pichkoo-kanban-meta-label" }, props.label),
      h("span", { className: "pichkoo-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "pichkoo-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "pichkoo-kanban-meta-row" },
        h("span", { className: "pichkoo-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "pichkoo-kanban-meta-value pichkoo-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "pichkoo-kanban-meta-row" },
        h("span", { className: "pichkoo-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "pichkoo-kanban-meta-value pichkoo-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "pichkoo-kanban-section" },
      h("div", { className: "pichkoo-kanban-section-head-row" },
        h("span", { className: "pichkoo-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "pichkoo-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "pichkoo-kanban-section" },
      h("div", { className: "pichkoo-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "pichkoo-kanban-deps-row" },
        h("span", { className: "pichkoo-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "pichkoo-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "pichkoo-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "pichkoo-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "pichkoo-kanban-dep-chip-x",
      ...
      h("div", { className: "pichkoo-kanban-deps-row" },
        h("span", { className: "pichkoo-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "pichkoo-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "pichkoo-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "pichkoo-kanban-msg-ok"
          : "pichkoo-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "pichkoo-kanban-section" },
      ...
      h("div", { className: "pichkoo-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "pichkoo-kanban-home-sub pichkoo-kanban-home-sub--on"
              : "pichkoo-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__PICHKOO_PLUGINS__ && typeof window.__PICHKOO_PLUGINS__.register === "function") {
    window.__PICHKOO_PLUGINS__.register("kanban", KanbanPage);
  }
})();
