...
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `pichkoo-{profile}` isolates memory per active Hermes profile. Empty placeholders collapse cleanly (e.g. `pichkoo-{user}` with no user becomes `pichkoo`). |
