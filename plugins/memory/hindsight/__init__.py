...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: pichkoo)
...
Or via $PICHKOO_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Pichkoo can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $PICHKOO_HOME/hindsight/config.json  (profile-scoped)
    ...
    return {
        ...
        "banks": {
            "pichkoo": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "pichkoo"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Pichkoo config."""
    profile = config.get("profile", "pichkoo")
    return str(profile or "pichkoo")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Pichkoo profile name (from agent_identity)
      {workspace} — Pichkoo workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``pichkoo-{user}`` with no user becomes ``pichkoo``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Pichkoo AI Agent and the User"
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        from pichkoo_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "pichkoo")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "pichkoo"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: pichkoo-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Pichkoo AI Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "pichkoo"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "pichkoo"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "pichkoo", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "pichkoo")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Pichkoo AI Agent and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See pichkoo-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Pichkoo created/used that client on the shared async loop,
