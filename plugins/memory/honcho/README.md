...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$PICHKOO_HOME/honcho.json` | Profile-local (isolated Pichkoo instances) |
| 2 | `~/.pichkoo/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Pichkoo profile: `pichkoo` (default) or `pichkoo_<profile>`.
...
- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Pichkoo to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Pichkoo session ID (`20260415_a3f2b1`) |
...
- **`per-session`** — Pichkoo session ID (timestamp + hex). Every `pichkoo` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Pichkoo profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Host key is derived from the active Pichkoo profile: `pichkoo` (default) or `pichkoo_<profile>` (e.g. `pichkoo -p coder` -> host key `pichkoo_coder`). Older `pichkoo.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `PICHKOO_HONCHO_HOST` | Host key override |
...
| `pichkoo honcho sync` | Create host blocks for all Pichkoo profiles |
