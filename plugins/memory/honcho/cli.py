...
def cmd_sync(args) -> None:
    ...
    Scans all Pichkoo profiles and creates host blocks for any that don't
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Pichkoo profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $PICHKOO_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Pichkoo persistent cross-session memory.")
...
def _active_profile_name() -> str:
    """Return the active Pichkoo profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Pichkoo' identity in Honcho. Seed with 'pichkoo honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Pichkoo asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Pichkoo + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Pichkoo\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Pichkoo persistent memory")
    ...
    if user_files:
        ...
        print("  (Pichkoo calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Pichkoo, they are seeded once into Honcho's AI peer through the")
    ...
    print("    Pichkoo:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Pichkoo:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Pichkoo:   Honcho observes every message and updates representations")
    ...
    print("    Pichkoo:   per-session by default — each run gets its own session")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Pichkoo Honcho",
    )
