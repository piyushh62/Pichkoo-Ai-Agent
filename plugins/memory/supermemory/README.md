...
Config file: `$PICHKOO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `pichkoo` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `pichkoo-{identity}` → `pichkoo-coder`). |
...
All Supermemory API calls send `x-sm-source: pichkoo`, and document writes stamp
`metadata.sm_source: pichkoo`. This is a **functional routing key, not telemetry**:
it groups Pichkoo-written memories into a dedicated "Pichkoo" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Pichkoo profile:

```json
{
  "container_tag": "pichkoo-{identity}"
}
```

For a profile named `coder`, this resolves to `pichkoo-coder`. The default profile resolves to `pichkoo-default`. Without `{identity}`, all profiles share the same container.
