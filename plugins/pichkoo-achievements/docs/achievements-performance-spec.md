# Pichkoo Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: pichkoo-achievements plugin
...
### `GET /api/plugins/pichkoo-achievements/achievements`
...
### `POST /api/plugins/pichkoo-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/pichkoo-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/pichkoo-achievements/overview`
...
- Persist plugin data under:
  - `~/.pichkoo/plugins/pichkoo-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.pichkoo/plugins/pichkoo-achievements/scan_checkpoint.json`
...
- `~/.pichkoo/plugins/pichkoo-achievements/`
