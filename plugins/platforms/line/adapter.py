...
def is_connected(config) -> bool:
    """Surface in ``pichkoo status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``pichkoo status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``pichkoo setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.pichkoo/.env`` via ``pichkoo_cli.config``.
    """
    print()
    ...
    try:
        from pichkoo_cli.config import get_env_var, set_env_var
    except ImportError:
        print("pichkoo_cli.config not available; set LINE_* vars manually in ~/.pichkoo/.env")
        return
