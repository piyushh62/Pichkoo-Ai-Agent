...
This plugin connects Pichkoo AI Agent to iMessage (and other Spectrum
...
TypeScript-only, Pichkoo runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` are loopback POSTs to the sidecar,
  authenticated with a shared `X-Pichkoo-Sidecar-Token`.
...
2. **Find or create** the `Pichkoo AI Agent` project on the Photon dashboard.
...
There is no separate `login` command; like every other Pichkoo channel,
