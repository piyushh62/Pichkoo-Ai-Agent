"""
Photon Spectrum (iMessage) platform adapter for Pichkoo AI Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Pichkoo dep
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Pichkoo defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Pichkoo-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Pichkoo-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Pichkoo plugin loader at startup."""
