...
Small Node helper that bridges Pichkoo AI Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Pichkoo is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Pichkoo-Sidecar-Token`)
...
The Pichkoo plugin's `pichkoo photon setup` command runs `npm install`
here automatically.
