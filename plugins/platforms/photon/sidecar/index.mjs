// Pichkoo Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Pichkoo): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Pichkoo -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Pichkoo-Sidecar-Token: ${TOKEN}`):
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Pichkoo supplied them so a known-good
