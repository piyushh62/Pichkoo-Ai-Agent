"""SimpleX Chat platform adapter (Pichkoo plugin).
...
This adapter ships as a Pichkoo platform plugin under
``plugins/platforms/simplex/``. The Pichkoo plugin loader scans the
...
Optional environment variables:
    ...
    PICHKOO_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``pichkoo setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Pichkoo — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("PICHKOO_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``pichkoo setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.pichkoo/.env`` via
    ``pichkoo_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "pichkoo_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.pichkoo/.env"
        )
        return
