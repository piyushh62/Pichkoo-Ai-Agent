...
``_check_spotify_available()`` — when the user has not run ``pichkoo auth
spotify``, the tools remain registered (so they appear in ``pichkoo
...
The Spotify auth flow (``pichkoo auth spotify``), CLI plumbing, and docs
