...
2. User plugins: ``$PICHKOO_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$PICHKOO_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$PICHKOO_HOME/plugins/model-providers/`` if it exists."""
    try:
        from pichkoo_constants import get_pichkoo_home

        d = get_pichkoo_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple PICHKOO_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_pichkoo_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$PICHKOO_HOME/plugins/model-providers/<name>/``
    ...
    # 2. User plugins — under $PICHKOO_HOME/plugins/model-providers/<name>/.
