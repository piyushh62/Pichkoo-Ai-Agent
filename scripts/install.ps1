# ============================================================================
# Pichkoo Agent Installer for Windows
# ============================================================================
...
param(
    ...
    # builds apps/desktop into a launchable Pichkoo.exe.
    #
    # Why opt-in:
    #   * Pichkoo-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Pichkoo.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Pichkoo.exe
    ...
    #     `pichkoo desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:NousResearch/pichkoo-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/pichkoo-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Pichkoo Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $PichkooHome)) {
        New-Item -ItemType Directory -Force -Path $PichkooHome | Out-Null
    }
    $envFile = Join-Path $PichkooHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $PichkooHome "node"
...
function Install-Uv {
    # Pichkoo owns its own uv at $PichkooHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (pichkoo_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `pichkoo update` stay in sync.
    $managedUv = Join-Path $PichkooHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $PichkooHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $PichkooHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $PichkooHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $PichkooHome "bin"
...
function Install-Git {
    ...
    which Pichkoo uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\pichkoo\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Pichkoo needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Pichkoo copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\pichkoo\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HERMES_GIT_BASH_PATH`` (User scope) so Pichkoo can find it in a fresh
    ...
    # Download PortableGit into $PichkooHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $PichkooHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Pichkoo features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Pichkoo needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope) so Pichkoo can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $PichkooHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $PichkooHome\git\usr\bin\bash.exe
    $candidates += "$PichkooHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$PichkooHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    Write-Warn "Could not locate bash.exe -- Pichkoo may not find Git Bash."
...
function Test-Node {
    ...
    $managedNode = "$PichkooHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$PichkooHome\node;$env:Path"
        Write-Success "Node.js $version found (Pichkoo-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Pichkoo-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $PichkooHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $PichkooHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\pichkoo-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$PichkooHome\node") { Remove-Item -Recurse -Force "$PichkooHome\node" }
                Move-Item $extractedDir.FullName "$PichkooHome\node"
                ...
                $version = & "$PichkooHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $PichkooHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\pichkoo-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    $stashName = "pichkoo-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Pichkoo behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running pichkoo processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/pichkoo-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/pichkoo-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/pichkoo-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\pichkoo-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\pichkoo-agent-extract"
    ...
    # next `pichkoo update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `pichkoo.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install pichkoo-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and pichkoo.exe broken with
    ...
    # here, not 30 seconds later when the user runs `pichkoo`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `pichkoo dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `pichkoo dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `pichkoo dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up pichkoo command..."
    
    if ($NoVenv) {
        $pichkooBin = "$InstallDir"
    } else {
        $pichkooBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so pichkoo is globally available
    # On Windows, the pichkoo.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$pichkooBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$pichkooBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $pichkooBin"
    } else {
    ...
    Write-Success "pichkoo command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.pichkoo-bootstrap-complete which tells the Pichkoo
    ...
    # Pichkoo-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `pichkoo update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".pichkoo-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$PichkooHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PichkooHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PichkooHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PichkooHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PichkooHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PichkooHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$PichkooHome\.env"
    ...
    # Create config.yaml
    $configPath = "$PichkooHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Pichkoo's prompt-injection scanner
...
"@
    ...
    Write-Success "Configuration directory ready: $PichkooHome"
    
    # Seed bundled skills into $PichkooHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $PichkooHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $PichkooHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$PichkooHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $PichkooHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Pichkoo-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'pichkoo setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\pichkoo-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\pichkoo-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\pichkoo-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Pichkoo` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Pichkoo.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_pichkoo_desktop command
    # resolves apps/desktop/release/win-unpacked/Pichkoo.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Pichkoo-managed
    ...
    # The --dir mode produces an unpacked Pichkoo.exe in
    ...
    # are NOT trying to work around). The Pichkoo icon + product name are
    # stamped onto Pichkoo.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\pichkoo-desktop-build-$(Get-Random).log"
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Pichkoo` rename: app-builder's
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Pichkoo.exe",
        "$desktopDir\release\win-arm64-unpacked\Pichkoo.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Pichkoo.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Pichkoo icon + identity are stamped onto Pichkoo.exe by the
    ...
    #    Pichkoo.exe. We deliberately do NOT point them at `pichkoo desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Pichkoo.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Pichkoo.lnk')
        )
        ...
        # with the Pichkoo icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.pichkoo/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'pichkoo setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m pichkoo_cli.main setup
    } else {
        python -m pichkoo_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $pichkooCmd = "$InstallDir\venv\Scripts\pichkoo.exe"
    if (-not (Test-Path $pichkooCmd)) {
        $pichkooCmd = "pichkoo"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$PichkooHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'pichkoo whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: pichkoo gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$PichkooHome\logs\gateway.log"
            Start-Process -FilePath $pichkooCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$PichkooHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: pichkoo gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: pichkoo gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$PichkooHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$PichkooHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$PichkooHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$PichkooHome\pichkoo-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   pichkoo              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   pichkoo setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   pichkoo config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   pichkoo config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   pichkoo gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   pichkoo update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Pichkoo repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Pichkoo-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Pichkoo to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://pichkoo-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
