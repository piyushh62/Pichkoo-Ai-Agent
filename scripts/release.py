#!/usr/bin/env python3
"""Pichkoo AI Agent Release Script
...
VERSION_FILE = REPO_ROOT / "pichkoo_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "pichkoo@marian.local": "Schrotti77",
    ...
    "274096618+pichkoo-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "pichkoo@nousresearch.com": "NousResearch",
    ...
    "pichkoo@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@pichkoo-agent": "teknium1",
    ...
    "github@nadyapichkoo.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (PICHKOO_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (PICHKOO_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: pichkoo-eval community link)
    ...
    "pichkooagent26@gmail.com": "pichkooagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+pichkooagent26@users.noreply.github.com": "pichkooagent26",
    ...
    "vigo@pichkoo": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.pichkoo@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Pichkoo runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level PICHKOO_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Pichkoo
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"pichkoo-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "pichkoo@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/piyushh62/Pichkoo-AI-Agent",
    ...
    lines.append(f"# Pichkoo AI Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Pichkoo AI Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Pichkoo AI Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Pichkoo AI Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Pichkoo AI Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Pichkoo AI Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Pichkoo AI Agent v{new_version} ({calver_date})' "
