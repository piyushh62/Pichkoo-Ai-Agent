...
# Bootstrap Open WebUI against Pichkoo Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.pichkoo/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-pichkoo.sh
...
#   OPEN_WEBUI_NAME='Johnny Pichkoo'
...
#   HERMES_API_MODEL_NAME='Pichkoo Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Pichkoo Agent WebUI}"
...
HERMES_ENV_FILE="${HERMES_ENV_FILE:-$HOME/.pichkoo/.env}"
...
HERMES_API_MODEL_NAME="${HERMES_API_MODEL_NAME:-Pichkoo Agent}"
HERMES_API_BASE_URL="http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-pichkoo.sh"
LOG_DIR="$HOME/.pichkoo/logs"
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.pichkoo.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.pichkoo</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.pichkoo"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.pichkoo"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-pichkoo.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Pichkoo Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-pichkoo.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.pichkoo/logs/openwebui.log
StandardError=append:%h/.pichkoo/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-pichkoo.service
}
...
main() {
  ...
  log 'Ensuring Pichkoo API server is configured...'
  ...
  log 'Restarting Pichkoo gateway so API server settings take effect...'
  pichkoo gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    log 'Pichkoo API server did not answer on the first check. Trying to start gateway in the background...'
    nohup pichkoo gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  ...
  log "Pichkoo API endpoint: ${HERMES_API_BASE_URL}"
