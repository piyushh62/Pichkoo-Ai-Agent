#!/usr/bin/env node
/**
 * Pichkoo AI Agent WhatsApp Bridge
 *
...
const DEFAULT_REPLY_PREFIX = '⚕ *Pichkoo AI Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Pichkoo AI Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Pichkoo' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `pichkoo_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
