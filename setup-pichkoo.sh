...
# 5. Symlinks the 'pichkoo' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Pichkoo Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/pichkoo-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink pichkoo into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up pichkoo command..."

HERMES_BIN="$SCRIPT_DIR/venv/bin/pichkoo"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/pichkoo"
echo -e "${GREEN}✓${NC} Symlinked pichkoo → $COMMAND_LINK_DISPLAY_DIR/pichkoo"
...
echo ""
echo "Syncing bundled skills to ~/.pichkoo/skills/ ..."
...
echo "Other commands:"
echo "  pichkoo status        # Check configuration"
if is_termux; then
    echo "  pichkoo gateway       # Run gateway in foreground"
else
    echo "  pichkoo gateway install # Install gateway service (messaging + cron)"
fi
echo "  pichkoo cron list     # View scheduled jobs"
echo "  pichkoo doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m pichkoo_cli.main setup
fi
