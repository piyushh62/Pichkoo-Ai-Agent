...
PICHKOO_HOME_DIR="${PICHKOO_HOME:-$HOME/.pichkoo}"
HERMES_CFG="${PICHKOO_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Pichkoo config has twozero_td MCP entry ──
if [[ ! -f "$HERMES_CFG" ]]; then
    echo -e " ${FAIL} Pichkoo config not found at ${HERMES_CFG}"
    manual_steps+=("Create ${HERMES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HERMES_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Pichkoo config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Pichkoo config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
    ...
    manual_steps+=("Restart Pichkoo session to pick up config change")
fi
