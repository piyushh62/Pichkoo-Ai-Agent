#!/usr/bin/env python3
"""Google Workspace API CLI for Pichkoo AI Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Pichkoo-facing JSON contract and falls back to the Python client
...
from _pichkoo_home import get_pichkoo_home
...
def _gws_binary() -> str | None:
    override = os.getenv("PICHKOO_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Pichkoo AI Agent")
