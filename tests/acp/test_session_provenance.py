...
def test_root_session_no_compression(db):
    ...
    assert prov["currentPichkooSessionId"] == "root1"
    assert prov["rootPichkooSessionId"] == "root1"
    assert prov["parentPichkooSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    assert prov["rootPichkooSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousPichkooSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootPichkooSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootPichkooSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    assert prov["previousPichkooSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"pichkoo"}
    assert "sessionProvenance" in meta["pichkoo"]
    assert meta["pichkoo"]["sessionProvenance"]["currentPichkooSessionId"] == "root1"
