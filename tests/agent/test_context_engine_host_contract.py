...
engine plugins (e.g. pichkoo-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``pichkoo plugins`` picker."""
    from pichkoo_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "pichkoo_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "pichkoo_cli.plugins.get_plugin_context_engine",
