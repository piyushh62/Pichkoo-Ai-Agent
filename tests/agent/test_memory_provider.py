...
class TestUserInstalledProviderDiscovery:
    """Memory providers installed to $PICHKOO_HOME/plugins/ should be found.
    ...
    def test_load_user_plugin(self, tmp_path, monkeypatch):
        """load_memory_provider() can load from $PICHKOO_HOME/plugins/."""
        from plugins.memory import load_memory_provider
        self._make_user_memory_plugin(tmp_path, "myexternal")
        monkeypatch.setattr(
            "plugins.memory._get_user_plugins_dir",
    ...
    def test_load_user_plugin_with_relative_import(self, tmp_path, monkeypatch):
        ...
        synthetic ``_pichkoo_user_memory.<name>`` package but never registered
        that parent namespace in sys.modules, so any relative import inside
        the plugin raised
        ``ModuleNotFoundError: No module named '_pichkoo_user_memory'``.
        """
        from plugins.memory import load_memory_provider
...
class TestUserInstalledProviderCli:
    ...
    ``_pichkoo_user_memory.<name>.cli`` without registering the parent
