...
""",
        ...
        from pichkoo_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        pichkoo_home = tmp_path / ".pichkoo"
        pichkoo_home.mkdir()
        (pichkoo_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("PICHKOO_HOME", str(pichkoo_home))
        from pichkoo_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from pichkoo_cli.plugins import PluginContext, PluginManifest, PluginManager
