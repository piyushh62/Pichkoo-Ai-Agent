...
def test_pichkoo_client_tag_includes_current_version():
    """The client tag must reflect pichkoo_cli.__version__ verbatim."""
    from pichkoo_cli import __version__
    from agent.portal_tags import pichkoo_client_tag

    assert pichkoo_client_tag() == f"client=pichkoo-client-v{__version__}"


def test_pichkoo_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import pichkoo_client_tag

    tag = pichkoo_client_tag()
    assert tag.startswith("client=pichkoo-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import pichkoo_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=pichkoo-agent" in tags
    assert pichkoo_client_tag() in tags
    assert len(tags) == 2
