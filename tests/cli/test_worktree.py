...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("pichkoo/pichkoo-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned pichkoo/* and pr-* branches."""

    def test_prunes_orphaned_pichkoo_branch(self, git_repo):
        """pichkoo/pichkoo-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "pichkoo/pichkoo-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "pichkoo/pichkoo-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "pichkoo/pichkoo-deadbeef" in result.stdout
        ...
        assert "pichkoo/pichkoo-deadbeef" in orphaned
        ...
        assert "pichkoo/pichkoo-deadbeef" not in result.stdout
