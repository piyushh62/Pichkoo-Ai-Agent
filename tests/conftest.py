"""Shared fixtures for the pichkoo-agent test suite.
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    #    ~/.pichkoo/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import pichkoo_cli.plugins as _plugins_mod
...
@pytest.fixture()
def mock_config():
    """Return a minimal pichkoo config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``pichkoo-gateway`` process
...
# correlated with a ``tests/pichkoo_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> pichkoo-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart pichkoo-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting pichkoo/python patterns are also blocked.
    """
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch pichkoo-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m pichkoo_cli.main".
                if (
                    "pichkoo" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live pichkoo-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting pichkoo/python could hit the live gateway. "
