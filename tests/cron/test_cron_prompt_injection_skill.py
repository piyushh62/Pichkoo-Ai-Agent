...
def cron_env(tmp_path, monkeypatch):
    """Isolated PICHKOO_HOME with an empty skills tree.

    `tools.skills_tool` snapshots `SKILLS_DIR` at module-import time, so
    setting `PICHKOO_HOME` alone doesn't reach it. We also patch the
    ...
    skills_dir.mkdir()
    (pichkoo_home / "cron").mkdir()
    (pichkoo_home / "cron" / "output").mkdir()
    monkeypatch.setenv("PICHKOO_HOME", str(pichkoo_home))
    monkeypatch.setenv("PICHKOO_BUNDLES_DIR", str(pichkoo_home / "skill-bundles"))
    ...
    monkeypatch.setattr(_skills_tool, "PICHKOO_HOME", pichkoo_home)
    ...
    return pichkoo_home, _scheduler


def _plant_skill(pichkoo_home: Path, name: str, body: str) -> None:
    """Drop a SKILL.md into ~/.pichkoo/skills/<name>/ bypassing skills_guard."""
    skill_dir = pichkoo_home / "skills" / name
...
def _plant_bundle(pichkoo_home: Path, name: str, skills: list[str], instruction: str = "") -> None:
    """Drop a bundle YAML into ~/.pichkoo/skill-bundles/ and refresh cache."""
    bundles_dir = pichkoo_home / "skill-bundles"
...
class TestBuildJobPromptScansSkillContent:
    def test_clean_skill_builds_normally(self, cron_env):
        pichkoo_home, scheduler = cron_env
        _plant_skill(pichkoo_home, "news-digest", "Fetch the top 5 headlines and summarize.")
    ...
    def test_skill_with_invisible_unicode_sanitized_not_blocked(self, cron_env):
        ...
        _plant_skill(pichkoo_home, "zwsp-skill", "clean looking\u200bskill content")
    ...
    def test_skill_bundle_in_job_skills_loads_referenced_skills(self, cron_env):
        pichkoo_home, scheduler = cron_env
        _plant_skill(pichkoo_home, "alpha-skill", "Alpha guidance for the cron task.")
        _plant_skill(pichkoo_home, "beta-skill", "Beta guidance for the cron task.")
        _plant_bundle(
            pichkoo_home,
            "article-pipeline",
    ...
    def test_bundle_name_shadows_skill_name_for_cron_jobs(self, cron_env):
        pichkoo_home, scheduler = cron_env
        _plant_skill(pichkoo_home, "article-pipeline", "Standalone skill should not win.")
        _plant_skill(pichkoo_home, "bundle-member", "Bundle member should win.")
        _plant_bundle(pichkoo_home, "article-pipeline", ["bundle-member"])
...
class TestScriptOutputNotStrictScanned:
    ...
    def test_command_shapes_in_context_from_output_not_blocked(self, cron_env, monkeypatch):
        ...
        output_root = pichkoo_home / "cron" / "output"
