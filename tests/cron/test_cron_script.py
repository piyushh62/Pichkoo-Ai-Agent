...
- Path resolution (absolute, relative to PICHKOO_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp PICHKOO_HOME."""
    pichkoo_home = tmp_path / ".pichkoo"
    pichkoo_home.mkdir()
    (pichkoo_home / "cron").mkdir()
    (pichkoo_home / "cron" / "output").mkdir()
    (pichkoo_home / "scripts").mkdir()
    monkeypatch.setenv("PICHKOO_HOME", str(pichkoo_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "PICHKOO_DIR", pichkoo_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", pichkoo_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", pichkoo_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", pichkoo_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.pichkoo/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("PICHKOO_SESSION_PLATFORM") is None
        assert os.environ.get("PICHKOO_SESSION_CHAT_ID") is None
        assert os.environ.get("PICHKOO_SESSION_CHAT_NAME") is None
