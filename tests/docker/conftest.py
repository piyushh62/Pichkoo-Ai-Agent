...
Override the image with ``PICHKOO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("PICHKOO_TEST_IMAGE", "pichkoo-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``PICHKOO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("PICHKOO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged pichkoo user
# ---------------------------------------------------------------------------
#
# Background: every Pichkoo runtime path inside the container drops to UID
# 10000 (the ``pichkoo`` user) via ``s6-setuidgid pichkoo``. ``docker exec``
...
# fails (PermissionError swallowed) for pichkoo, so a test that ran as root
...
# helpers below run every probe under ``-u pichkoo`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: pichkoo).
