...
The shim (docker/pichkoo-exec-shim.sh, installed at /opt/pichkoo/bin/pichkoo)
...
1. ``docker exec <c> pichkoo …`` (defaulting to root) gets dropped to the
...
def test_shim_drops_root_to_pichkoo_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `pichkoo` with a Python-style `-c` shim equivalent — there's no
    pure-pichkoo "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `pichkoo` is a console_scripts entry — under the
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "pichkoo", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by pichkoo, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "pichkoo:pichkoo", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected pichkoo:pichkoo. "
        "The shim did not drop privileges before invoking pichkoo."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user pichkoo already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "pichkoo", sleep_container,
         "pichkoo", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user pichkoo failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up pichkoo:pichkoo — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_strict_truthiness(
    ...
    assert r.stdout.strip() == "pichkoo:pichkoo", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected pichkoo:pichkoo"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/pichkoo/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid pichkoo pichkoo <args>` which resolves `pichkoo`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> pichkoo login`` (root) wrote
    ...
    `docker exec` path produces is pichkoo-readable.

    Specifically: pretend the operator ran `pichkoo login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the pichkoo UID. We use `pichkoo auth list` since that touches the
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "pichkoo", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the pichkoo user "
