"""Regression test: ``pichkoo dump`` reports a real git SHA inside the container.
...
is absent and ``pichkoo dump`` correctly falls back to ``(unknown)``.
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's pichkoo user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"pichkoo dump exited {r.returncode}: "
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``PICHKOO_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``pichkoo dump``
