"""Tests for Codex auth — tokens stored in Pichkoo auth store (~/.pichkoo/auth.json)."""
...
def _setup_pichkoo_auth(pichkoo_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Pichkoo auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_pichkoo_auth(pichkoo_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_pichkoo_auth(pichkoo_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("PICHKOO_HOME", str(pichkoo_home))
    ...
    monkeypatch.setattr("pichkoo_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    pichkoo_home = tmp_path / "pichkoo"
    _setup_pichkoo_auth(pichkoo_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("PICHKOO_HOME", str(pichkoo_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((pichkoo_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``pichkoo auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``pichkoo auth add openai-codex``.
    ...
    (pichkoo_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `pichkoo auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``pichkoo auth add openai-codex`` for accounts B and C
    ...
    (pichkoo_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``pichkoo auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``pichkoo auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Pichkoo auth store, not ~/.codex/."""
    pichkoo_home = tmp_path / "pichkoo"
    codex_home = tmp_path / "codex-cli"
    pichkoo_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "pichkoo-at", "refresh_token": "pichkoo-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Pichkoo store
    assert not (codex_home / "auth.json").exists()

    # Pichkoo auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "pichkoo-at"


def test_resolve_returns_pichkoo_auth_store_source(tmp_path, monkeypatch):
    pichkoo_home = tmp_path / "pichkoo"
    _setup_pichkoo_auth(pichkoo_home)
    monkeypatch.setenv("PICHKOO_HOME", str(pichkoo_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run pichkoo auth".
    """
    from pichkoo_cli.auth import (
    ...
    assert "pichkoo auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from pichkoo_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from pichkoo_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "pichkoo_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "pichkoo_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("pichkoo_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("pichkoo_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
