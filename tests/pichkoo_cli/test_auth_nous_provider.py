...
from pichkoo_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("PICHKOO_CA_BUNDLE", raising=False)
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="pichkoo_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pichkoo_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pichkoo_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `pichkoo model` → Nous Portal → Skip (keep current) after
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `pichkoo auth add nous`
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Pichkoo is not logged into Nous Portal").
    ...
    # doesn't raise "Pichkoo is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `pichkoo auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Pichkoo must surface an
    ...
    re-authenticate" string led users to report this as a Pichkoo persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from pichkoo_cli.auth import _refresh_access_token
    ...
    assert "pichkoo auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect PICHKOO_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("PICHKOO_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without PICHKOO_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.pichkoo/shared/`` across CI runs.
    """
    from pichkoo_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("PICHKOO_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """PICHKOO_SHARED_AUTH_DIR must redirect the path."""
    from pichkoo_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("PICHKOO_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from pichkoo_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from pichkoo_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from pichkoo_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `pichkoo auth add nous
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from pichkoo_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "pichkoo-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different PICHKOO_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("PICHKOO_HOME", str(profile_a))
    ...
    # Profile B: fresh PICHKOO_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("PICHKOO_HOME", str(profile_b))
