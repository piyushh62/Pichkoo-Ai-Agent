"""Tests for pichkoo backup and import commands."""
...
def _make_pichkoo_tree(root: Path) -> None:
    ...
    (root / "pichkoo_state.db").write_bytes(b"fake-state")
    ...
    # pichkoo-agent repo (should be EXCLUDED)
    (root / "pichkoo-agent").mkdir(exist_ok=True)
    (root / "pichkoo-agent" / "run_agent.py").write_text("# big file\n")
    (root / "pichkoo-agent" / ".git").mkdir()
    (root / "pichkoo-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_pichkoo_ai_agent(self):
        from pichkoo_cli.backup import _should_exclude
        assert _should_exclude(Path("pichkoo-agent/run_agent.py"))
        assert _should_exclude(Path("pichkoo-agent/.git/HEAD"))
...
class TestBackup:
    ...
    def test_excludes_pichkoo_ai_agent(self, tmp_path, monkeypatch):
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "pichkoo-agent" in n]
            assert agent_files == [], f"pichkoo-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        ...
        zips = list(tmp_path.glob("pichkoo-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Pichkoo backup."""
        from pichkoo_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only pichkoo_state.db (old wrong name) is rejected."""
        from pichkoo_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["pichkoo_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_strips_pichkoo_prefix(self, tmp_path, monkeypatch):
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".pichkoo/config.yaml": "model: test\n",
            ".pichkoo/skills/a/SKILL.md": "# A\n",
        })
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from pichkoo_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".pichkoo"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("PICHKOO_HOME", str(dst_home))
        ...
        # pichkoo-agent should NOT be present
        assert not (dst_home / "pichkoo-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from pichkoo_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without pichkoo markers fails validation."""
        import io
        from pichkoo_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_pichkoo(self):
        ...
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".pichkoo/config.yaml", "test")
            zf.writestr(".pichkoo/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".pichkoo/"
...
class TestBackupEdgeCases:
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("pichkoo-backup-*.zip"))
        assert len(zips) == 1
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "pichkoo -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from pichkoo_cli.backup import _safe_copy_db
...
# ---------------------------------------------------------------------------
# Pre-update backup (pichkoo update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``pichkoo update``
    ...
    def test_backup_contents_match_full_backup(self, pichkoo_home):
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # pichkoo-agent repo excluded
        assert not any(n.startswith("pichkoo-agent/") for n in names)
...
class TestRunPreUpdateBackup:
    ...
    def pichkoo_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for pichkoo_cli.config + pichkoo_constants so they pick up PICHKOO_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("pichkoo_cli.config") or mod == "pichkoo_constants":
    ...
    def test_config_enabled_creates_backup(self, pichkoo_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("pichkoo_cli.config"):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (pichkoo claw migrate safety net)
# ---------------------------------------------------------------------------
...
class TestRestoreCronJobsIfEmptied:
    """`pichkoo update` config migration can leave cron/jobs.json valid-but-empty,
