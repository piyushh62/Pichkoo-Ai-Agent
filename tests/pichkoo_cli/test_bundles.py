"""Tests for pichkoo_cli/bundles.py — the `pichkoo bundles` CLI subcommand."""

import argparse

import pytest

from pichkoo_cli.bundles import (
    bundles_command,
    register_cli,
)
...
def bundles_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PICHKOO_BUNDLES_DIR", str(bundles_dir))
