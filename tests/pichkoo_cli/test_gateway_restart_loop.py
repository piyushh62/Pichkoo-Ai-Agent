...
Covers:
- Defense 1: gateway stop/restart refuse when _PICHKOO_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "pichkoo  gateway  restart",         # double spaces
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.pichkoo.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.pichkoo.gateway.plist",
        "launchctl stop ai.pichkoo.gateway",
        "systemctl restart pichkoo-gateway",
        "systemctl stop pichkoo-gateway.service",
        "systemctl start pichkoo-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill pichkoo gateway process",
        "pkill -f pichkoo.*gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "pichkoo cron list",
        "pichkoo update",
        "pichkoo config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_hermes_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade pichkoo then run pichkoo gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.pichkoo.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify pichkoo gateway stop/restart refuse when _PICHKOO_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_PICHKOO_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_PICHKOO_GATEWAY", raising=False)
