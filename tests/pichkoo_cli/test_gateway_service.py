...
import pichkoo_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "pichkoo gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`pichkoo gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `pichkoo gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `pichkoo gateway restart`)."""
        unit_path = tmp_path / "pichkoo-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``PICHKOO_HOME`` into the unit's
        ...
        # Realistic generated unit referencing a pytest tmpdir PICHKOO_HOME
        polluted_unit = (
            "[Service]\n"
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "pichkoo gateway install" in out
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("PICHKOO_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("PICHKOO_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("PICHKOO_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.pichkoo.gateway.plist"
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup pichkoo gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: pichkoo gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup pichkoo gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.pichkoo.gateway"])
            ),
        )
...
class TestSystemUnitPichkooHome:
    """PICHKOO_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'PICHKOO_HOME=/home/alice/.pichkoo' in unit
        assert '/root/.pichkoo' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: PICHKOO_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
    ...
    def test_system_unit_preserves_custom_pichkoo_home(self, monkeypatch):
        ...
        assert 'PICHKOO_HOME=/opt/pichkoo-shared' in unit
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "pichkoo gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.pichkoo/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".pichkoo" / "profiles" / "mybot"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile PICHKOO_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "pichkoo"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.pichkoo/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".pichkoo" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".pichkoo" / "profiles" / "My Bot!"
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".pichkoo" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.pichkoo.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".pichkoo" / "pichkoo-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".pichkoo" / "pichkoo-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/pichkoo", str(tmp_path / "alice"))
        assert result == "/opt/pichkoo"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".pichkoo" / "pichkoo-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".pichkoo" / "pichkoo-agent"
        ...
        # WorkingDirectory is anchored at the target user's PICHKOO_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.pichkoo" in unit
        assert "WorkingDirectory=/home/alice/.pichkoo/pichkoo-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "pichkoo-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "pichkoo-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'pichkoo gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'pichkoo gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'pichkoo gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "pichkoo gateway run" in out
...
class TestLegacyPichkooUnitDetection:
    ...
    renamed to ``pichkoo-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``pichkoo.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``pichkoo-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Pichkoo Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m pichkoo_cli.main gateway run --replace\n"
    )
    ...
    def test_ignores_profile_unit_pichkoo_gateway_coder(self, tmp_path, monkeypatch):
        ...
        for base in (user_dir, system_dir):
            (base / "pichkoo-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "pichkoo-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "pichkoo-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_ignores_unrelated_pichkoo_service(self, tmp_path, monkeypatch):
        ...
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "pichkoo.service").write_text(
            "[Unit]\nDescription=Some Other Pichkoo\n[Service]\n"
            "ExecStart=/opt/other-pichkoo/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m pichkoo_cli.main gateway run
          - python path/to/pichkoo_cli/main.py gateway run
          - pichkoo gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m pichkoo_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/pichkoo/pichkoo_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/pichkoo gateway run --replace",
            "ExecStart=/venv/bin/python /opt/pichkoo/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"pichkoo.service" if i == 0 else f"pichkoo.service"  # same name
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "pichkoo.service" in out
        assert "pichkoo gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "pichkoo.service"
...
class TestRemoveLegacyPichkooUnits:
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop pichkoo.service" in c for c in cmds_joined)
        assert any("--user disable pichkoo.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo pichkoo gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop pichkoo.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable pichkoo.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "pichkoo.service"
        system_legacy = system_dir / "pichkoo.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (pichkoo-gateway-coder.service)
        ...
        profile_unit = user_dir / "pichkoo-gateway-coder.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `pichkoo gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import pichkoo_cli.main as cli_main
        ...
        # public, just confirm that `pichkoo gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import pichkoo_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "pichkoo_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pichkoo_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "pichkoo-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "pichkoo-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "pichkoo-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``pichkoo gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "pichkoo-gateway")
        ...
        assert "sudo systemctl start pichkoo-gateway" in out
        assert "sudo pichkoo gateway uninstall --system" in out
        assert "pichkoo gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart pichkoo-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop pichkoo-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``pichkoo gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (PICHKOO_HOME), never the source checkout / worktree, so a relocated or
