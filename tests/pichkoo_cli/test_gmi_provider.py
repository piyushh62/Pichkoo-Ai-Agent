...
from pichkoo_cli.auth import resolve_provider
...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from pichkoo_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # pichkoo model, not via upgrade prompts.
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("pichkoo_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in PICHKOO_OVERLAYS
        overlay = PICHKOO_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    ...
    def test_run_doctor_checks_gmi_models_endpoint(self, monkeypatch, tmp_path):
        ...
        try:
            from pichkoo_cli import auth as _auth_mod
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a PichkooAgent User-Agent
    ...
    def test_gmi_profile_declares_hermes_user_agent(self):
        ...
        assert ua.startswith("PichkooAgent/"), (
            f"expected GMI profile User-Agent to start with 'PichkooAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("pichkoo_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "pichkoo_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["pichkoo", "chat", "--provider", "gmi"])

        from pichkoo_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("pichkoo_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("pichkoo_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("pichkoo_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
