"""Tests for pichkoo_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import pichkoo_cli.gui_uninstall as gu


def _make_agent(pichkoo_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = pichkoo_home / "pichkoo-agent"
    (agent_root / "pichkoo_cli").mkdir(parents=True)
    (agent_root / "pichkoo_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(pichkoo_home: Path) -> None:
    """Create the source-built GUI artifacts a `pichkoo desktop` run produces."""
    desktop = pichkoo_home / "pichkoo-agent" / "apps" / "desktop"
    ...
    (pichkoo_home / "pichkoo-agent" / "node_modules").mkdir(parents=True)
    (pichkoo_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(pichkoo_home: Path) -> None:
    (pichkoo_home / "config.yaml").write_text("x: 1\n")
    (pichkoo_home / ".env").write_text("KEY=secret\n")
    (pichkoo_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    pichkoo_home = tmp_path / ".pichkoo"
    pichkoo_home.mkdir()
    assert gu.agent_is_installed(pichkoo_home) is False
    _make_agent(pichkoo_home)
    assert gu.agent_is_installed(pichkoo_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    pichkoo_home = tmp_path / ".pichkoo"
    (pichkoo_home / "pichkoo-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(pichkoo_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    pichkoo_home = tmp_path / ".pichkoo"
    _make_gui_build(pichkoo_home)
    artifacts = gu.source_built_gui_artifacts(pichkoo_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(pichkoo_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(pichkoo_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    pichkoo_home = tmp_path / ".pichkoo"
    agent_root = _make_agent(pichkoo_home)
    _make_gui_build(pichkoo_home)
    _make_user_data(pichkoo_home)
    ...
    removed = gu.uninstall_gui(pichkoo_home)
    ...
    assert not (pichkoo_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "pichkoo_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (pichkoo_home / "config.yaml").exists()
    assert (pichkoo_home / ".env").exists()
    assert (pichkoo_home / "sessions").exists()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(pichkoo_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(pichkoo_home)
    ...
    assert summary["pichkoo_home"] == str(pichkoo_home)
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(uninstall, "get_pichkoo_home", lambda: pichkoo_home)
    ...
    from pichkoo_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    ...
    pichkoo_home = tmp_path / ".pichkoo"
    (pichkoo_home / "pichkoo-agent" / "pichkoo_cli").mkdir(parents=True)
    (pichkoo_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not pichkoo_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m pichkoo_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_pichkoo_home", lambda: pichkoo_home)
    ...
    assert not (desktop / "release").exists()
    assert not (pichkoo_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "pichkoo_cli").exists()
    assert (pichkoo_home / "config.yaml").exists()
