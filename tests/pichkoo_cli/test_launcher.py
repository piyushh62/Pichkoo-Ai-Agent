"""Tests for the top-level `./pichkoo` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./pichkoo` should use `pichkoo_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "pichkoo"
    called = []

    fake_main_module = types.ModuleType("pichkoo_cli.main")

    def fake_main():
        called.append("pichkoo_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "pichkoo_cli.main", fake_main_module)
    ...
    assert called == ["pichkoo_cli.main"]
