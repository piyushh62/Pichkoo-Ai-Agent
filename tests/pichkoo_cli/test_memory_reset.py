"""Tests for the `pichkoo memory reset` CLI command.
...
def memory_env(tmp_path, monkeypatch):
    ...
    memories = pichkoo_home / "memories"
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `pichkoo memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        pichkoo_home = tmp_path / ".pichkoo"
        (pichkoo_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("PICHKOO_HOME", str(pichkoo_home))
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_pichkoo_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
