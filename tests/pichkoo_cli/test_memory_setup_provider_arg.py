"""Tests for `pichkoo memory setup [provider]` routing.
...
``pichkoo memory setup honcho``) without the interactive picker — which matters
because the per-provider ``pichkoo <provider>`` subcommand is only registered
...
from pichkoo_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "pichkoo memory setup" in out
