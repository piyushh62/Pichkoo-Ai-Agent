"""Tests for pichkoo_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from pichkoo_cli.model_catalog import _validate_manifest
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in pichkoo_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from pichkoo_cli import model_catalog
        from pichkoo_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``pichkoo model`` CLI — not the live
        ...
        try:
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("pichkoo_cli.models.check_nous_free_tier", return_value=False), patch(
                "pichkoo_cli.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "pichkoo_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
