"""Tests for the Pichkoo plugin system (pichkoo_cli.plugins)."""
...
from pichkoo_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<pichkoo_home>/plugins/``; we derive
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <PICHKOO_HOME>/config.yaml.
...
class TestPluginDiscovery:
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PICHKOO_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".pichkoo" / "plugins"
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("pichkoo_cli.plugins._plugin_manager", mgr):
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("pichkoo_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("pichkoo_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("pichkoo_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("pichkoo_cli.plugins.PluginContext.dispatch_tool.__module__", "pichkoo_cli.plugins"):
...
class TestPluginDebugLogging:
    """PICHKOO_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("PICHKOO_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With PICHKOO_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("PICHKOO_PLUGINS_DEBUG", "1")
        from pichkoo_cli import plugins as plugins_mod
