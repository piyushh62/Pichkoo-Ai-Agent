"""Tests for pichkoo_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("pichkoo_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from pichkoo_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("pichkoo_cli.config.load_config")
    ...
    @patch("pichkoo_cli.config.load_config")
    @patch.dict("os.environ", {"PICHKOO_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """PICHKOO_SESSION_PLATFORM should be used when PICHKOO_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PICHKOO_HOME", str(tmp_path))
        monkeypatch.setenv("PICHKOO_PLATFORM", "telegram")
        monkeypatch.setenv("PICHKOO_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from pichkoo_cli.skills_config import _get_categories
        skills = [
