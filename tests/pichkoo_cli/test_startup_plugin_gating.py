...
``pichkoo_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``pichkoo --help``, ``pichkoo version``,
``pichkoo logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``pichkoo --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from pichkoo_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["pichkoo", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["pichkoo", "-w", "--tui"], None),
        (["pichkoo", "version"], "version"),
        (["pichkoo", "--tui", "chat"], "chat"),
        (["pichkoo", "-w", "logs"], "logs"),
        (["pichkoo", "chat", "hello world"], "chat"),
        (["pichkoo", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["pichkoo", "-m", "gpt5", "chat"], "chat"),
        (["pichkoo", "--model", "gpt5", "chat", "hi"], "chat"),
        (["pichkoo", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["pichkoo", "-z", "hello world"], None),
        (["pichkoo", "-z", "hello", "chat"], "chat"),
        (["pichkoo", "--model=gpt5", "chat"], "chat"),     # inline form
        (["pichkoo", "--", "chat"], "chat"),               # -- terminator
        (["pichkoo", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["pichkoo", "some-plugin-cmd"], "some-plugin-cmd"),
        (["pichkoo", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["pichkoo"],                          # bare → chat
        ["pichkoo", "--help"],                # top-level help
        ["pichkoo", "-h"],
        ["pichkoo", "version"],               # known built-in
        ["pichkoo", "logs"],
        ["pichkoo", "gateway", "run"],
        ["pichkoo", "--tui"],
        ["pichkoo", "-w", "--tui"],
        ["pichkoo", "chat", "hi"],
        ["pichkoo", "help"],                  # accepted built-in-ish
        ["pichkoo", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["pichkoo", "meet", "join"],          # potential google_meet plugin
        ["pichkoo", "honcho", "status"],      # potential memory plugin
        ["pichkoo", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``pichkoo help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"pichkoo_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
