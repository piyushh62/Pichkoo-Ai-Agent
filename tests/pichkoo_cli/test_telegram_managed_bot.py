"""Tests for pichkoo_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateDeepLink:
    ...
    def test_name_url_encoded(self):
        link = generate_deep_link(
            ...
            suggested_name="Pichkoo & Friends",
        )
        assert "Pichkoo+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        with patch(
            "pichkoo_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Pichkoo AI Agent")
        ...
        post.assert_called_once_with(
            ...
            json={"bot_name": "Pichkoo AI Agent"},
            timeout=10.0,
        )
...
class TestPollForToken:
    ...
    def test_immediate_success(self):
        ...
        with patch(
            "pichkoo_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("pichkoo_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("pichkoo_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("pichkoo_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("pichkoo_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("pichkoo_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "pichkoo_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("pichkoo_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("pichkoo_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from pichkoo_cli.setup import _setup_telegram_auto
