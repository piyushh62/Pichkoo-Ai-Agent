...
def main_mod(monkeypatch):
    import pichkoo_cli.main as mod
...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["pichkoo", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "pichkoo_cli.plugins",
...
def test_termux_fast_tui_launch_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["pichkoo", "--tui", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
        main_mod,
        "cmd_chat",
...
def test_termux_fast_tui_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(sys, "argv", ["pichkoo", "--tui", "--help"])
...
def test_termux_fast_cli_launch_chat_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["pichkoo", "chat", "-q", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("PICHKOO_TUI", raising=False)
    monkeypatch.delenv("PICHKOO_DEFER_AGENT_STARTUP", raising=False)
    monkeypatch.delenv("PICHKOO_FAST_STARTUP_BANNER", raising=False)
    ...
    assert os.environ["PICHKOO_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["PICHKOO_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_fast_cli_launch_oneshot_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["pichkoo", "-z", "hello", "--model", "gpt-test", "--provider", "openai"],
    )
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("PICHKOO_TERMUX_DISABLE_FAST_CLI", raising=False)
    monkeypatch.setattr(sys, "argv", ["pichkoo", "--version"])

    assert main_mod._try_termux_ultrafast_version() is True

    out = capsys.readouterr().out
    assert "Pichkoo AI Agent v" in out
...
def test_termux_fast_cli_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("PICHKOO_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["pichkoo", "chat", "--help"])
...
def test_termux_fast_cli_launch_can_be_disabled(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setenv("PICHKOO_TERMUX_DISABLE_FAST_CLI", "1")
    monkeypatch.delenv("PICHKOO_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["pichkoo", "version"])
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["pichkoo", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "pichkoo_cli.plugins",
...
def test_oneshot_fails_closed_on_empty_final_response(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    import pichkoo_cli.oneshot as oneshot_mod
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from pichkoo_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "pichkoo -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """pichkoo -z bypasses PichkooCLI, but recall still needs SessionDB."""
    from pichkoo_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "pichkoo_state", mod("pichkoo_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "pichkoo_cli.config",
        mod("pichkoo_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "pichkoo_cli.models",
        mod("pichkoo_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "pichkoo_cli.tools_config",
        mod("pichkoo_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["PICHKOO_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/pichkoo-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["PICHKOO_MODEL"] == "nous/pichkoo-test"
    assert env["PICHKOO_INFERENCE_MODEL"] == "nous/pichkoo-test"
    assert env["PICHKOO_TUI_PROVIDER"] == "nous"
    assert env["PICHKOO_INFERENCE_PROVIDER"] == "nous"
    assert env["PICHKOO_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["PICHKOO_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("pichkoo-tui-active-session-")
...
def test_launch_tui_applies_terminal_backend_config(
    ...
    assert captured["env"]["TERMINAL_DOCKER_IMAGE"] == "example/pichkoo-tools:latest"
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("pichkoo_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("PICHKOO_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "PICHKOO_TUI_RESUME" not in captured["env"]
...
def test_make_tui_argv_dev_prebuilds_pichkoo_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("PICHKOO_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "pichkoo_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "pichkoo --tui --resume 20260409_000001_abc123" in out
    assert 'pichkoo --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "pichkoo --tui --resume actual_session" in out
