...
def _make_pichkoo_node(pichkoo_home: Path) -> Path:
    """Create a fake $PICHKOO_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = pichkoo_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_pichkoo_node(fake_home):
    pichkoo_home = fake_home / ".pichkoo"
    node_bin = _make_pichkoo_node(pichkoo_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(pichkoo_home)
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    pichkoo_home = fake_home / ".pichkoo"
    _make_pichkoo_node(pichkoo_home)
    local_bin = fake_home / ".local" / "bin"
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(pichkoo_home) == []


def test_removes_dangling_symlink_into_pichkoo_node(fake_home):
