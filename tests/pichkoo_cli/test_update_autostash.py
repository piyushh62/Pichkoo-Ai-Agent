...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M pichkoo_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    Leaving conflict markers in source files makes pichkoo unrunnable (SyntaxError).
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="pichkoo_cli/main.py\n", stderr="", returncode=0)
    ...
    assert "pichkoo_cli/main.py" in out
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``pichkoo update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    marker = tmp_path / ".pichkoo-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by pichkoo update (pichkoo_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "pichkoo-update-autostash")

    assert marker.exists(), (
        ".pichkoo-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".pichkoo-bootstrap-complete" not in status
