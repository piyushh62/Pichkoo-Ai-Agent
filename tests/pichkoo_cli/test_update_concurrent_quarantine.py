"""Tests for issue #26670 — concurrent pichkoo.exe detection and improved
quarantine retry / reboot-deferred fallback during `pichkoo update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_pichkoo_instances
# helper (and need the autouse stub in tests/pichkoo_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_pichkoo_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "pichkoo.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "pichkoo.exe").write_bytes(b"")
    (scripts_dir / "pichkoo-gateway.exe").write_bytes(b"")

    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter([]))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_pichkoo_instances(scripts_dir)
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "pichkoo.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_pichkoo_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "pichkoo.exe"
    shim.write_bytes(b"")
    ...
    procs = [
        _make_proc(other_pid, str(shim), "pichkoo.exe"),
    ...
    assert result == [(other_pid, "pichkoo.exe")]
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_pichkoo_instances(tmp_path) == []
...
def test_detect_concurrent_excludes_parent_chain(_winp, tmp_path):
    ...
    Simulates the real Windows topology: pichkoo.exe launcher (PID L) spawns
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "pichkoo.exe"),
    ]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_still_finds_unrelated_other_pichkoo(_winp, tmp_path):
    """A sibling pichkoo.exe outside our ancestor chain must still be reported."""
    scripts_dir = tmp_path
    shim = scripts_dir / "pichkoo.exe"
    shim.write_bytes(b"")
    me = os.getpid()
    launcher_pid = me + 100  # our .exe launcher (parent — must be excluded)
    sibling_pid = me + 200  # an UNRELATED pichkoo.exe (must still be reported)

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "pichkoo.exe"),
        _make_proc(sibling_pid, str(shim), "pichkoo.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
        parent_chain=[launcher_pid],
        proc_iter_rows=rows,
    ...
    assert result == [(sibling_pid, "pichkoo.exe")]
...
def test_detect_concurrent_parent_chain_walks_deep(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(parent_pid, str(shim), "pichkoo.exe"),
        _make_proc(grandparent_pid, str(shim), "pichkoo.exe"),
        _make_proc(greatgrandparent_pid, str(shim), "pichkoo.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    assert result == [(launcher_pid, "pichkoo.exe")]
...
def test_detect_concurrent_parent_walk_handles_stub_without_process(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "pichkoo.exe"),
        _make_proc(other_pid, str(shim), "pichkoo.exe"),
    ]
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    matches = [(1234, "pichkoo.exe"), (5678, "pichkoo.exe")]
...
# ---------------------------------------------------------------------------
# _quarantine_running_pichkoo_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "pichkoo.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_pichkoo_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("pichkoo.exe.old.")
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_pichkoo_exe_shims", lambda d: [shim])
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another pichkoo.exe is running, the update bails out before
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "pichkoo.exe")])
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_pichkoo_instances", detect
    ), patch.object(
