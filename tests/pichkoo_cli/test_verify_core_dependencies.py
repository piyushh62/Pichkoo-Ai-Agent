...
base deps) during ``pichkoo update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point pichkoo_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with patch("pichkoo_cli.main._resolve_install_target_python", return_value=py), \
             patch("pichkoo_cli.main.subprocess.run") as mock_run, \
             patch("pichkoo_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_triggers_reinstall_when_dep_missing(self, temp_pyproject, fake_venv_python):
        ...
        with patch("pichkoo_cli.main._resolve_install_target_python", return_value=py), \
             patch("pichkoo_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("pichkoo_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("pichkoo_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("pichkoo_cli.main._run_install_with_heartbeat") as mock_install:
            from pichkoo_cli.main import _verify_core_dependencies_installed
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        quarantine the running ``pichkoo.exe`` on Windows before installing.
        ...
        is left missing and ``pichkoo`` drops off PATH. Previously this path
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `pichkoo update`.
        ...
        with patch("pichkoo_cli.main._is_windows", return_value=True):
            from pichkoo_cli.main import _resolve_install_target_python
