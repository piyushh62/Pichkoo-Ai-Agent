"""Tests for pichkoo_cli.web_server and related config utilities."""
...
def _install_example_plugin(_isolate_pichkoo_home):
    ...
    isolated ``PICHKOO_HOME``.

    The user-plugin source is preferred over a transient
    ``PICHKOO_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, pichkoo-achievements, model providers
    ...
    #      and start serving requests against a torn-down PICHKOO_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Pichkoo vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Pichkoo env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints PICHKOO_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("PICHKOO_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("PICHKOO_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def test_get_media_requires_auth(self):
        from pichkoo_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("pichkoo_state.SessionDB", _FakeDB)
    ...
    def test_update_pichkoo_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        resp = self.client.post("/api/pichkoo/update")
    ...
    def test_update_pichkoo_spawns_on_non_docker_install(self, monkeypatch):
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "pichkoo-update"}
        assert calls == [(["update"], "pichkoo-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("pichkoo-update", None)
        web_server._ACTION_RESULTS.pop("pichkoo-update", None)
        web_server._ACTION_PROCS["pichkoo-update"] = proc

        resp = self.client.get("/api/actions/pichkoo-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "pichkoo-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["pichkoo-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from pichkoo_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from pichkoo_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "pichkoo_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from pichkoo_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from pichkoo_cli.config import save_env_value
        from pichkoo_cli.web_server import _SESSION_TOKEN
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from pichkoo_cli.config import load_config, load_env
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        payload = ws._telegram_onboarding_request_sync(
            ...
            body={"bot_name": "Pichkoo AI Agent"},
        ...
        assert kwargs["json"] == {"bot_name": "Pichkoo AI Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("PichkooDashboard/")
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``pichkoo gateway restart`` child (e.g. when a stale
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/pichkoo_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import pichkoo_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "pichkoo-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from pichkoo_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from pichkoo_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "pichkoo-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `pichkoo model`), not the first curated paid entry."""
        import pichkoo_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from pichkoo_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from pichkoo_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    def test_profile_setup_command_uses_pichkoo_for_default_profile(self):
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "pichkoo setup"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import pichkoo_cli.skills_config as skills_config
        import pichkoo_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import pichkoo_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from pichkoo_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from pichkoo_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from pichkoo_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.pichkoo/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``pichkoo_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in pichkoo_cli/web_server.py."
        )
...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_pichkoo_home, _install_example_plugin):
        ...
        installs it into the per-test ``PICHKOO_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into PICHKOO_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, pichkoo-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (pichkoo-achievements is loaded by default).
        resp = self.client.get("/api/plugins/pichkoo-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``pichkoo --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["PICHKOO_TUI_INLINE"] == "1"
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        ...
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/pichkoo-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/pichkoo-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"pichkoo-ws-ok" in buf:
                    break
            assert b"pichkoo-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from pichkoo_cli.pty_bridge import PtyUnavailableError
        ...
        import pichkoo_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        PICHKOO_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from pichkoo_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("PICHKOO_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_pichkoo_home, _install_example_plugin):
        ...
        the per-test ``PICHKOO_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_pichkoo_home):
        ...
        monkeypatch.setenv("PICHKOO_DESKTOP", "1")
    ...
    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_pichkoo_home):
        ...
        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("PICHKOO_DESKTOP", raising=False)
