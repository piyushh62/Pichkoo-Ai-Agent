...
Run from the PR worktree:
    ...
    cd ~/.pichkoo/pichkoo-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.pichkoo/pichkoo-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.pichkoo/pichkoo-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a pichkoo-agent checkout"
)
...
# Isolated PICHKOO_HOME for the config write.
