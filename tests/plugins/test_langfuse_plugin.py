...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_pichkoo_config(self, monkeypatch):
        ...
        # Drop any cached import of pichkoo_cli.config.
        sys.modules.pop("pichkoo_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "pichkoo_cli.config" not in sys.modules, (
            "langfuse plugin imported pichkoo_cli.config — regression toward "
