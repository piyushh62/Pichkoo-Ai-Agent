...
AlexKucera reported on Discord (2026-04-16) that after ``pichkoo update`` pulled
...
Opt-in — not part of default CI:
    PICHKOO_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.pichkoo/.env).
"""
...
LIVE = os.environ.get("PICHKOO_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set PICHKOO_LIVE_TESTS=1"),
