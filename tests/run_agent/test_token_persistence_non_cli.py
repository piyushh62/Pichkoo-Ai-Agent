...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    pichkoo_state = ModuleType("pichkoo_state")
    pichkoo_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "pichkoo_state", pichkoo_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Pichkoo"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Pichkoo"
