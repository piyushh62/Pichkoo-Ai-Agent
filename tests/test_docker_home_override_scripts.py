...
def test_dashboard_run_does_not_derive_insecure_from_bind_host() -> None:
    ...
    ``PICHKOO_DASHBOARD_HOST``. Doing so disables the OAuth auth gate on
    ...
    The opt-in is now explicit: ``PICHKOO_DASHBOARD_INSECURE=1`` (truthy).
    ...
    assert '127.0.0.1|localhost' not in text, (
        ...
        "is the authority now — opt in via PICHKOO_DASHBOARD_INSECURE instead."
    )
    ...
    assert "PICHKOO_DASHBOARD_INSECURE" in text, (
        "Explicit PICHKOO_DASHBOARD_INSECURE opt-in is missing."
    )
    # Truthy values aligned with the rest of the s6 scripts
    # (e.g. PICHKOO_DASHBOARD).
    for truthy in ("1", "true", "TRUE", "True", "yes", "YES", "Yes"):
        assert truthy in text, (
            f"PICHKOO_DASHBOARD_INSECURE should accept truthy value {truthy!r}"
        )
...
def test_stage2_hook_repairs_profiles_and_cron_ownership_on_every_boot() -> None:
    ...
    assert 'if [ -d "$PICHKOO_HOME/profiles" ]; then' in text
    assert 'chown -R pichkoo:pichkoo "$PICHKOO_HOME/profiles" 2>/dev/null || true' in text

    assert 'if [ -d "$PICHKOO_HOME/cron" ]; then' in text
    assert 'chown -R pichkoo:pichkoo "$PICHKOO_HOME/cron" 2>/dev/null || true' in text
