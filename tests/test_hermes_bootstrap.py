"""Tests for pichkoo_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Pichkoo entry point
(pichkoo, pichkoo-agent, pichkoo-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Pichkoo entry point imports pichkoo_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported pichkoo_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("pichkoo_bootstrap", None)
    import pichkoo_bootstrap  # noqa: WPS433
    return pichkoo_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Pichkoo CLI
...
class TestEntryPointsImportBootstrap:
    """Every Pichkoo entry point must import pichkoo_bootstrap as its
    ...
    # Entry points that invoke Pichkoo as a process.  Each one must
    # import pichkoo_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "pichkoo_cli/main.py",   # pichkoo CLI (console_script)
        "run_agent.py",          # pichkoo-agent (console_script)
        "acp_adapter/entry.py",  # pichkoo-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import pichkoo_bootstrap' and that
        ...
        half-finished ``pichkoo update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``pichkoo_bootstrap`` as a top-level module) leaves pichkoo
        ...
        """
        # Resolve relative to the pichkoo-agent repo root.  Tests live
        # at tests/test_pichkoo_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # pichkoo start even when pichkoo_bootstrap hasn't been
        ...
        assert first_import_name == "pichkoo_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'pichkoo_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import pichkoo_bootstrap' line to be the first import."
        )
