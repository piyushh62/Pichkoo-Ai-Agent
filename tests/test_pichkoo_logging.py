"""Tests for pichkoo_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    pichkoo_logging._logging_initialized = False
    ...
    pichkoo_logging._install_session_record_factory()
    yield
...
class TestSessionContext:
    ...
    def test_session_context_thread_isolated(self, pichkoo_home):
        ...
        def thread_a():
            pichkoo_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            pichkoo_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        pichkoo_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        pichkoo_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = pichkoo_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = pichkoo_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_tools_prefix(self):
        assert ("tools",) == pichkoo_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = pichkoo_logging.COMPONENT_PREFIXES["cli"]
        assert "pichkoo_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == pichkoo_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = pichkoo_logging.COMPONENT_PREFIXES["gui"]
        assert "pichkoo_cli.web_server" in prefixes
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        pichkoo_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = pichkoo_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        pichkoo_logging.set_session_context("factory_test")
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> pichkoo_logging._ManagedRotatingFileHandler:
        handler = pichkoo_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that pichkoo_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = pichkoo_logging._safe_stderr()
