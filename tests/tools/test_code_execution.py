...
UDS socket lifecycle, pichkoo_tools generation, timeout enforcement,
...
class TestPichkooToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_pichkoo_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)

    def test_generates_subset(self):
        src = generate_pichkoo_tools_module(["terminal", "web_search"])
    ...
    def test_empty_list_generates_nothing(self):
        src = generate_pichkoo_tools_module([])
    ...
    def test_non_allowed_tools_ignored(self):
        src = generate_pichkoo_tools_module(["vision_analyze", "terminal"])
        self.assertIn("def terminal(", src)
        self.assertNotIn("def vision_analyze(", src)

    def test_rpc_infrastructure_present(self):
        src = generate_pichkoo_tools_module(["terminal"])
        self.assertIn("PICHKOO_RPC_SOCKET", src)
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_pichkoo_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"pichkoo_rpc\")", src)
        self.assertNotIn('os.environ.get("PICHKOO_RPC_DIR", "/tmp/pichkoo_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_pichkoo_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/pichkoo_exec_", mkdir_cmd)
        self.assertIn("PICHKOO_RPC_DIR=/data/data/com.termux/files/usr/tmp/pichkoo_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/pichkoo_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/pichkoo_exec_", mkdir_cmd)
...
"""
        ...
        # terminal won't be in pichkoo_tools.py, so import fails
        self.assertEqual(result["status"], "error")
...
"""
    ...
    def test_web_search_tool(self):
        ...
        code = """
from pichkoo_tools import web_search
...
"""
    ...
    def test_json_parse_helper(self):
        ...
        code = r"""
from pichkoo_tools import json_parse
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from pichkoo_tools import shell_quote
...
"""
    ...
    def test_retry_helper_success(self):
        ...
        code = """
from pichkoo_tools import retry
counter = [0]
def flaky():
    counter[0] += 1
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_pichkoo_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("PICHKOO_RPC_SOCKET", child_env)
...
class TestExecuteCodeEdgeCases(unittest.TestCase):
    ...
    def test_none_enabled_tools_uses_all(self):
        ...
        code = (
            "from pichkoo_tools import terminal, web_search, read_file\n"
    ...
    def test_empty_enabled_tools_uses_all(self):
        ...
        code = (
            "from pichkoo_tools import terminal, web_search\n"
            "print('imports ok')\n"
        )
        with patch("model_tools.handle_function_call",
                    return_value=json.dumps({"ok": True})):
