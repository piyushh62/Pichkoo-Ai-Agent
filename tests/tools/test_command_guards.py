...
def _clean_state():
    ...
    saved = {}
    for k in ("PICHKOO_INTERACTIVE", "PICHKOO_GATEWAY_SESSION", "PICHKOO_EXEC_ASK", "PICHKOO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["PICHKOO_INTERACTIVE"] = "1"
        session_key = os.getenv("PICHKOO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No PICHKOO_INTERACTIVE or PICHKOO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
