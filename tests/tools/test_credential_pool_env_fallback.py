...
- _seed_from_env reads API keys from ~/.pichkoo/.env when not in os.environ
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.pichkoo/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.pichkoo/.env, not just os.environ.
