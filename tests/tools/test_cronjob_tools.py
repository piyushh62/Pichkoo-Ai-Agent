...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `pichkoo-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.pichkoo/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.pichkoo/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because pichkoo uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("PICHKOO_INTERACTIVE", raising=False)
        monkeypatch.delenv("PICHKOO_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("PICHKOO_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("PICHKOO_INTERACTIVE", false_like_value)
        monkeypatch.delenv("PICHKOO_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("PICHKOO_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["PICHKOO_INTERACTIVE", "PICHKOO_GATEWAY_SESSION", "PICHKOO_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("PICHKOO_INTERACTIVE", "PICHKOO_GATEWAY_SESSION", "PICHKOO_EXEC_ASK"):
