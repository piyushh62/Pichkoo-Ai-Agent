...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad PICHKOO_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: PICHKOO_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("PICHKOO_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("PICHKOO_CRON_SESSION", "1")
    monkeypatch.delenv("PICHKOO_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_pichkoo_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "PICHKOO_HOME": "/h", "PICHKOO_PROFILE": "p",
        "PICHKOO_CONFIG": "/c.yaml", "PICHKOO_ENV": "/e",
        # other PICHKOO_* → dropped (broad prefix removed)
        "PICHKOO_BASE_URL": "https://x", "PICHKOO_INTERACTIVE": "1",
        "PICHKOO_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("PICHKOO_HOME", "PICHKOO_PROFILE", "PICHKOO_CONFIG", "PICHKOO_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "PICHKOO_BASE_URL", "PICHKOO_INTERACTIVE", "PICHKOO_KANBAN_DB",
...
def test_env_scrub_logs_dropped_pichkoo_vars(caplog):
    """Dropping a non-allowlisted, non-secret PICHKOO_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset PICHKOO_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "PICHKOO_HOME": "/h",          # allowlisted → kept, not logged
        "PICHKOO_BASE_URL": "https://x",   # dropped → logged
        "PICHKOO_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "PICHKOO_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "PICHKOO_HOME" in out and "PATH" in out
    assert "PICHKOO_BASE_URL" not in out and "PICHKOO_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "PICHKOO_BASE_URL" in msgs and "PICHKOO_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "PICHKOO_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped PICHKOO_* vars."""
    import logging
