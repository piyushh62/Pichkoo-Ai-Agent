...
def test_pichkoo_provider_subclass_exists():
    """PichkooMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _PICHKOO_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _PICHKOO_PROVIDER_CLS is not None
    assert issubclass(_PICHKOO_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_pichkoo_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns PichkooMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _PICHKOO_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("PICHKOO_HOME", str(tmp_path))
    ...
    assert _PICHKOO_PROVIDER_CLS is not None
    assert isinstance(provider, _PICHKOO_PROVIDER_CLS)
    assert provider._pichkoo_server_name == "srv"
