...
class TestSyncSkills:
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "pichkoo skills reset new-skill" in captured
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `pichkoo profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.pichkoo) rely on so bundled
    skills are never seeded at install time NOR re-injected by `pichkoo update`.
    """
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        pichkoo_home = tmp_path / "home"
        pichkoo_home.mkdir()
        (pichkoo_home / ".no-bundled-skills").write_text("opted out\n")

        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.PICHKOO_HOME", pichkoo_home):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    """`pichkoo skills opt-out/opt-in` core: marker toggle + safe removal."""
