...
PICHKOO_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
PICHKOO_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as
...
This mirrors the existing PICHKOO_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def test_seed_block_present_and_guarded(stage2_text: str) -> None:
    ...
    assert '[ ! -f "$PICHKOO_HOME/gateway_state.json" ]' in block, (
        "seed must be guarded by [ ! -f ] so persisted state wins on restart"
    )
    assert "PICHKOO_GATEWAY_BOOTSTRAP_STATE" in block
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $PICHKOO_HOME.

    ``env_value`` is the PICHKOO_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            f'export PICHKOO_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'
            if env_value is not None
            else "unset PICHKOO_GATEWAY_BOOTSTRAP_STATE\n"
        )
        script = (
            "set -e\n"
            f'PICHKOO_HOME="{home}"\n'
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when PICHKOO_GATEWAY_BOOTSTRAP_STATE is unset"
