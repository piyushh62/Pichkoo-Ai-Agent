...
the volume.  PICHKOO_UID/PICHKOO_GID must still take precedence when both are
set.

The s6-overlay rework moved bootstrap from docker/entrypoint.sh (now a shim)
to docker/stage2-hook.sh, which is installed as /etc/cont-init.d/01-pichkoo-setup
...
def _alias_lines(text: str) -> list[str]:
    """The stage2 hook lines that resolve PICHKOO_UID/PICHKOO_GID from aliases."""
    return [
        line.strip()
        for line in text.splitlines()
        if line.strip().startswith(("PICHKOO_UID=", "PICHKOO_GID="))
    ]
...
def test_stage2_hook_resolves_puid_pgid_aliases(stage2_text: str) -> None:
    ...
    assert any("PUID" in line for line in alias_lines), (
        "docker/stage2-hook.sh must resolve PICHKOO_UID from a PUID alias; see #15290"
    )
    assert any("PGID" in line for line in alias_lines), (
        "docker/stage2-hook.sh must resolve PICHKOO_GID from a PGID alias; see #15290"
    )
...
def _resolve(stage2_text: str, env: dict[str, str]) -> str:
    ...
    resolved ``PICHKOO_UID:PICHKOO_GID`` pair."""
    bash = shutil.which("bash")
    if bash is None:
        pytest.skip("bash not available")
    script = "\n".join(_alias_lines(stage2_text))
    script += '\necho "${PICHKOO_UID:-}:${PICHKOO_GID:-}"\n'
    proc = subprocess.run(
        [bash, "-ec", script],
...
def test_no_uid_vars_leaves_values_empty(stage2_text: str) -> None:
    # An empty resolution means the stage2 hook keeps the default pichkoo user.
...
def test_stage2_hook_documents_config_migration_opt_out(stage2_text: str) -> None:
    assert "PICHKOO_SKIP_CONFIG_MIGRATION" in stage2_text
