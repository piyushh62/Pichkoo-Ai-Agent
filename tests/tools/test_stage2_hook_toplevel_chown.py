...
top-level state files in $PICHKOO_HOME — but only those, never arbitrary
host-owned files.

Regression guard for the gateway restart loop reported in #35098: files such
as gateway.lock / state.db / auth.json live directly under $PICHKOO_HOME (not in
...
bind-mounted $PICHKOO_HOME may contain host-owned files Pichkoo does not manage,
...
def _toplevel_chown_loop(text: str) -> str:
    """Extract the `for f in … chown pichkoo:pichkoo "$PICHKOO_HOME/$f" … done`
    ...
    block = m.group(1)
    assert 'chown pichkoo:pichkoo "$PICHKOO_HOME/$f"' in block, (
...
def test_no_blanket_find_user_root_sweep(stage2_text: str) -> None:
    ...
    $PICHKOO_HOME contents — that would clobber host-owned files in a bind mount
    (#19788 / PR #19795)."""
    assert not re.search(r"find\s+\"?\$\{?PICHKOO_HOME\}?\"?[^\n]*-user\s+root", stage2_text), (
        "stage2-hook.sh must not blanket-chown root-owned files under "
        "$PICHKOO_HOME via `find -user root`; use the targeted allowlist instead "
        "so host-owned bind-mounted files are preserved (#19788, #19795)."
    )


def _run_loop(text: str, present_files: list[str]) -> list[str]:
    """Run the extracted chown loop in a sandbox $PICHKOO_HOME, with `chown`
