...
Lets the LLM write a Python script that calls Pichkoo tools via RPC,
...
# available on every platform Pichkoo itself runs on.
...
# match a safe prefix, the operational PICHKOO_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "PICHKOO_" prefix was deliberately removed (#27303) — it leaked
# PICHKOO_*-named config that lacks a secret substring (e.g. PICHKOO_BASE_URL,
# PICHKOO_KANBAN_DB, PICHKOO_*_WEBHOOK).  The child only needs the few
# location/profile vars in _PICHKOO_CHILD_ALLOWED below; PICHKOO_RPC_SOCKET /
# PICHKOO_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational PICHKOO_* vars the child legitimately needs by exact name — these
...
_PICHKOO_CHILD_ALLOWED = frozenset({
    "PICHKOO_HOME",
    "PICHKOO_PROFILE",
    "PICHKOO_CONFIG",
    "PICHKOO_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational PICHKOO_* vars (_PICHKOO_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret PICHKOO_* vars dropped by the tightened allowlist (#27303). The
    # broad "PICHKOO_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like PICHKOO_KANBAN_DB / PICHKOO_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _PICHKOO_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Pichkoo-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Pichkoo-profile soft guard."""',
...
"""Auto-generated Pichkoo tools RPC stubs."""
...
def _connect():
    ...
    PICHKOO_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["PICHKOO_RPC_SOCKET"]
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"PICHKOO_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("PICHKOO_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # selector from PICHKOO_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  PICHKOO_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["PICHKOO_RPC_SOCKET"] = rpc_endpoint
        ...
        # PICHKOO_TIMEZONE is an internal Pichkoo setting and must not leak
        # into child processes.
        _tz_name = os.getenv("PICHKOO_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("PICHKOO_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {PICHKOO_HOME}/home/ when that directory exists.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        "Run a Python script that can call Pichkoo tools programmatically. "
