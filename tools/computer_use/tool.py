...
# regardless of approval level (e.g. logout kills the session Pichkoo runs in).
_BLOCKED_KEY_COMBOS = {
...
def _get_backend() -> ComputerUseBackend:
    ...
    with _backend_lock:
        if _backend is None:
            backend_name = os.environ.get("PICHKOO_COMPUTER_USE_BACKEND", "cua").lower()
            if backend_name in {"cua", "cua-driver", ""}:
                ...
                raise RuntimeError(f"Unknown PICHKOO_COMPUTER_USE_BACKEND={backend_name!r}")
            _backend.start()
        return _backend
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$PICHKOO_HOME/cache/vision/``
