...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.pichkoo",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``PICHKOO_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside PICHKOO_HOME so that a malicious
    ...
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to PICHKOO_HOME)",
            relative_path,
        )
        return False
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.pichkoo/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="pichkoo-skills-safe-"))
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.pichkoo``,
    SSH ``<remote_home>/.pichkoo``, etc.) and whether translation is wanted.
