"""
Cron job management tools for Pichkoo AI Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("PICHKOO_SESSION_PLATFORM")
    origin_chat_id = get_session_env("PICHKOO_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("PICHKOO_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("PICHKOO_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within PICHKOO_HOME/scripts/.
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "profile": {
                "type": "string",
                "description": "Optional Pichkoo profile name to run the job under. When set, the scheduler resolves that profile, applies a context-local Pichkoo home override, loads that profile's config/.env for the run, and bridges PICHKOO_HOME into subprocesses. Any temporary process-environment changes from profile .env loading are restored after the job exits. Use 'default' for the root Pichkoo profile. Named profiles must already exist. When unset (default), preserves the scheduler's existing profile. On update, pass an empty string to clear. Jobs with profile run sequentially (not parallel) to keep profile-scoped runtime state isolated."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("PICHKOO_INTERACTIVE")
        or env_var_enabled("PICHKOO_GATEWAY_SESSION")
        or env_var_enabled("PICHKOO_EXEC_ASK")
    )
