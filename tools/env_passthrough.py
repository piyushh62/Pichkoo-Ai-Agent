...
def _is_hermes_provider_credential(name: str) -> bool:
    ...
    Non-Pichkoo API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _PICHKOO_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Pichkoo-managed provider credentials (from
    ``_PICHKOO_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Pichkoo-managed
    ...
    Non-Pichkoo third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Pichkoo-managed provider credentials must not be passed
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        ...
                        "by _PICHKOO_PROVIDER_ENV_BLOCKLIST). Operator "
