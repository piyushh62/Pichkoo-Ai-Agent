...
def auth_json_path():
    """Return the Pichkoo auth store path, respecting PICHKOO_HOME overrides."""
    return get_pichkoo_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`pichkoo tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    ...
    try:
        from pichkoo_cli.auth import resolve_nous_access_token
