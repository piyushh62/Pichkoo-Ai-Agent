#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Pichkoo Skills Hub.
...
Used by pichkoo_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
# ---------------------------------------------------------------------------
# Pichkoo centralized index source
# ---------------------------------------------------------------------------

PICHKOO_INDEX_URL = "https://pichkoo-agent.nousresearch.com/docs/api/skills-index.json"
PICHKOO_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "pichkoo-index.json"
...
def _load_pichkoo_index() -> Optional[dict]:
    ...
    We cache it locally for PICHKOO_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if PICHKOO_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - PICHKOO_INDEX_CACHE_FILE.stat().st_mtime
            if age < PICHKOO_INDEX_TTL:
                return json.loads(PICHKOO_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(PICHKOO_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Pichkoo index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Pichkoo index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        PICHKOO_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        PICHKOO_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class PichkooIndexSource(SkillSource):
    """Skill source backed by the centralized Pichkoo Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "pichkoo-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "pichkoo-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        PichkooIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "pichkoo-index"
