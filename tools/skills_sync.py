...
Copies bundled skills from the repo's skills/ directory into ~/.pichkoo/skills/
...
The manifest lives at ~/.pichkoo/skills/.bundled_manifest.
"""
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.pichkoo/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.pichkoo/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.pichkoo/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.pichkoo) that wrote the
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.pichkoo/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `pichkoo update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `pichkoo skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `pichkoo skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `pichkoo update` runs "
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `pichkoo update` (or `pichkoo skills "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.pichkoo/skills/ ...")
