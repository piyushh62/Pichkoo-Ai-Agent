...
Backend is selected during ``pichkoo tools`` setup (web.backend in config.yaml).
When available, Pichkoo can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Pichkoo config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Pichkoo' config/.env layer (``pichkoo config set``, ``pichkoo tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.pichkoo/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``pichkoo tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `pichkoo tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `pichkoo tools` to set one up."
                ),
            }
        else:
            logger.info(
