...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``pichkoo tools`` repaint, tool-registration scans,
    ...
    2. ``~/.pichkoo/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.pichkoo/.env`` first, then ``os.environ``.

    Wraps :func:`pichkoo_cli.config.get_env_value` so tests can patch
