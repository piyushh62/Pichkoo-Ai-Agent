...
React + Ink terminal UI for Pichkoo. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `PICHKOO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.pichkoo/.pichkoo_history` or under `PICHKOO_HOME`.
...
```text
ui-tui/
  packages/pichkoo-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      pichkoo-ink.d.ts    type declarations for @pichkoo/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent PichkooCLI subprocess for slash commands
```
