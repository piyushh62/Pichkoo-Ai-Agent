...
await build({
  ...
  // Skip the prebuilt @pichkoo/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@pichkoo/ink': resolve(root, 'packages/pichkoo-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The pichkoo_cli launcher
