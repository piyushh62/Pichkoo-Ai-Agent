import type { RunExternalProcess } from '@pichkoo/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`pichkoo ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching pichkoo: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`pichkoo ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
