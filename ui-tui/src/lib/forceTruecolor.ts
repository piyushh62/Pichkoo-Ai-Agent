...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.PICHKOO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Pichkoo on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via PICHKOO_TUI_TRUECOLOR=1.
