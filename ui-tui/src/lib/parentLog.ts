...
const logDir = join(process.env.PICHKOO_HOME?.trim() || join(homedir(), '.pichkoo'), 'logs')
...
// a real ~/.pichkoo (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('pichkoo-tui: parent lifecycle log unavailable\n')
    }
  }
}
