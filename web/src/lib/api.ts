...
// injects ``window.__PICHKOO_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__PICHKOO_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const PICHKOO_BASE_PATH = readBasePath();
const BASE = PICHKOO_BASE_PATH;
...
const SESSION_HEADER = "X-Pichkoo-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // ``window.__PICHKOO_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__PICHKOO_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__PICHKOO_SESSION_TOKEN__`` is valid, so the next
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__PICHKOO_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Pichkoo dashboard server");
}
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__PICHKOO_AUTH_REQUIRED__) {
 ...
 * ``window.__PICHKOO_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  checkPichkooUpdate: (force = false) =>
