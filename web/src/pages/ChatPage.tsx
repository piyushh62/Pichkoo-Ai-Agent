/**
 * ChatPage — embeds `pichkoo --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (pichkoo_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { PICHKOO_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${PICHKOO_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__PICHKOO_SESSION_TOKEN__ &&
    !window.__PICHKOO_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `pichkoo dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const token = window.__PICHKOO_SESSION_TOKEN__;
    const gated = !!window.__PICHKOO_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Pichkoo TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[pichkoo-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__PICHKOO_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Pichkoo. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="pichkoo-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
...
declare global {
  interface Window {
    __PICHKOO_SESSION_TOKEN__?: string;
    __PICHKOO_AUTH_REQUIRED__?: boolean;
  }
}
