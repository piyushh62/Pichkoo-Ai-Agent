/**
 * Pichkoo Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__PICHKOO_PLUGIN_SDK__`` and
 * ``window.__PICHKOO_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__PICHKOO_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``PichkooPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@pichkoo/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__PICHKOO_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__PICHKOO_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__PICHKOO_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface PichkooPluginSDK {
...
declare global {
  interface Window {
    __PICHKOO_PLUGIN_SDK__?: PichkooPluginSDK;
    __PICHKOO_PLUGINS__?: PluginRegistry;
  }
}
