...
import { api, PICHKOO_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${PICHKOO_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      script.setAttribute("data-pichkoo-plugin", manifest.name);
