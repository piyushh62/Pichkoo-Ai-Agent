...
The ACP adapter wraps Pichkoo' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
pichkoo acp / pichkoo-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.pichkoo/.env
  -> configure stderr logging
  -> construct PichkooACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'pichkoo-agent[acp]==<version>' pichkoo-acp`, pointed at the `pichkoo-agent` PyPI release.
...
- `allow_once` -> Pichkoo `once`
- `allow_always` -> Pichkoo `always`
- reject options -> Pichkoo `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Pichkoo tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["pichkoo-acp"])
...
Instead it reuses Pichkoo' runtime resolver:

- `acp_adapter/auth.py`
- `pichkoo_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Pichkoo provider/credentials. It also always advertises a terminal setup auth method (`pichkoo-setup`, args `--setup`) so first-run registry clients can open Pichkoo' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.pichkoo/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `pichkoo-acp` toolset definition
- `pichkoo_cli/main.py` — `pichkoo acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `pichkoo-acp` script
