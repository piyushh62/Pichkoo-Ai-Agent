...
sidebar_position: 1
title: "Architecture"
description: "Pichkoo AI Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Pichkoo AI Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ pichkoo_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # PichkooCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── pichkoo_state.py           # SQLite session/state database with FTS5
├── pichkoo_constants.py       # PICHKOO_HOME, profile-aware paths
...
│
├── pichkoo_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `pichkoo` subcommands (large file)
...
│   ├── skills_config.py      # pichkoo skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # pichkoo tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # pichkoo gateway start/stop
│
...
```text
User input → PichkooCLI.process_input()
...
Three discovery sources: `~/.pichkoo/plugins/` (user), `.pichkoo/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `pichkoo plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-pichkoo-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Pichkoo as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`pichkoo -p <name>`) gets its own PICHKOO_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
