...
description: "How Pichkoo stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `pichkoo_cli/cron.py` | CLI `pichkoo cron` subcommands |
...
Jobs are stored in `~/.pichkoo/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `pichkoo cron` commands are run or during active CLI sessions.
...
```python
# ~/.pichkoo/scripts/check_competitors.py
...
2. **Environment variable** — `PICHKOO_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.pichkoo/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `pichkoo cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `pichkoo cron` CLI provides direct job management:

```bash
pichkoo cron list                    # Show all jobs
pichkoo cron create                  # Interactive job creation (alias: add)
pichkoo cron edit <job_id>           # Edit job configuration
pichkoo cron pause <job_id>          # Pause a running job
pichkoo cron resume <job_id>         # Resume a paused job
pichkoo cron run <job_id>            # Trigger immediate execution
pichkoo cron remove <job_id>         # Delete a job
```
