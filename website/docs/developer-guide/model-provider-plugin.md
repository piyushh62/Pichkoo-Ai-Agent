...
description: "How to build a model provider (inference backend) plugin for Pichkoo AI Agent"
---
...
The only required file is `__init__.py`. `plugin.yaml` is used by `pichkoo plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `pichkoo_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `pichkoo_cli/main.py` | Accepts `acme-inference` |
| `pichkoo model` picker | `pichkoo_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `pichkoo doctor` | `pichkoo_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `pichkoo setup` | `pichkoo_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `pichkoo_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| `display_name` | str | Human label shown in `pichkoo model` picker |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.pichkoo/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. Pichkoo picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but Pichkoo' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
- [Building a Pichkoo Plugin](/guides/build-a-pichkoo-plugin) — general plugin authoring
