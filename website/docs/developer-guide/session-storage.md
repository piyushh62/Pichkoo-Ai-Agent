...
Pichkoo AI Agent uses a SQLite database (`~/.pichkoo/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `pichkoo_state.py`
...
Multiple pichkoo processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.pichkoo/state.db
...
`~/.pichkoo/` by default, or the value of `HERMES_HOME` environment variable.
