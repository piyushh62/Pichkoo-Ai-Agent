...
description: "Install and deploy Pichkoo AI Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

Pichkoo AI Agent ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`pichkoo setup`, `pichkoo gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage pichkoo the same way you manage any other NixOS service.
:::
...
After `nix profile install`, `pichkoo`, `pichkoo-agent`, and `pichkoo-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `pichkoo setup` walks you through provider selection, `pichkoo gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.pichkoo/`.
...
# /etc/nixos/flake.nix (or your system flake)
{
  ...
  outputs = { nixpkgs, pichkoo-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        pichkoo-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
# configuration.nix
{ config, ... }: {
  services.pichkoo-agent = {
    enable = true;
    settings.model.default = "anthropic/claude-sonnet-4";
    environmentFiles = [ config.sops.secrets."pichkoo-env".path ];
    addToSystemPackages = true;
  };
}
```

That's it. `nixos-rebuild switch` creates the `pichkoo` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o pichkoo /dev/stdin /var/lib/pichkoo/env
```

```nix
services.pichkoo-agent.environmentFiles = [ "/var/lib/pichkoo/env" ];
```
:::
...
:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `pichkoo` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.
...
Users listed in `hostUsers` are automatically added to the `pichkoo` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo pichkoo chat` manually.
:::
...
systemctl status pichkoo-agent

# Watch logs (Ctrl+C to stop)
journalctl -u pichkoo-agent -f

# If addToSystemPackages is true, test the CLI
pichkoo version
pichkoo config       # shows the generated config
```
...
```nix
# base.nix
services.pichkoo-agent.settings = {
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Pichkoo defaults to OpenRouter.
:::
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."pichkoo-env".path ]` |
| Give the agent a personality | `${services.pichkoo-agent.stateDir}/.pichkoo/SOUL.md` | manage the file directly |
...
| Override the pichkoo package | `package` | `inputs.pichkoo-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/pichkoo"` |
...
{
  sops = {
    defaultSopsFile = ./secrets/pichkoo.yaml;
    age.keyFile = "/home/user/.config/sops/age/keys.txt";
    secrets."pichkoo-env" = { format = "yaml"; };
  };

  services.pichkoo-agent.environmentFiles = [
    config.sops.secrets."pichkoo-env".path
  ];
}
```
...
```yaml
# secrets/pichkoo.yaml (encrypted with sops)
pichkoo-env: |
...
```nix
{
  age.secrets.pichkoo-env.file = ./secrets/pichkoo-env.age;

  services.pichkoo-agent.environmentFiles = [
    config.age.secrets.pichkoo-env.path
  ];
}
```
...
{
  services.pichkoo-agent = {
    authFile = config.sops.secrets."pichkoo/auth.json".path;
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Pichkoo looks for specific filenames by convention:
...
```nix
{
  services.pichkoo-agent.documents = {
...
```nix
{
  services.pichkoo-agent.mcpServers = {
    filesystem = {
      command = "npx";
      args = [ "-y" "@modelcontextprotocol/server-filesystem" "/data/workspace" ];
    };
...
```nix
{
  services.pichkoo-agent.mcpServers.remote-api = {
...
Set `auth = "oauth"` for servers using OAuth 2.1. Pichkoo implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.

```nix
{
  services.pichkoo-agent.mcpServers.my-oauth-server = {
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Pichkoo prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u pichkoo` (native):

```bash
# Container mode
docker exec -it pichkoo-agent \
...
```bash
pichkoo mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.pichkoo/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/pichkoo/.pichkoo/mcp-tokens/
# Ensure: chown pichkoo:pichkoo, chmod 0600
```
...
```nix
{
  services.pichkoo-agent.mcpServers.analysis = {
    command = "npx";
...
When pichkoo runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `pichkoo setup` | Config is declarative — edit `settings` in your Nix config |
| `pichkoo config edit` | Config is generated from `settings` |
| `pichkoo config set <key> <value>` | Config is generated from `settings` |
| `pichkoo gateway install` | The systemd service is managed by NixOS |
| `pichkoo gateway uninstall` | The systemd service is managed by NixOS |
...
When container mode is enabled, pichkoo runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-pichkoo-agent-0.1.0  ──►  /nix/store/... (ro)
~/.pichkoo -> /var/lib/pichkoo/.pichkoo       (symlink bridge, per hostUsers)
/var/lib/pichkoo/                    ──►  /data/          (rw)
...
The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted — it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/pichkoo gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated — the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/pichkoo` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
| `systemctl restart pichkoo-agent` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the pichkoo package itself do **not** trigger recreation.
...
If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/pichkoo-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current pichkoo package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `pichkoo plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [pichkoo-lcm](https://github.com/stephenschoettler/pichkoo-lcm)):

```nix
services.pichkoo-agent.extraPlugins = [
...
The package's `site-packages` is added to PYTHONPATH in the pichkoo wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in pichkoo-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.pichkoo-agent.extraDependencyGroups = [ "messaging" ];
```
...
```nix
{
  ...
  outputs = { pichkoo-agent, nixpkgs, ... }: {
    ...
    # Then:
    #   pkgs.pichkoo-agent.override { extraPythonPackages = [...]; }
    #   pkgs.pichkoo-agent.override { extraDependencyGroups = [ "hindsight" ]; }
  };
}
```
...
```nix
services.pichkoo-agent.settings.plugins.enabled = [
  "pichkoo-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core pichkoo dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the pichkoo-agent service |
| `package` | `package` | `pichkoo-agent` | The pichkoo-agent package to use |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.pichkoo` symlink to the service stateDir and are auto-added to the `pichkoo` group |
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Pichkoo binary + all Nix deps |
| `/data` | `/var/lib/pichkoo` | `rw` | All state, config, workspace |
| `/home/pichkoo` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
cd /etc/nixos && nix flake update pichkoo-agent
...
```bash
systemctl status pichkoo-agent
docker ps -a --filter name=pichkoo-agent
docker inspect pichkoo-agent --format='{{.State.Status}}'
docker exec -it pichkoo-agent bash
docker exec pichkoo-agent readlink /data/current-package
docker exec pichkoo-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop pichkoo-agent
docker rm -f pichkoo-agent
sudo rm /var/lib/pichkoo/.container-identity
sudo systemctl start pichkoo-agent
```
...
```bash
# Native mode
sudo -u pichkoo cat /var/lib/pichkoo/.pichkoo/.env

# Container mode
docker exec pichkoo-agent cat /data/.pichkoo/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec pichkoo-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u pichkoo-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `pichkoo version` shows old version | Container not restarted | `systemctl restart pichkoo-agent` |
| Permission denied on `/var/lib/pichkoo` | State dir is `0750 pichkoo:pichkoo` | Use `docker exec` or `sudo -u pichkoo` |
| `nix-collect-garbage` removed pichkoo | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "pichkoo-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
| `unable to find user pichkoo` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart pichkoo-agent` |
