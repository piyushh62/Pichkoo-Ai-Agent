...
pichkoo cron create "0 2 * * *" \
  "You are a project manager triaging the piyushh62/Pichkoo-AI-Agent GitHub repo.

1. Run: gh issue list --repo piyushh62/Pichkoo-AI-Agent --state open --json number,title,labels,author,createdAt --limit 30
...
pichkoo cron create "0 9 * * 1" \
  "Scan the piyushh62/Pichkoo-AI-Agent repo for documentation drift.

1. Run: gh pr list --repo piyushh62/Pichkoo-AI-Agent --state merged --json number,title,files,mergedAt --limit 30
