...
With `AZURE_ANTHROPIC_KEY` set in `~/.hermes/.env`. Pichkoo detects `azure.com` in the base URL and short-circuits around the Claude Code OAuth token chain so the Azure key is used directly with `x-api-key` auth.
...
The Azure SDK reads the `AZURE_*` env vars directly. Pichkoo never inspects them other than to report which sources are present in `hermes doctor` output.
