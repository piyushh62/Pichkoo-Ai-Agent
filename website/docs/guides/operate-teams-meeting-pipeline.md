...
```bash
pichkoo teams-pipeline validate
```
...
```bash
pichkoo teams-pipeline token-health
pichkoo teams-pipeline token-health --force-refresh
```
...
```bash
pichkoo teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
pichkoo teams-pipeline maintain-subscriptions
pichkoo teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Pichkoo cron (recommended if you already run the Pichkoo gateway)

Pichkoo ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.pichkoo/scripts/`. First create the script:

```bash
mkdir -p ~/.pichkoo/scripts
cat > ~/.pichkoo/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec pichkoo teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.pichkoo/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
pichkoo cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/pichkoo-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Pichkoo Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=pichkoo
EnvironmentFile=/etc/pichkoo/env
ExecStart=/usr/local/bin/pichkoo teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/pichkoo-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Pichkoo Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now pichkoo-teams-pipeline-maintain.timer
systemctl list-timers pichkoo-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/pichkoo teams-pipeline maintain-subscriptions >> /var/log/pichkoo/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.pichkoo/.env` at the top of a wrapper script that crontab calls.
...
```bash
pichkoo teams-pipeline subscriptions   # should show expirationDateTime advanced
pichkoo teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
pichkoo teams-pipeline list
pichkoo teams-pipeline list --status failed
pichkoo teams-pipeline show <job-id>
```

### Replay a stored job

```bash
pichkoo teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
pichkoo teams-pipeline fetch --meeting-id <meeting-id>
pichkoo teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `pichkoo teams-pipeline maintain-subscriptions --dry-run`
- inspect `pichkoo teams-pipeline list --status failed`
...
- run `pichkoo teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `pichkoo teams-pipeline run`
...
- [ ] `pichkoo teams-pipeline validate` returns an OK snapshot
- [ ] `pichkoo teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Pichkoo cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
