...
This tutorial walks you through setting up a Telegram bot powered by Pichkoo AI Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Pichkoo AI Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.pichkoo/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Pichkoo itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Pichkoo AI Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.pichkoo/.env`:
...
```
[Gateway] Starting Pichkoo Gateway...
...
# View live logs
journalctl --user -u pichkoo-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `pichkoo gateway install` to update the plist.
:::
...
```bash
pichkoo gateway stop && pichkoo gateway start
```
...
pichkoo pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.pichkoo/.env`:
...
Control how much detail the bot shows when using tools. In `~/.pichkoo/config.yaml`:
...
Customize how the bot communicates by editing `~/.pichkoo/SOUL.md`:

For a full guide, see [Use SOUL.md with Pichkoo](/guides/use-soul-with-pichkoo).
...
```bash
# From the CLI
pichkoo cron list          # View all scheduled jobs
pichkoo cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u pichkoo-gateway` (Linux) or `~/.pichkoo/logs/gateway.log` (macOS) |
| Cron job output | `~/.pichkoo/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.pichkoo/cron/jobs.json` |
| Pairing data | `~/.pichkoo/pairing/` |
| Session history | `~/.pichkoo/sessions/` |
