---
sidebar_position: 6
title: "Use MCP with Pichkoo"
description: "A practical guide to connecting MCP servers to Pichkoo AI Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Pichkoo

This guide shows how to actually use MCP with Pichkoo AI Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Pichkoo tool
- you want Pichkoo to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Pichkoo to internal APIs, databases, or company systems without modifying Pichkoo core

Do not use MCP when:
- a built-in Pichkoo tool already solves the job well
...
- Pichkoo remains the agent
- MCP servers contribute tools
- Pichkoo discovers those tools at startup or reload time
...
If you installed Pichkoo with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Pichkoo banner/status should show MCP integration when configured
- ask Pichkoo what tools it has available
...
## WSL2: bridge Pichkoo in WSL to Windows Chrome

This is the practical setup when:

- Pichkoo runs inside WSL2
...
In this setup, Pichkoo does **not** connect to Chrome directly. Instead:

- Pichkoo runs in WSL
- Pichkoo starts a local stdio MCP server
...
```text
Pichkoo (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Pichkoo stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Pichkoo core browser transport
...
Once loaded, Pichkoo can use the MCP-prefixed browser tools directly. For example:
...
If Pichkoo runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Pichkoo from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Pichkoo from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Pichkoo:
...
2. Pichkoo-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Pichkoo will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Pichkoo to reason over a bounded workspace.
...
Now Pichkoo can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Pichkoo core.
...
Because Pichkoo now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
