---
sidebar_position: 7
title: "Use SOUL.md with Pichkoo"
description: "How to use SOUL.md to shape Pichkoo AI Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Pichkoo

`SOUL.md` is the **primary identity** for your Pichkoo instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Pichkoo to feel like the same assistant every time you talk to it — or if you want to replace the Pichkoo persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Pichkoo should be
- what Pichkoo should avoid stylistically
- how Pichkoo should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Pichkoo is and how Pichkoo speaks
...
Pichkoo now uses only the global SOUL file for the current instance:

```text
~/.pichkoo/SOUL.md
```

If you run Pichkoo with a custom home directory, it becomes:

```text
$PICHKOO_HOME/SOUL.md
```

## First-run behavior

Pichkoo automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Pichkoo does not overwrite it
- if the file exists but is empty, Pichkoo adds nothing from it to the prompt

## How Pichkoo uses it

When Pichkoo starts a session, it reads `SOUL.md` from `PICHKOO_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Pichkoo feels.
...
Pichkoo already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Pichkoo should sound.

# Avoid
What Pichkoo should not do.

# Defaults
How Pichkoo should behave when ambiguity appears.
```
...
Then restart Pichkoo or start a new session.
...
### I edited SOUL.md but Pichkoo still sounds the same

Check:
- you edited `~/.pichkoo/SOUL.md` or `$PICHKOO_HOME/SOUL.md`
...
### Pichkoo is ignoring parts of my SOUL.md
