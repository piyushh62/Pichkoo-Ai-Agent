...
3. xAI redirects back to Pichkoo and the tokens are saved to `~/.hermes/auth.json`.
4. From then on, Pichkoo refreshes the access token in the background — you stay signed in until you `hermes auth remove xai-oauth` or revoke access from your xAI account settings.
