...
description: "Frequently asked questions and solutions to common issues with Pichkoo AI Agent"
---
...
### What LLM providers work with Pichkoo?

Pichkoo AI Agent works with any OpenAI-compatible API. Supported providers include:
...
- let Pichkoo use the resulting MCP browser tools

This is more reliable than trying to force Pichkoo core browser transport to attach directly across the WSL2/Windows boundary.
...
Yes — Pichkoo now has a tested Termux install path for Android phones.
...
# Context length: 64000   ← Pichkoo minimum; set this to match your server's actual context window
```
...
Pichkoo persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Pichkoo — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Pichkoo auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `PICHKOO_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Pichkoo AI Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Pichkoo AI Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Pichkoo programmatically:
...
**Cause:** Pichkoo requires Python 3.11 or newer.
...
**Cause:** The conversation has grown too long for the model's context window, or Pichkoo detected the wrong context length for your model.
...
**Cause:** Pichkoo detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Pichkoo never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
:::warning
If an MCP server crashes mid-request, Pichkoo will report a timeout. Check the server's own logs (not just Pichkoo logs) to diagnose the root cause.
:::
...
Now when you tell Pichkoo "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Pichkoo do this?

**Current limitation:** Pichkoo profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Pichkoo reasoning, and tool call details in Telegram instead of just the final output.
...
**Current behavior:** Pichkoo creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Pichkoo to another machine
...
**Scenario:** After running the Pichkoo installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Pichkoo-specific issue — it's a shell config permissions problem.
...
1. **Search existing issues:** [GitHub Issues](https://github.com/piyushh62/Pichkoo-AI-Agent/issues)
