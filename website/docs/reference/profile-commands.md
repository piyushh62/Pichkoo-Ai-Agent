...
This page covers all commands related to [Pichkoo profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `pichkoo profile` without a subcommand shows help.
...
```bash
pichkoo profile use <name>
```

Sets `<name>` as the active profile. All subsequent `pichkoo` commands (without `-p`) will use this profile.
...
pichkoo profile use default
```

## `pichkoo profile create`

```bash
pichkoo profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `pichkoo profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `pichkoo update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.pichkoo`), use `pichkoo skills opt-out` / `pichkoo skills opt-in`. |
...
pichkoo profile create mybot

# Clone config only from current profile
pichkoo profile create work --clone

# Clone everything from current profile
pichkoo profile create backup --clone-all

# Clone config from a specific profile
pichkoo profile create work2 --clone --clone-from work
```

## `pichkoo profile describe`

```bash
pichkoo profile describe [<name>] [options]
```
...
pichkoo profile describe researcher

# Set it explicitly
pichkoo profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
pichkoo profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
pichkoo profile describe --all --auto
```

## `pichkoo profile delete`

```bash
pichkoo profile delete <name> [options]
```
...
```bash
pichkoo profile delete mybot
pichkoo profile delete mybot --yes
```
...
## `pichkoo profile show`

```bash
pichkoo profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Pichkoo home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ pichkoo profile show work
Profile: work
Path:    ~/.pichkoo/profiles/work
...
## `pichkoo profile alias`

```bash
pichkoo profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Pichkoo installation.
...
```bash
pichkoo profile alias work
# Creates/updates ~/.local/bin/work

pichkoo profile alias work --name mywork
# Creates ~/.local/bin/mywork

pichkoo profile alias work --remove
# Removes the wrapper script
```

## `pichkoo profile rename`

```bash
pichkoo profile rename <old-name> <new-name>
```
...
```bash
pichkoo profile rename mybot assistant
# ~/.pichkoo/profiles/mybot → ~/.pichkoo/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `pichkoo profile export`

```bash
pichkoo profile export <name> [options]
```
...
```bash
pichkoo profile export work
# Creates work.tar.gz in the current directory

pichkoo profile export work -o ./work-2026-03-29.tar.gz
```

## `pichkoo profile import`

```bash
pichkoo profile import <archive> [options]
```
...
```bash
pichkoo profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

pichkoo profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`pichkoo profile export` / `import` are still the right commands for
...
### `pichkoo profile install`

```bash
pichkoo profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `pichkoo -p telemetry`). |
...
pichkoo profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
pichkoo profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
pichkoo profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
pichkoo profile install ./telemetry/
```

### `pichkoo profile update`

```bash
pichkoo profile update <name> [--force-config] [--yes]
```
...
### `pichkoo profile info`

```bash
pichkoo profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Pichkoo version, author, env var requirements, the source URL/path, and
...
`pichkoo profile list` also shows the distribution name and version in a
`Distribution` column, and `pichkoo profile show <name>` / `delete <name>`
...
pichkoo profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
pichkoo profile install https://github.com/your-org/internal-assistant.git
```
...
`pichkoo_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Pichkoo version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Pichkoo can clone from.
3. Tell recipients to run `pichkoo profile install <your-repo-url>`.
...
## `pichkoo -p` / `pichkoo --profile`

```bash
pichkoo -p <name> <command> [options]
pichkoo --profile <name> <command> [options]
```

Global flag to run any Pichkoo command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
pichkoo -p work chat -q "Check the server status"
pichkoo --profile dev gateway start
pichkoo -p personal skills list
pichkoo -p work config edit
```

## `pichkoo completion`

```bash
pichkoo completion <shell>
```
...
- `pichkoo profile <TAB>` — subcommands (list, use, create, etc.)
- `pichkoo profile use <TAB>` — profile names
- `pichkoo -p <TAB>` — profile names
