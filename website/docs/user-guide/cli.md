...
:::tip
Pichkoo also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `hermes --tui` — see the [TUI](tui.md) guide.
:::
...
CLI sessions are stored in Pichkoo's SQLite state database under `~/.hermes/state.db`. The database keeps:
