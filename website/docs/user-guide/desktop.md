...
sidebar_position: 3
title: "Desktop App"
description: "The native Pichkoo desktop app — a polished experience for chatting with Pichkoo, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Pichkoo desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Pichkoo AI Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `pichkoo` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Pichkoo has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`pichkoo`) and **[TUI](./tui.md)** (`pichkoo --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`pichkoo dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Pichkoo Desktop](../getting-started/installation.md).

If you already have Pichkoo installed, simply run
...
- **The same conversation history** as every other Pichkoo surface — sessions started here resume in the CLI/TUI and vice versa.
...
Talk to Pichkoo and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `pichkoo model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Pichkoo management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#pichkoo-cron).
- **Profiles** — switch between [Pichkoo profiles](./profiles.md) (isolated config/skills/sessions).
...
The [manual update process](https://pichkoo-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Pichkoo agent, your config, and your chats stay. (Same as `pichkoo uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `pichkoo uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `pichkoo uninstall --full`.)
...
You can do the same from the terminal — `pichkoo uninstall --gui` for the GUI alone, or `pichkoo uninstall` / `pichkoo uninstall --full` for the agent too.

:::note
Running `pichkoo uninstall --gui` from a **source checkout** (a `pichkoo desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `pichkoo desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `pichkoo desktop`

To launch via the CLI, simply run `pichkoo desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.pichkoo/.env` (the secrets file, mode 0600):
...
Keep that `pichkoo dashboard` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.pichkoo/pichkoo-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
