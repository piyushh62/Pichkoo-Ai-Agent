...
Pichkoo ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.hermes/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Pichkoo Plugin](/guides/build-a-hermes-plugin) to write your own.
...
| `hermes-achievements` | dashboard tab | Steam-style collectible badges generated from your real Pichkoo session history |
...
Session grouping keys off the Pichkoo session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `hermes chat` session lives under one Langfuse session.

**Verify:**

```bash
hermes plugins list                 # observability/langfuse should show "enabled"
hermes chat -q "hello"              # check the Langfuse UI for a "Pichkoo turn" trace
```
...
Bundled plugins are written exactly like any other Pichkoo plugin — see [Build a Pichkoo Plugin](/guides/build-a-hermes-plugin). The only differences are:
