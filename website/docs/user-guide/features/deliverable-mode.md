...
When Pichkoo AI Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.pichkoo/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.pichkoo/config.yaml` (switchable per session
via `/personality`).
...
If you use Pichkoo' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.pichkoo/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Pichkoo AI Agent's deliverable
