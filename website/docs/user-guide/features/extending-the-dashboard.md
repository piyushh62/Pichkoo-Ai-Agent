...
description: "Build themes and plugins for the Pichkoo web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Pichkoo web dashboard (`pichkoo dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.pichkoo/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.pichkoo/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.pichkoo/dashboard-themes
```

```yaml
# ~/.pichkoo/dashboard-themes/neon.yaml
name: neon
label: Neon
...
The CSS is injected as a single scoped `<style data-pichkoo-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
|-------|---------|------------|--------|
| **Pichkoo Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Pichkoo Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Pichkoo Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
```yaml
# ~/.pichkoo/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
```bash
mkdir -p ~/.pichkoo/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.pichkoo/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.pichkoo/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
// Pichkoo API client + raw fetcher
...
#### Calling built-in Pichkoo endpoints
...
| `header-left` | Before the Pichkoo brand in the top bar. |
...
```json
// ~/.pichkoo/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
A reference plugin (`example-dashboard` in [`pichkoo-example-plugins`](https://github.com/NousResearch/pichkoo-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
```python
# ~/.pichkoo/plugins/my-plugin/dashboard/plugin_api.py
...
#### Accessing Pichkoo internals

Backend routes run inside the dashboard process, so they can import from the pichkoo-agent codebase directly:
...
The [`strike-freedom-cockpit`](https://github.com/NousResearch/pichkoo-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `pichkoo-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/NousResearch/pichkoo-example-plugins.git

# Theme
cp pichkoo-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.pichkoo/dashboard-themes/

# Plugin
cp -r pichkoo-example-plugins/strike-freedom-cockpit ~/.pichkoo/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Pichkoo Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
Check that the file is in `~/.pichkoo/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.pichkoo/logs/`.
...
2. Restart `pichkoo dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.pichkoo/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.pichkoo/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
