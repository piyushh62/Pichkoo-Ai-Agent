...
| **AI peer** (`aiPeer`) | One per Pichkoo profile. Host key `hermes` → default; `hermes.<profile>` for others. |
...
Scans every Pichkoo profile, creates host blocks for any profile without one, inherits settings from the default `hermes` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hermes-{identity}` → `hermes-coder`) to isolate memories per Pichkoo profile
