...
Pichkoo AI Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
pichkoo memory setup      # interactive picker + configuration
pichkoo memory status     # check what's active
pichkoo memory off        # disable external provider
```

You can also select the active memory provider via `pichkoo plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.pichkoo/config.yaml`:
...
When a memory provider is active, Pichkoo automatically:
...
**Setup Wizard:**
```bash
pichkoo memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `pichkoo honcho setup` command still works (it now redirects to `pichkoo memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$PICHKOO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$PICHKOO_HOME/honcho.json` > `~/.pichkoo/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/piyushh62/Pichkoo-AI-Agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/pichkoo).
...
:::tip Migrating from `pichkoo honcho`
If you previously used `pichkoo honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Pichkoo profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Pichkoo profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Pichkoo profile. Host key `pichkoo` → default; `pichkoo.<profile>` for others. |
...
```bash
pichkoo profile create coder --clone
```

`--clone` creates a `pichkoo.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Pichkoo profile, creates host blocks for any profile without one, inherits settings from the default `pichkoo` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/piyushh62/Pichkoo-AI-Agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/pichkoo).
...
pichkoo memory setup    # select "openviking"
# Or manually:
pichkoo config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.pichkoo/.env
```
...
**Config:** `$PICHKOO_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `pichkoo-user` | User identifier |
...
**Setup:**
```bash
pichkoo memory setup    # select "hindsight"
# Or manually:
pichkoo config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.pichkoo/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p pichkoo ui start`

**Config:** `$PICHKOO_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `pichkoo` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Pichkoo AI Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/piyushh62/Pichkoo-AI-Agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
pichkoo memory setup    # select "holographic"
# Or manually:
pichkoo config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.pichkoo-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$PICHKOO_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
pichkoo memory setup    # select "retaindb"
# Or manually:
pichkoo config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.pichkoo/.env
```
...
- Knowledge tree stored at `$PICHKOO_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
pichkoo memory setup    # select "supermemory"
# Or manually:
pichkoo config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.pichkoo/.env
```

**Config:** `$PICHKOO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `pichkoo` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `pichkoo-{identity}` → `pichkoo-coder`) to isolate memories per Pichkoo profile
...
| **Requires** | `pip install pichkoo-memori` + `pichkoo-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install pichkoo-memori
pichkoo-memori install
pichkoo config set memory.provider memori
pichkoo memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `pichkoo-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$PICHKOO_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$PICHKOO_HOME/` so each profile has its own credentials
