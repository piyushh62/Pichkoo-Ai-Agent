...
PICHKOO_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
- **CLI — `pichkoo dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `pichkoo setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `PICHKOO_DASHBOARD_OAUTH_CLIENT_ID` into `~/.pichkoo/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  pichkoo dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes PICHKOO_DASHBOARD_OAUTH_CLIENT_ID to ~/.pichkoo/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `PICHKOO_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `PICHKOO_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `pichkoo dashboard register` |
...
**1. Log in and register the dashboard.** `pichkoo dashboard register` uses your existing Nous login to provision an OAuth client and writes `PICHKOO_DASHBOARD_OAUTH_CLIENT_ID` into `~/.pichkoo/.env` for you:
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://pichkoo.example.com/auth/callback` and set `PICHKOO_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `PICHKOO_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `PICHKOO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `PICHKOO_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `PICHKOO_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `PICHKOO_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
cat >> ~/.pichkoo/.env <<EOF
PICHKOO_DASHBOARD_BASIC_AUTH_USERNAME=admin
PICHKOO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `PICHKOO_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `PICHKOO_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `PICHKOO_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
```bash
export PICHKOO_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/pichkoo"
export PICHKOO_DASHBOARD_OIDC_CLIENT_ID="pichkoo-dashboard"
export PICHKOO_DASHBOARD_PUBLIC_URL="http://localhost:9119"
pichkoo dashboard --host 0.0.0.0 --port 9119 --no-open
```

`PICHKOO_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `PICHKOO_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `PICHKOO_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `PICHKOO_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$PICHKOO_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```bash
# Quick env-var path.
PICHKOO_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  pichkoo dashboard --host 0.0.0.0
...
Prefer no plaintext at rest? Use `PICHKOO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.
...
The session refreshes automatically and survives restarts when `PICHKOO_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `PICHKOO_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `PICHKOO_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `PICHKOO_DASHBOARD_BASIC_AUTH_USERNAME` / `PICHKOO_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `PICHKOO_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
| Theme | Character |
|-------|-----------|
| **Pichkoo Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Pichkoo Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
