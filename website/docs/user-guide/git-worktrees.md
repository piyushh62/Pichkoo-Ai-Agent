...
title: "Git Worktrees"
description: "Run multiple Pichkoo agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Pichkoo AI Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Pichkoo so each session has a clean, isolated working directory.

## Why Use Worktrees with Pichkoo?

Pichkoo treats the **current working directory** as the project root:

- CLI: the directory where you run `pichkoo` or `pichkoo chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.pichkoo/config.yaml`
...
git worktree add ../repo-feature feature/pichkoo-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/pichkoo-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Pichkoo there:

```bash
cd ../repo-feature

# Start Pichkoo in the worktree
pichkoo
```
...
git worktree add ../repo-experiment-a feature/pichkoo-a
git worktree add ../repo-experiment-b feature/pichkoo-b
```
...
Each Pichkoo process:

- Works on its own branch (`feature/pichkoo-a` vs `feature/pichkoo-b`).
...
- Pichkoo checkpoint data under `~/.pichkoo/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Pichkoo experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/pichkoo-checkpoints-docs`, `feature/pichkoo-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Pichkoo from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `pichkoo -w` (Automatic Worktree Mode)

Pichkoo has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `pichkoo/pichkoo-<hash>`).
...
```bash
pichkoo -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `pichkoo -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Pichkoo session its own clean checkout.
