...
Connect Pichkoo to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Pichkoo and the BlueBubbles server
...
By default, Pichkoo responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Pichkoo uses conservative defaults for `Pichkoo` and `@Pichkoo agent` variants.
...
```
iMessage → Messages.app → BlueBubbles Server → Webhook → Pichkoo
Pichkoo → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Pichkoo sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Pichkoo resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
