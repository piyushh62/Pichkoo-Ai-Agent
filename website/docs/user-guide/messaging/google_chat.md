...
sidebar_position: 12
title: "Google Chat"
description: "Set up Pichkoo AI Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Pichkoo AI Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Pichkoo
...
> Run `pichkoo gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Pichkoo can read (e.g.,
`~/.pichkoo/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a pichkoo restart)
...
- Principal: `pichkoo-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Pichkoo calls
...
- **App name**: whatever you want users to see ("Pichkoo" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/pichkoo-chat-events`.
...
`ADDED_TO_SPACE` event that Pichkoo uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Pichkoo

Add the Google Chat section to `~/.pichkoo/.env`:
...
Install the dependencies the Google Chat adapter needs (no Pichkoo extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Pichkoo is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Pichkoo detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Pichkoo session.
...
3. Download the JSON. Move it onto the host that runs Pichkoo.
4. Register the client with Pichkoo (run under the profile you want it scoped to):
...
pichkoo -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Pichkoo home (e.g.
`~/.pichkoo/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.pichkoo/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.pichkoo/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Pichkoo isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `pichkoo gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Pichkoo will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.pichkoo/google_chat_user_tokens/<sanitized_email>.json` (filesystem
