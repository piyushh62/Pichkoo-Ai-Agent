...
description: Control your smart home with Pichkoo AI Agent via Home Assistant integration.
...
Pichkoo AI Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Pichkoo AI Agent"
5. Copy the token
...
Pichkoo AI Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.pichkoo/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Pichkoo AI Agent".
...
The adapter reads credentials from `~/.pichkoo/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Pichkoo profile
...
running Pichkoo — `curl -H "Authorization: Bearer <token>" <url>/api/` should
