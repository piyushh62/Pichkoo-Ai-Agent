...
description: "Chat with Pichkoo from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Pichkoo from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Pichkoo](/guides/use-voice-mode-with-pichkoo).
...
```mermaid
flowchart TB
    subgraph Gateway["Pichkoo Gateway"]
...
```bash
pichkoo gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
pichkoo gateway              # Run in foreground
pichkoo gateway setup        # Configure messaging platforms interactively
pichkoo gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo pichkoo gateway install --system   # Linux only: install a boot-time system service
pichkoo gateway start        # Start the default service
pichkoo gateway stop         # Stop the default service
pichkoo gateway status       # Check default service status
pichkoo gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Pichkoo AI Agent to the latest version |
...
Configure per-platform overrides in `~/.pichkoo/gateway.json`:
...
pichkoo pairing approve telegram XKGH5N7P

# Other pairing commands:
pichkoo pairing list          # View pending + approved users
pichkoo pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Pichkoo appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.pichkoo/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.pichkoo/config.yaml`:
...
```bash
pichkoo gateway install               # Install as user service
pichkoo gateway start                 # Start the service
pichkoo gateway stop                  # Stop the service
pichkoo gateway status                # Check status
journalctl --user -u pichkoo-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo pichkoo gateway install --system
sudo pichkoo gateway start --system
sudo pichkoo gateway status --system
journalctl -u pichkoo-gateway -f
```
...
```bash
pichkoo gateway install               # Install as launchd agent
pichkoo gateway start                 # Start the service
pichkoo gateway stop                  # Stop the service
pichkoo gateway status                # Check status
tail -f ~/.pichkoo/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.pichkoo.gateway.plist`. It includes three environment variables:
...
|----------|---------|--------------|
| CLI | `pichkoo-cli` | Full access |
| Telegram | `pichkoo-telegram` | Full tools including terminal |
| Discord | `pichkoo-discord` | Full tools including terminal |
| WhatsApp | `pichkoo-whatsapp` | Full tools including terminal |
| Slack | `pichkoo-slack` | Full tools including terminal |
| Google Chat | `pichkoo-google_chat` | Full tools including terminal |
| Signal | `pichkoo-signal` | Full tools including terminal |
| SMS | `pichkoo-sms` | Full tools including terminal |
| Email | `pichkoo-email` | Full tools including terminal |
| Home Assistant | `pichkoo-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `pichkoo-mattermost` | Full tools including terminal |
| Matrix | `pichkoo-matrix` | Full tools including terminal |
| DingTalk | `pichkoo-dingtalk` | Full tools including terminal |
| Feishu/Lark | `pichkoo-feishu` | Full tools including terminal |
| WeCom | `pichkoo-wecom` | Full tools including terminal |
| WeCom Callback | `pichkoo-wecom-callback` | Full tools including terminal |
| Weixin | `pichkoo-weixin` | Full tools including terminal |
| BlueBubbles | `pichkoo-bluebubbles` | Full tools including terminal |
| QQBot | `pichkoo-qqbot` | Full tools including terminal |
| Yuanbao | `pichkoo-yuanbao` | Full tools including terminal |
| Microsoft Teams | `pichkoo-teams` | Full tools including terminal |
| API Server | `pichkoo-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `pichkoo-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.pichkoo/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
