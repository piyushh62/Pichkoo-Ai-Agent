...
Connect Pichkoo to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Pichkoo — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Pichkoo runs it in a
...
2. **Finds or creates** the `Pichkoo AI Agent` project on your account.
...
Photon uses the same authorization model as every other Pichkoo
...
By default Pichkoo responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Pichkoo` and
`@Pichkoo agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Pichkoo will reply.
...
assigned iMessage line Pichkoo uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** Pichkoo sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Pichkoo wake words  | JSON list / comma / newline regex patterns for group mentions |
