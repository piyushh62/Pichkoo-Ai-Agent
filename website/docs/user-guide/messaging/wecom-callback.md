...
Connect Pichkoo to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Pichkoo supports two WeCom integration modes:
...
> Run `pichkoo gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Pichkoo decrypts the message, queues it for the agent
...
(Use `pichkoo gateway start` only after `pichkoo gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Pichkoo and the token the
...
in `~/.pichkoo/.env` values around `=` will also break signature checks. After
fixing, restart `pichkoo gateway run`.
...
Check `pichkoo gateway run` logs for the bound host/port. If the adapter bound to
