...
2. Pichkoo stores that single token in `~/.hermes/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `hermes` (or the gateway, or a cron job) starts, after `~/.hermes/.env` has loaded, Pichkoo calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
