...
Pichkoo AI Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Pichkoo checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.
...
| `timeout` | `60` | Seconds Pichkoo waits for an approval reply before timing out. |
...
3. **Environment variable**: Set `PICHKOO_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `PICHKOO_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Pichkoo shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Pichkoo refuses to run them **regardless** of:
...
The `PICHKOO_EXEC_ASK=1` environment variable is automatically set when running the gateway.
...
When running the messaging gateway, Pichkoo controls who can interact with the bot through a layered authorization system.
...
For more flexible authorization, Pichkoo includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.
...
When using the `docker` terminal backend, Pichkoo applies strict security hardening to every container.
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `PICHKOO_HOME`. Skills declare these in frontmatter:
...
When loaded, Pichkoo checks if these files exist in the active profile's `PICHKOO_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Pichkoo infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Pichkoo infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Pichkoo integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Pichkoo under WSL.
