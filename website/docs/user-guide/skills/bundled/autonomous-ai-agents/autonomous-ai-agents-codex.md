...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`pichkoo-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-pichkoo-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Pichkoo terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Pichkoo itself, `model.provider: openai-codex` uses Pichkoo-managed Codex
OAuth from `~/.pichkoo/auth.json` after `pichkoo auth add openai-codex`. For the
...
## Pichkoo Gateway Caveat

When invoking the Codex CLI from a Pichkoo gateway/service context (for example,
