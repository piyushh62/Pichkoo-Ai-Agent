...
| | |
|---|---|
| Source | Optional — install with `pichkoo skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Pichkoo or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Pichkoo. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.pichkoo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.pichkoo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Pichkoo integration, either:

- configure the server in `~/.pichkoo/config.yaml` using the `native-mcp` skill, or
...
### Pichkoo cannot see the deployed server

The server-building part may be correct while the Pichkoo config is not. Load the `native-mcp` skill and configure the server in `~/.pichkoo/config.yaml`, then restart Pichkoo.
