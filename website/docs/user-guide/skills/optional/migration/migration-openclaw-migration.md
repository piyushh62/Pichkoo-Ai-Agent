---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Pichkoo AI Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Pichkoo AI Agent"
---
...
Migrate a user's OpenClaw customization footprint into Pichkoo AI Agent. Imports Pichkoo-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
# OpenClaw -> Pichkoo Migration

Use this skill when a user wants to move their OpenClaw setup into Pichkoo AI Agent with minimal manual cleanup.
...
It uses `scripts/openclaw_to_pichkoo.py` to:

- import `SOUL.md` into the Pichkoo home directory as `SOUL.md`
...
With `--migrate-secrets`, it will also import a small allowlisted set of Pichkoo-compatible secrets, currently:
...
Pichkoo CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Pichkoo skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Pichkoo unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Pichkoo targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Pichkoo destination. Unsupported auth blobs must still be reported as skipped.
