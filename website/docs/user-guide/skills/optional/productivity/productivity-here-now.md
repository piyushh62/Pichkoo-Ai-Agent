...
| | |
|---|---|
| Source | Optional — install with `pichkoo skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${PICHKOO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${PICHKOO_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `pichkoo`)    |
