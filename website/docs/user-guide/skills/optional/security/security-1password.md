...
| | |
|---|---|
| Source | Optional — install with `pichkoo skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Pichkoo AI Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Pichkoo terminal calls (desktop app flow only)
...
### Service Account (recommended for Pichkoo)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.pichkoo/.env` (the skill will prompt for this on first load).
...
## Pichkoo Execution Pattern (desktop app flow)

Pichkoo terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/pichkoo-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/pichkoo-op.sock"
