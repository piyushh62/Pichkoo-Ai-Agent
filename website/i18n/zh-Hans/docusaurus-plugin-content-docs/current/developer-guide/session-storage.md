...
Pichkoo AI Agent 使用 SQLite 数据库（`~/.pichkoo/state.db`）跨 CLI 和 gateway 会话持久化会话元数据、完整消息历史及模型配置。这替代了早期的逐会话 JSONL 文件方案。

源文件：`pichkoo_state.py`


## 架构概览

```
~/.pichkoo/state.db (SQLite, WAL mode)
...
多个 pichkoo 进程（gateway + CLI 会话 + worktree agent）共享同一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
...
db = SessionDB()                           # 默认：~/.pichkoo/state.db
...
该路径由 `pichkoo_constants.get_pichkoo_home()` 推导，默认解析为 `~/.pichkoo/`，或 `PICHKOO_HOME` 环境变量的值。
