...
若两个键均未设置，Pichkoo 将根据环境中存在的 API key/URL 自动检测后端。`hermes tools` 会引导用户完成选择。
...
| `requires_env` | 在 `hermes plugins install` 期间进行交互式凭据提示（富格式说明参见[构建 Pichkoo 插件](/guides/build-a-hermes-plugin#gate-on-environment-variables)） |
...
如果你的提供商封装了第三方 SDK（如 DDGS 封装了 `ddgs` 包），请勿在模块顶层 `import`。在 `is_available()` 或 `search()` 内部使用 `tools.lazy_deps.ensure(...)` ——Pichkoo 将在首次使用时安装该包，并受 `security.allow_lazy_installs` 控制。安全模型详见[构建 Pichkoo 插件 → 懒加载](/guides/build-a-hermes-plugin#lazy-install-optional-python-dependencies)。
...
- [构建 Pichkoo 插件](/guides/build-a-hermes-plugin) — 通用工具/hook/斜杠命令指南