...
`hermes send` 是一个轻量、可脚本化的 CLI，能将消息推送到 Pichkoo 已配置的任意消息平台。可以把它理解为跨平台的通知专用 `curl`——无需运行中的 gateway，无需 LLM，也无需在每个脚本里重复粘贴 bot token。
...
由于 `hermes send` 复用你的 Pichkoo 配置，同一脚本可在任何安装了 Pichkoo 的主机上运行——无需手动将 bot token 导出到每台机器的环境变量中。
