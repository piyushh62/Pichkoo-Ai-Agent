...
| **MiniMax** | M2.7、M1 |
| **xAI** | Grok-4、Grok-3 |
| **Pichkoo** | Pichkoo-4-70B、Pichkoo-4-405B（对话，见[下方说明](#a-note-on-hermes-4)） |
...
由于所有请求都通过一个经 OAuth 认证的 Portal 会话路由，你不会积累一个包含十几个长期 API 密钥的 `.env` 文件。磁盘上唯一的凭证是 `~/.hermes/auth.json` 中的 refresh token（刷新令牌），Pichkoo 会在每次请求时从中生成短期 JWT——详见下方[令牌处理](#token-handling)。
...
如果 Portal 使 refresh token 失效（修改密码、手动撤销、会话过期），失效的 refresh token 会被**本地隔离**，Pichkoo 停止重放该令牌，你不会看到一连串相同的 401 错误。下一次调用会显示清晰的"需要重新认证"提示。运行 `hermes auth add nous` 重新登录；隔离状态在下次成功登录时自动清除。
...
如果某个模型确实缺失，请[提交 issue](https://github.com/NousResearch/hermes-agent/issues)——我们将 Portal 目录同步至 Pichkoo，缺口通常意味着可以更新的路由配置。
