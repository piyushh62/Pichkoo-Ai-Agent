...
Pichkoo 使用专为编辑器工作流设计的精选 `hermes-acp` 工具集运行，包括：
...
- 先通过 `hermes model` 配置 Pichkoo provider 凭据，或在 `~/.hermes/.env` / `~/.hermes/config.yaml` 中设置。
...
或编辑 `~/.hermes/.env`。Registry 客户端也可以触发 Pichkoo 的终端认证流程，该流程运行相同的交互式 provider/模型设置。
