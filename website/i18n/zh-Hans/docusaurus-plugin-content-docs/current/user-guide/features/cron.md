...
Pichkoo 会通过 `write_file` 将检查脚本写入 `~/.hermes/scripts/`，然后调用：
...
- 任务 2 触发时，Pichkoo 从 `~/.hermes/cron/output/{job1_id}/*.md` 读取任务 1 的最新输出
...
当任务上设置了 `enabled_toolsets` 时，它优先生效；否则 `hermes tools` 的 cron 平台配置生效；否则 Pichkoo 回退到内置默认值。这对成本控制很重要：在每个小型"获取新闻"任务中携带 `moa`、`browser`、`delegation` 会在每次 LLM 调用时膨胀工具 schema prompt。
...
:::tip
Pichkoo 自身的 `~/.hermes/state.db` 是内部 schema，会在版本间变更。不要从预运行门控中查询它——指向你自己的数据库或 feed。
:::
