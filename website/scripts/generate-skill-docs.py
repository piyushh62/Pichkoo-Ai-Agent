...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/piyushh62/Pichkoo-AI-Agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    pichkoo_meta = (fm.get("metadata") or {}).get("pichkoo") or {}
    tags = pichkoo_meta.get("tags") or []
    related = pichkoo_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `pichkoo skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Pichkoo AI Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Pichkoo ships with a large built-in skill library copied into `~/.pichkoo/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Pichkoo also syncs bundled skills on `pichkoo update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.pichkoo/skills/` tree, it is still shipped with Pichkoo; restore it with `pichkoo skills reset <name> --restore`.",
        "",
